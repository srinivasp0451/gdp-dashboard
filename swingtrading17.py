...
def generate_confluence_signals(df_local, params, side="Both", signal_mode="Both", advanced_leading=True):
    ...
    for idx in df_calc.index:
        ...
        # ----------------- Leading: candlestick patterns & breakouts -----------------
...
def backtest_point_strategy(df_signals, params):
    ...
    for i in range(n):
        ...
        # 1) If currently in position -> check exits using THIS row (future relative to entry)
        if in_pos:
        ...
        if (not in_pos) and sig != 0:
            ...
            in_pos = True
            # do not check exits on the same bar where we entered
            continue
...
random_iters = st.number_input("Random iterations (1-2000)", min_value=1, max_value=2000, value=100, step=1)
    ...
    if run_btn:
        ...
        with st.spinner("Running optimization on restricted dataset..."):
            ...
            def explain_indicator(it):
                ...
                if it == 'BEAR_ENGULF':
                    return "Bearish engulfing: current bearish candle engulfs previous bullish candle (early reversal)"
                if it == 'HAMMER':
                    return "Hammer: long lower shadow with small body — possible bullish reversal"
                if it == 'HANGING_MAN':
