...
with tab_opt:
    ...
    st.caption("Runs backtests across chosen strategy / timeframe / period / stoploss / target combinations. Keep the grid small — each combo triggers a rate-limited yfinance call and its own backtest loop.")
    ...
    n_combos = (len(opt_strategies) * len(opt_sl_types) * len(opt_target_types)
    ...
    if n_combos > MAX_COMBOS:
        st.error(f"That's {n_combos} combinations — over the safety cap of {MAX_COMBOS}. Narrow your selections (fewer strategies/timeframes/periods/SL-Target types) before running.")
    ...
    if st.button("🧪 Run Optimization", type="primary", disabled=run_disabled):
        ...
        data_cache = {}
        for idx, (s, iv, p, slt, tgt) in enumerate(combos):
            cache_key = (iv, p)
            if cache_key not in data_cache:
                data_cache[cache_key] = fetch_data(ticker, iv, p)
            raw = data_cache[cache_key]
            if not raw.empty and len(raw) >= 30:
                tdf, _ = run_backtest(raw, s, slt, tgt, params, filters, qty)
                m = compute_metrics(tdf)
                rows.append({"Strategy": s, "Timeframe": iv, "Period": p, "SL Type": slt, "Target Type": tgt, **m})
            progress.progress((idx + 1) / max(len(combos), 1), text=f"{s} · {iv}/{p} · {slt[:15]}/{tgt[:15]}")
        progress.empty()
    ...
    if isinstance(results, pd.DataFrame) and not results.empty:
        ...
        st.success(f"🏆 Best overall by {rank_metric}: **{best_overall['Strategy']}** · {best_overall['Timeframe']}/{best_overall['Period']} · "
                   f"SL: {best_overall['SL Type']} · Target: {best_overall['Target Type']} "
...
with tab_spread:
    ...
    if st.button("🔍 Fetch & Compare"):
        ...
        if p1 is None or p2 is None:
            ...
            st.caption("Where does today's difference sit relative to its own 2-year history, and how did each ticker behave afterwards when the difference was in that same range?")
            with st.spinner("Fetching 2 years of daily history for both tickers…"):
                h1 = fetch_data(t1, "1d", "2y")
                h2 = fetch_data(t2, "1d", "2y")

            if h1.empty or h2.empty:
                st.warning("Not enough historical data for one of the tickers to run bin analysis.")
            else:
                ...
                if len(joined) < 30:
                    st.warning("Not enough overlapping history between these two tickers for a reliable bin analysis.")
                else:
                    n_bins = 8
                    try:
                    ...
                    st.dataframe(
                        display_stats.rename(columns={
                            ...
                            "t1_avg_fwd_pct": f"{t1_name} Avg Fwd {fwd_n}d %", "t1_pct_up": f"{t1_name} % Up",
                            "t2_avg_fwd_pct": f"{t2_name} Avg Fwd {fwd_n}d %", "t2_pct_up": f"{t2_name} % Up",
                        }),
                    ...
                    st.info(
                        ...
                        f"[{current_row['diff_lo']:.1f} to {current_row['diff_hi']:.1f}], seen {int(current_row['n'])} times in the last 2 years. "
                        f"In the {fwd_n} trading days *after* being in this bin, historically **{t1_name}** averaged "
                    ...
                    if bias != "NEUTRAL":
                        # SL/Target sized from ticker1's OWN volatility (ATR), never from the
                        # raw cross-instrument price gap. Using the raw diff as a distance was
                        # exactly the bug that produced the nonsensical levels you flagged
                        # (e.g. an SL nearly double the entry price, and a negative target) —
                        # that happens whenever the two instruments trade on very different
                        # scales (e.g. Nifty vs Sensex vs Gold). The diff is only used here to
                        # look up historical conditional behavior, never to size risk.
                        a1 = atr(h1, 14).iloc[-1] if len(h1) > 20 and not np.isnan(atr(h1, 14).iloc[-1]) else p1 * 0.01
                        ...
                        st.markdown(
                            f"**Reference levels for {t1_name}** (sized off its own 14-period ATR ≈ {a1:.2f}, "
                            f"*not* the raw price gap): Entry ≈ `{entry_ref:.2f}` · SL ≈ `{sl_ref:.2f}` · Target ≈ `{tgt_ref:.2f}`"
                        )

                    st.caption(
                        "This is an empirical conditional-return lookup, not a validated statistical-arbitrage model — "
                        "small sample sizes in a bin (check the '# Occurrences' column) make the average unreliable. "
                        "Treat as a directional hint only, and confirm with your chosen strategy/indicators before entering."
                    )
