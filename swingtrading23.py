# streamlit_swing_recommender_fixed.py
...
# -------------------------
# Utilities: column mapping / standardize
# -------------------------
...
def standardize_df_from_file(raw):
    ...
    # localize to UTC then convert to IST safely (handles tz-naive vs tz-aware)
    try:
    ...
    if "Close" not in df.columns:
        possibles = [c for c in df.columns if re.search(r"\bprice\b", str(c).lower())]
        if possibles:
...
def get_pivots(price_series, left=3, right=3):
    ...
    L = len(s)
    for i in range(left, max(left, L - right)):
...
def generate_signals_and_meta(df, params):
    ...
    sup_zones = [(float(lv - lv * params["zone_width"]), float(lv + lv * params["zone_width"])) for lv in supports]
    res_zones = [(float(lv - lv * params["zone_width"]), float(lv + lv * params["zone_width"])) for lv in resistances]
    ...
    for p in patterns:
        ...
        pat_map.setdefault(int(end), []).append(p[0])

    for i in range(len(df)):
        score = 0.0
        fired = []
        price = float(df.loc[i, "Close"]) if not pd.isna(df.loc[i, "Close"]) else 0.0
        for p in pat_map.get(i, []):
            score += weights.get(p, 0)
            fired.append(p)
        # trend
        try:
            if price > float(df.loc[i, "ema50"]):
                score += 0.5
                fired.append("trend_long")
            else:
                score -= 0.5
                fired.append("trend_short")
        except Exception:
            pass
        # rsi
        try:
            if df.loc[i, "rsi"] > 55:
                score += 0.5
                fired.append("rsi_long")
            if df.loc[i, "rsi"] < 45:
                score -= 0.5
                fired.append("rsi_short")
        except Exception:
            pass
        # volume
        try:
            if vol_med > 0 and float(df.loc[i, "Volume"]) > vol_med * params["volume_factor"]:
                score += 0.5
        ...
        try:
            ...
            if h - max(c, o) > params["wick_factor"] * body:
                score -= 1.0
                fired.append("upper_wick")
            if min(c, o) - l > params["wick_factor"] * body:
                score += 1.0
                fired.append("lower_wick")
        except Exception:
            pass
...
# -------------------------
# Backtest (same conservative intra-candle logic)
# -------------------------
def backtest(df_signals, params):
    ...
    while i < L - 1:
        ...
        for j in range(entry_idx, min(L, entry_idx + params["max_hold"] + 1)):
            ...
            # conservative: assume SL hit before TP in same candle
            if sig == 1:
                if day_low <= sl:
                    exit_price = sl
                    exit_idx = j
...
# -------------------------
# Small helpers: random/grid param sampling (defined so NameError doesn't appear)
# -------------------------
...
# -------------------------
# find_best_on_train + walk-forward CV
# -------------------------
def find_best_on_train(df_train, search_iters=80, params_base=None):
    ...
    for _ in range(search_iters):
        s = {
            "pivot_window": random.choice([2, 3, 4]),
            "cluster_tol": random.choice([0.003, 0.005, 0.01]),
            "zone_width": random.choice([0.003, 0.005]),
            "sl_pct": random.choice([0.005, 0.01]),
            "tp_pct": random.choice([0.01, 0.02]),
            "max_hold": random.choice([3, 5, 7]),
            "breakout_lookback": random.choice([3, 5]),
            "pattern_tol": random.choice([0.01, 0.02]),
            "min_bars_between": random.choice([2, 3]),
            "signal_threshold": random.choice([2.5, 3.0]),
        ...
        if metric > best_metric:
            best_metric = metric
            best = {"params": params, "stats": stats, "trades": trades}
    return best, tried

def walk_forward_cv(df, train_days=365 * 2, test_days=90, step_days=90, search_iters=80, params_base=None):
    stats_list = []
    start = df["Date"].min()
    end = df["Date"].max()
...
# -------------------------
# Visualization helper (robust)
# -------------------------
def plot_with_overlays(df, meta, trades=None, title="Price with overlays"):
    df_plot = df.copy()
    # convert Date to tz-naive index for mplfinance stability
    df_plot["Date"] = pd.to_datetime(df_plot["Date"])
    df_plot = df_plot.set_index("Date")
    df_plot.index = pd.DatetimeIndex(df_plot.index)  # ensure DatetimeIndex
    # prepare hlines (unique floats)
    hlines = []
    colors = []
    supports = meta.get("supports", []) or []
    resistances = meta.get("resistances", []) or []
    # convert to float and unique
    for s in supports:
        try:
            val = float(s)
            if val not in hlines:
                hlines.append(val)
                colors.append("green")
        except Exception:
            continue
    ...
    hlines_dict = None
    if hlines:
        hlines_dict = dict(hlines=hlines, colors=colors, linewidths=1, alpha=0.6)

    addplots = []
    try:
        if trades is not None and not trades.empty:
            # build entry/exit series aligned with df_plot index
            ...
            for _, t in trades.iterrows():
                try:
                    ...
                    if len(df_plot.index) == 0:
                        continue
                    ei = df_plot.index.get_indexer([et], method="nearest")[0]
                    xi = df_plot.index.get_indexer([xt], method="nearest")[0]
                    if 0 <= ei < len(df_plot):
                        entries.iloc[ei] = t["entry_price"]
                    if 0 <= xi < len(df_plot):
                        exits.iloc[xi] = t["exit_price"]
                except Exception:
                    continue
            if entries.notna().any():
                addplots.append(mpf.make_addplot(entries, type="scatter", markersize=75, marker="^"))
            if exits.notna().any():
                addplots.append(mpf.make_addplot(exits, type="scatter", markersize=75, marker="v"))
        # use mpf plot; if it fails we fallback to matplotlib
        ...
        ax.set_title(title)
        for lvl in hlines:
            ax.axhline(lvl, color="green" if lvl in supports else "red", alpha=0.4)
        return fig
...
# -------------------------
def app():
    st.title("Swing Trading Recommender — Fixed")
    st.markdown("Fetch data, optimize, run walk-forward CV, and generate live recommendations. The live recommendation uses the exact same logic as the backtest (entry at next open estimate).")

    with st.sidebar:
        st.header("Data source")
        source = st.radio("Choose source", ["Nifty50 (yfinance)", "Upload file"])
        ...
        if source.startswith("Nifty"):
            ...
            timeframe = st.selectbox("Interval", ["1d", "1wk", "1mo"])
            period = st.selectbox("Period", ["6mo", "1y", "2y", "5y", "max"])
            st.markdown("Press **Fetch data** to download from yfinance (avoids repeated calls).")
        else:
    ...
    if source.startswith("Nifty"):
        if st.button("Fetch data"):
            st.info("Fetching data from yfinance (may be rate-limited).")
            try:
            ...
            if raw is None or raw.empty:
                st.error("No data returned by yfinance. Try another ticker/period/interval.")
                return
            ...
            st.success(f"Fetched {len(df_fetched)} rows.")
    else:
        if "upload" in locals() and upload is not None:
            ...
            st.success(f"Processed file with {len(df_fetched)} rows.")

    if st.session_state["fetched_df"] is None:
        st.info("No data. Use Fetch data or upload a file.")
        return
    ...
    if heat.empty:
        ...
        try:
            ...
            axh.set_xticklabels([str(x) for x in heat.columns])
            axh.set_yticks(range(len(heat.index)))
            axh.set_yticklabels([str(x) for x in heat.index])
            norm = plt.Normalize(np.nanmin(heat_vals), np.nanmax(heat_vals)) if np.nanmin(heat_vals) != np.nanmax(heat_vals) else plt.Normalize(-1, 1)
            ...
            st.warning("Heatmap plotting failed; showing table: " + str(e))
            st.dataframe(heat)
    ...
    st.subheader("Optimization (use current fetched data)")
    if st.button("Run Optimization (use fetched data)"):
    ...
    # Live recommendation (same logic)
    ...
    if st.button("Generate Live Recommendation"):
        ...
        if sig == 0:
            ...
            rec = {
                "direction": "long" if sig == 1 else "short",
                "entry_next_open_est": round(next_open_est, 4),
                "stop_loss": round(sl, 4),
                "target": round(tp, 4),
                "reason": last_sig_row["reason"],
                "probability_of_profit": stats_bt.get("accuracy", None),
                "suggested_units": suggested_units,
            }
            st.json(rec)
            st.write("Strategy backtest stats:")
            st.json(stats_bt)

    # Chart with overlays
    st.subheader("Chart with overlays (last N bars)")
    ...
    try:
        ...
        st.error(f"Chart plotting failed: {e}")
