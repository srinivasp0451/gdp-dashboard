...
def live_trading_iteration():
    ...
    • current_data        → updated from LTP FIRST, before anything else
    • Candle fetch        → runs in a daemon background thread so it NEVER
                            blocks the 0.3 s LTP / P&L / SL display cycle
    • apply pending       → result of the background fetch is picked up on the
                            next tick after it completes (zero blocking)

    This guarantees LTP, live P&L, and SL/Target checks update every tick
    ...
    #           This runs BEFORE the candle fetch so P&L / SL / Target
    #           checks always use the freshest possible price.
    # ══════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Step 3 — Visual forming-candle update (fast, no API)
    # ══════════════════════════════════════════════════════════════════════
    ...
    #            (zero-cost: just reads a session_state key)
    # ══════════════════════════════════════════════════════════════════════
    pending_result = st.session_state.pop('_pending_candle_result', None)
    if pending_result is not None:
        df_new   = pending_result.get('df')
        new_ts   = pending_result.get('ts')
        is_stale = pending_result.get('stale', False)
        fetch_ok = pending_result.get('ok', False)

        if fetch_ok and df_new is not None and not df_new.empty and not is_stale:
            ...
            df = df_new
            add_log(f"✅ Candles updated: {len(df_new)} bars, latest {new_ts}")
        elif is_stale:
            BACKOFF = 3
            ...
            add_log("⚠️ Background candle fetch failed — retaining cached data")

    # ══════════════════════════════════════════════════════════════════════
    # Step 4b — Launch background candle fetch if schedule says it's time
    #            (non-blocking: spawns a daemon thread and returns instantly)
    # ══════════════════════════════════════════════════════════════════════
    ...
    # Honour stale back-off
    if stale_detected and stale_retry_after and now < stale_retry_after:
        secs_left   = int((stale_retry_after - now).total_seconds())
        need_refresh = False
        if need_initial:
            add_log(f"⚠️ Stale back-off: {secs_left}s (no data yet — waiting)")

    if need_refresh and not fetch_running:
        st.session_state['candle_fetch_running']    = True
        st.session_state['last_candle_fetch_time']  = now   # throttle immediately

        # Capture locals for the thread closure
        _ticker, _interval, _period      = ticker, interval, period
        _custom_ticker, _config          = custom_ticker, config
        _last_candle_ts, _need_initial   = last_candle_ts, need_initial

        def _bg_fetch():
            """Runs in daemon thread — only pure Python / HTTP, no st.* calls."""
            result = {'ok': False, 'df': None, 'ts': None, 'stale': False}
            try:
                df_fetched = fetch_data(
                    _ticker, _interval, _period,
                    is_live_trading=True, custom_ticker=_custom_ticker, config=_config
                )
                if df_fetched is not None and not df_fetched.empty:
                    new_ts = df_fetched.iloc[-1]['Datetime']
                    is_stale = False
                    if _last_candle_ts is not None and not _need_initial:
                        try:
                            ts_new = pd.Timestamp(new_ts).value
                            ts_old = pd.Timestamp(_last_candle_ts).value
                            is_stale = (ts_new <= ts_old)
                        except Exception:
                            pass
                    result = {'ok': True, 'df': df_fetched, 'ts': new_ts, 'stale': is_stale}
            except Exception as exc:
                # Store error as log message — never call st.* from a thread
                st.session_state.setdefault('live_logs', []).append(
                    f"⚠️ Bg candle fetch error: {exc}"
                )
            finally:
                st.session_state['_pending_candle_result'] = result
                st.session_state['candle_fetch_running']   = False

        t = _threading.Thread(target=_bg_fetch, daemon=True)
        t.start()
        add_log(f"🔄 Candle fetch started in background (interval={interval})")

    # Ensure we have data before running entry/exit
    df = st.session_state.get('indicator_df')
    if df is None or df.empty:
        add_log("❌ No candle data yet — waiting for initial background fetch")
        return

    # ── Re-sync current_data idx after possible df refresh ─────────────────
    idx           = len(df) - 1
    current_data  = st.session_state.get('current_data', df.iloc[idx].copy())
    current_price = float(current_data['Close'])   # already LTP-updated in Step 2
