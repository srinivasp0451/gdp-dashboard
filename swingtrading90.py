...
def fetch_data_dhan(security_id, exchange_segment, instrument_type, interval, period,
                    client_id, access_token, is_live_trading=False, _silent=False):
    """
    ...
    """
    # Thread-safe UI helpers — when called from bg thread, skip st.* calls
    def _err(msg):  (not _silent) and _err(msg)
    def _inf(msg):  (not _silent) and _inf(msg)
    def _wrn(msg):  (not _silent) and _wrn(msg)

    try:
        from dhanhq import dhanhq
    except ImportError:
        _err("❌ dhanhq not installed. Run:  pip install dhanhq")
        return None
    ...
    try:
        ...
        if isinstance(raw, list):
            ...
            if not raw:
                _err("❌ DhanHQ returned an empty list.")
                return None
            df = pd.DataFrame(raw)
            df.rename(columns={
            ...
            _err(f"❌ DhanHQ: unexpected response type {type(raw)}.")
            return None
        ...
        import traceback
        _err(f"❌ Error fetching DhanHQ data: {e}")
        with st.expander("🔍 Full traceback"):
            st.code(traceback.format_exc())
        return None


def fetch_data(ticker_symbol, interval, period, is_live_trading=False, custom_ticker=None, config=None, _silent=False):
    """
    ...
    if not config.get('use_yfinance', True):
        return fetch_data_dhan(
            security_id      = config.get('dhan_data_security_id', '', _silent=_silent),
...
def _bg_candle_fetch(ticker, interval, period, config, last_candle_ts,
                     need_initial, result_q):
    """
    Background thread worker for candle fetching.
    • Never accesses st.session_state (not thread-safe in Streamlit)
    • Uses fetch_data(..., _silent=True) to suppress st.* calls
    • Communicates result via queue.Queue (thread-safe)
    """
    result = {'ok': False, 'df': None, 'ts': None, 'stale': False, 'msg': ''}
    try:
        df_new = fetch_data(
            ...
            config=config,
            _silent=True,          # ← suppresses st.error/info/warning
        )
        if df_new is not None and not df_new.empty:
            ...
            if last_candle_ts is not None and not need_initial:
                try:
                    is_stale = (
                        pd.Timestamp(new_ts).value
                        <= pd.Timestamp(last_candle_ts).value
                    )
                except Exception:
                    pass
            result = {'ok': True, 'df': df_new, 'ts': new_ts,
                      'stale': is_stale, 'msg': ''}
        else:
            result['msg'] = 'fetch_data returned empty'
    ...
    try:
        ...
        pass   # Previous result still pending — caller will pick it up next tick
...
def live_trading_iteration():
    ...
    ─────────────────────────
    • fetch_ltp()         → runs synchronously on EVERY tick  (~50-150 ms)
    • current_data        → updated from LTP IMMEDIATELY (before anything else)
    • Candle fetch        → runs in a daemon thread via _bg_candle_fetch()
                            so it NEVER blocks the 0.3 s LTP/P&L/SL cycle
    • queue.Queue         → thread-safe channel: thread writes result,
                            main tick reads it (no cross-thread session_state)

    Guarantee: LTP, live P&L, and SL/Target logic execute on every 0.3 s tick
    regardless of how long the Dhan/yfinance API call takes.
    """
    config = st.session_state.get('config', {})
    ...
    # ── Ensure per-session candle queue exists ──────────────────────────────
    if '_candle_q' not in st.session_state:
        st.session_state['_candle_q'] = _queue.Queue(maxsize=2)
    candle_q = st.session_state['_candle_q']

    # ══════════════════════════════════════════════════════════════════════
    # Step 1 — Fast LTP  (synchronous, every tick, always < 200 ms)
    # ══════════════════════════════════════════════════════════════════════
    ltp = fetch_ltp(config)

    # ══════════════════════════════════════════════════════════════════════
    # Step 2 — Update current_data from LTP IMMEDIATELY
    #           Runs BEFORE any slow operation so P&L/SL/Target checks
    #           always use the latest price on every tick.
    # ══════════════════════════════════════════════════════════════════════
    ...
    if df is not None and not df.empty:
        ...
        lc_open = st.session_state.get('live_candle_open', current_price)
        color   = '🟢' if current_price >= (lc_open or current_price) else '🔴'
        add_log(f"💹 LTP ₹{current_price:.2f} {color}")
    else:
        current_price = ltp if (ltp is not None and ltp > 0) else 0.0

    # ══════════════════════════════════════════════════════════════════════
    # Step 3 — Visual forming-candle  (no API, instant)
    # ══════════════════════════════════════════════════════════════════════
    if ltp is not None and ltp > 0:
        update_live_candle(ltp, now, interval_secs)

    # ══════════════════════════════════════════════════════════════════════
    # Step 4a — Apply any completed background candle fetch
    #            (reads from queue — zero blocking)
    # ══════════════════════════════════════════════════════════════════════
    try:
        bg_result = candle_q.get_nowait()
        st.session_state['candle_fetch_running'] = False

        if bg_result['ok'] and not bg_result['stale']:
            df_new = calculate_all_indicators(bg_result['df'], config)
            st.session_state['indicator_df']          = df_new
            st.session_state['last_candle_ts']         = bg_result['ts']
            st.session_state['stale_detected']         = False
            st.session_state['stale_retry_after']      = None
            df = df_new
            add_log(f"✅ Candles updated: {len(df_new)} bars, latest {bg_result['ts']}")
        elif bg_result['stale']:
            BACKOFF = 3
            st.session_state['stale_detected']      = True
            st.session_state['stale_retry_after']   = now + timedelta(seconds=BACKOFF)
            add_log(f"⚠️ Dhan stale data (ts unchanged). Retry in {BACKOFF}s")
        else:
            err_msg = bg_result.get('msg', 'unknown error')
            add_log(f"⚠️ Candle fetch failed: {err_msg}")
    except _queue.Empty:
        pass   # Thread still running or not yet started — use cached df

    # ══════════════════════════════════════════════════════════════════════
    # Step 4b — Launch background candle fetch if schedule says it's time
    #            (spawns daemon thread, returns INSTANTLY)
    # ══════════════════════════════════════════════════════════════════════
    ...
    fetch_running     = st.session_state.get('candle_fetch_running', False)

    df_now       = st.session_state.get('indicator_df')
    need_initial = (df_now is None or (hasattr(df_now, 'empty') and df_now.empty))
    time_elapsed = (now - last_fetch_time).total_seconds() if last_fetch_time else 9999
    need_refresh = need_initial or (time_elapsed >= interval_secs)

    if stale_detected and stale_retry_after and now < stale_retry_after:
        secs_left    = int((stale_retry_after - now).total_seconds())
        need_refresh = False
        if need_initial:
            add_log(f"⚠️ Stale back-off active: {secs_left}s (no initial data yet)")

    if need_refresh and not fetch_running:
        st.session_state['candle_fetch_running']    = True
        st.session_state['last_candle_fetch_time']  = now

        t = _th.Thread(
            target=_bg_candle_fetch,
            args=(ticker, interval, period, config,
                  last_candle_ts, need_initial, candle_q),
            daemon=True,
        )
        t.start()
        add_log(f"🔄 Candle fetch started in background ({interval}/{period})")

    # Ensure we have data before running entry/exit
    df = st.session_state.get('indicator_df')
    if df is None or (hasattr(df, 'empty') and df.empty):
        add_log("⏳ Waiting for initial candle data from background fetch…")
        return

    # Re-sync idx and current_data after possible df refresh
    idx           = len(df) - 1
    ...
    with col1:
        if st.button("▶️ Start Trading", type="primary"):
            st.session_state.update({
                ...
                'indicator_df': None,
            '_candle_q':          _queue.Queue(maxsize=2),
            'candle_fetch_running': False,
