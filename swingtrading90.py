...
def start_ltp_feed(config):
    """Start real-time LTP feed. Call once when trading starts."""
    # Stop any existing feed first
    old_feed = st.session_state.get('_ltp_feed')
    if old_feed:
        try:
    ...
    if not use_yf:
        ...
        try:
            ...
            seg_int     = seg_map.get(exch_raw, marketfeed.IDX)
            instruments = [(seg_int, security_id, marketfeed.Ticker)]
            ctx         = DhanContext(client_id, access_token)

            # Shared mutable container — thread-safe for single value writes
            ltp_box = [None]
            ...
            st.session_state['_ltp_feed'] = feed
            add_log("🔌 Dhan MarketFeed WebSocket started")
        except Exception as e:
            add_log(f"⚠️ MarketFeed failed ({e}) — falling back to REST polling")
            _start_yf_poll_thread(config)
    else:
        # ── yfinance background poll thread ───────────────────────────────
        _start_yf_poll_thread(config)


def _start_yf_poll_thread(config):
    """Poll yfinance every 1s in background. Stores LTP in session_state."""
    import threading as _th
    stop = _th.Event()
    st.session_state['_ltp_stop'] = stop
    ltp_box = [None]
    st.session_state['_ltp_box'] = ltp_box
    cfg = config.copy()
    ...
    def _run():
        while not stop.is_set():
            try:
                ticker_name   = cfg.get('asset', 'NIFTY 50')
                custom_ticker = cfg.get('custom_ticker', None)
    ...
    add_log("📡 yfinance LTP poll thread started (1s interval)")


def stop_ltp_feed():
    """Stop the LTP feed. Call on Stop Trading / Manual Close."""
    feed = st.session_state.get('_ltp_feed')
    if feed:
        try:
            feed.close_connection()
        except Exception:
            pass
        st.session_state['_ltp_feed'] = None

    stop = st.session_state.get('_ltp_stop')
    if stop:
        stop.set()
        st.session_state['_ltp_stop'] = None


def fetch_ltp(config):
    """
    Return latest LTP from the feed cache (0ms — no HTTP call).
    Falls back to a direct REST call only if feed not running.
    """
    # Read from shared ltp_box populated by WebSocket / poll thread
    ltp_box = st.session_state.get('_ltp_box')
    if ltp_box and ltp_box[0]:
        return float(ltp_box[0])

    # Feed not running yet — direct REST call as one-time fallback
    try:
        if not config.get('use_yfinance', True):
            from dhanhq import dhanhq
            client_id    = config.get('dhan_data_client_id') or config.get('dhan_client_id', '')
            ...
            security_id  = str(config.get('dhan_data_security_id', ''))
            exch_raw     = config.get('dhan_data_exchange', 'IDX_I (Index)')
            seg_map      = {'IDX_I (Index)':'IDX_I','NSE_EQ':'NSE_EQ','BSE_EQ':'BSE_EQ',
                            'NSE_FNO':'NSE_FNO','BSE_FNO':'BSE_FNO','MCX_COMM':'MCX_COMM'}
            exch_seg     = seg_map.get(exch_raw, 'NSE_EQ')
            d            = dhanhq(client_id, access_token)
            if hasattr(d, 'get_ltp_data'):
                resp = d.get_ltp_data(securities={exch_seg: [security_id]})
                if resp and resp.get('status') == 'success':
                    for sv in resp.get('data', {}).values():
                        if isinstance(sv, dict):
                            for sv2 in sv.values():
                                if isinstance(sv2, dict):
                                    v = sv2.get('last_price') or sv2.get('ltp') or sv2.get('LTP')
                                    if v:
                                        return float(v)
        else:
            ticker_name = config.get('asset','NIFTY 50')
            symbol = ASSET_MAPPING.get(ticker_name, ticker_name)
