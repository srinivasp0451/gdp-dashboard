...
class DhanBrokerIntegration:
    ...
    def enter_broker_position(self, signal, price, config, log_func):
        ...
        _order_type  = config.get('dhan_order_type', 'Stock (Intraday/Delivery)')
        is_options   = (_order_type == 'Options') or config.get('dhan_is_options', False)
        is_futures   = (_order_type == 'Futures') or config.get('dhan_is_futures', False)
...
def fetch_data(ticker_symbol, interval, period, is_live_trading=False, custom_ticker=None, config=None):
    ...
    try:
        ...
        try:
            ...
            if dt.dt.tz is None:
                df['Datetime'] = dt.dt.tz_localize('UTC').dt.tz_convert(ist)
            else:
...
def update_live_candle(ltp, now_dt, interval_secs, _config_unused=None):
    ...
    if period_start > prev_start:
        ...
        st.session_state.update({
            ...
            'live_candle_low':    ltp,
            'live_candle_close':  ltp,
            # Force immediate API candle fetch so the just-completed candle
            # is replaced with real Dhan data before strategy checks it
            '_force_candle_fetch': True,
        })
        return 'new_period'
...
# ─── Live LTP Feed ──────────────────────────────────────────────────────────
# ltp_box / debug_box are plain Python lists — safe to write from any thread.
# Background thread writes; fragment reads. No st.session_state from threads.
...
def start_ltp_feed(config):
    ...
    if not config.get('use_yfinance', True):
        ...
        def _dhan():
            """
            Run Dhan live feed in a background thread.
            Uses asyncio.run() to create a fresh event loop — avoids the
            Python 3.10+ issue where asyncio.get_event_loop() fails in threads.
            Works with dhanhq v2.0.2 (DhanFeed) and newer versions (MarketFeed).
            """
            import asyncio, time as _t

            # Detect which class is available in the installed dhanhq version
            try:
                ...
                try:
                    ...
                    FeedClass = MarketFeed
                    use_dhan_context = True
                except ImportError:
                    debug_box[0] = "err: cannot import DhanFeed or MarketFeed from dhanhq"
                    return
            ...
            seg = seg_map.get(exch_raw, 0)
            instruments = [(seg, security_id, 15)]  # 15 = Ticker mode

            async def _feed_loop():
                try:
                    if use_dhan_context:
                        ctx  = DhanContext(client_id, access_token)
                        feed = FeedClass(ctx, instruments, version="v2")
                    else:
                        feed = FeedClass(client_id, access_token, instruments, version="v2")
                        # Override loop with the one we're running on (fixes Python 3.10+)
                        feed.loop = asyncio.get_event_loop()

                    await feed.connect()
                    debug_box[0] = "connected — waiting for ticks..."

                    while not stop_ev.is_set():
                        try:
                            data = await feed.get_instrument_data()
                            if data:
                                debug_box[0] = str(data)
                                if isinstance(data, dict):
                                    v = data.get('LTP') or data.get('ltp')
                                    if v:
                                        ltp_box[0] = float(v)
                        except Exception as e:
                            debug_box[0] = f"tick err: {e}"
                            await asyncio.sleep(1)
                            await feed.connect()  # reconnect on error

                except Exception as e:
                    debug_box[0] = f"conn err: {e}"

            asyncio.run(_feed_loop())   # creates fresh event loop — thread safe

        threading.Thread(target=_dhan, daemon=True).start()
    else:
        def _yf():
            import time
            cfg = config.copy()
            while not stop_ev.is_set():
                try:
                    ...
                    sym  = cfg.get('custom_ticker') if name=='Custom Ticker'                            else ASSET_MAPPING.get(name, name)
                    df = yf.download(sym, period='1d', interval='1m',
                                     progress=False, auto_adjust=True)
                    if df is not None and not df.empty:
                        v = df['Close'].iloc[-1]
                        ltp_box[0] = float(v.item() if hasattr(v,'item') else v)
                        debug_box[0] = f"yf close: {ltp_box[0]}"
                except Exception as e:
                    debug_box[0] = f"yf err: {e}"
                stop_ev.wait(1.0)
        threading.Thread(target=_yf, daemon=True).start()
...
def fetch_ltp(config):
    """Read from feed cache — 0ms, no HTTP on main thread."""
    b = st.session_state.get('_ltp_box')
...
def check_ema_crossover_strategy(df, idx, config, current_position):
    ...
    Uses STATE-BASED crossover detection so no crossover is ever missed
    even if a new candle forms between API fetches.

    Logic: track whether EMA_Fast was above/below EMA_Slow on the last
    checked candle. A state CHANGE = crossover. This matches TradingView
    alertcondition(ta.crossover(...)) behaviour exactly.
    """
    if idx < 1:
        return None, None

    # Use the last COMPLETED candle (idx-1) for signal; idx may be forming
    check_idx = idx - 1 if idx >= 1 else idx
    current  = df.iloc[check_idx]
    previous = df.iloc[check_idx - 1] if check_idx >= 1 else df.iloc[check_idx]

    # Check if indicators are ready
    if pd.isna(current['EMA_Fast']) or pd.isna(current['EMA_Slow']):
        return None, None

    # ── State-based crossover detection ───────────────────────────────────────
    # Compare last two COMPLETED candles so a crossover is never skipped
...
def live_trading_iteration():
    ...
    force_fetch     = st.session_state.pop('_force_candle_fetch', False)
    need_refresh = need_initial or force_fetch or (
        last_fetch_time is not None and
        (now - last_fetch_time).total_seconds() >= interval_secs
    )
    if force_fetch:
        add_log('🔄 Candle period ended — fetching real data from Dhan API')
    ...
    if position is None:
        ...
        if _last_exit is not None:
            ...
            _user_cool = config.get('entry_cooldown_seconds',0) if config.get('enable_entry_cooldown',False) else 0
            _min_cool = max(5, _user_cool)
...
def render_config_ui():
    ...
    if config['dhan_enabled']:
        ...
        # ── Order Type: Stock | Options | Futures (mutually exclusive) ─────────
        config['dhan_order_type'] = st.sidebar.radio(
            "Order Type",
            ["Stock (Intraday/Delivery)", "Options", "Futures"],
            index=0,
            help="Select one: Stock for equity, Options for CE/PE, Futures for F&O contracts"
        )
        _order_type = config['dhan_order_type']

        if _order_type == "Stock (Intraday/Delivery)":
            # ── Stock / Equity ────────────────────────────────────────────────
            config['dhan_is_options'] = False
            config['dhan_is_futures'] = False
            ...
            _prod = "CNC" if config['dhan_trading_type'] == 'Delivery (CNC)' else "INTRA"
            st.sidebar.info(f"Order Type: MARKET | Product: {_prod}")

        elif _order_type == "Options":
            # ── Options (CE / PE) ─────────────────────────────────────────────
            config['dhan_is_options'] = True
            config['dhan_is_futures'] = False
            ...
            # ── Futures ───────────────────────────────────────────────────────
            config['dhan_is_options'] = False
            config['dhan_is_futures'] = True
            config['dhan_futures_exchange'] = st.sidebar.selectbox(
                "Futures Exchange", ['NSE_FNO', 'BSE_FNO'],
            ...
            config['dhan_futures_security_id'] = st.sidebar.text_input(
                ...
                help=(
                    "Futures security IDs change every expiry.\n"
                    "Download instrument master:\n"
                    "https://api.dhan.co/data/api-scrip-master\n"
                    "Filter by NSE_FNO / BSE_FNO + instrument type FUT.\n"
                    "Common examples (verify current expiry):\n"
                    "  NIFTY50 FUT  → NSE_FNO\n"
                    "  BANKNIFTY FUT → NSE_FNO\n"
                    "  SENSEX FUT   → BSE_FNO"
                )
            )
            config['dhan_quantity'] = st.sidebar.number_input(
                "Futures Quantity (lots)", min_value=1, value=1
            )
            st.sidebar.info(
                "BUY signal → BUY futures  |  SELL signal → SELL futures\n"
                "Order Type: MARKET | Product: INTRA"
            )
...
def _live_trading_fragment(config):
    ...
    if current_data is not None:
        ...
        with r2c2:
            ...
            st.metric("ADX", f"{float(adx):.1f}" if adx is not None and not pd.isna(adx) else "N/A")
        with r2c3:
            rsi = current_data.get('RSI')
            st.metric("RSI", f"{float(rsi):.1f}" if rsi is not None and not pd.isna(rsi) else "N/A")
        with r2c4:
            atr = current_data.get('ATR')
            st.metric("ATR", f"{float(atr):.2f}" if atr is not None and not pd.isna(atr) else "N/A")
    ...
    if df_plot is not None and not df_plot.empty:
        ...
        try:
            ...
            # Already appended to plot_df above — renders as a normal candle
            # Add a subtle annotation showing forming candle OHLC
            if lc_start is not None and lc_open is not None:
                elapsed   = (datetime.now(pytz.timezone('Asia/Kolkata')) - lc_start).total_seconds()
                pct       = min(100, int(elapsed / interval_secs * 100)) if interval_secs > 0 else 0
                ...
                fig_live.add_annotation(
                    text=f"{color_tag} Forming {pct}%  O:{float(lc_open):.0f} H:{float(lc_high):.0f} L:{float(lc_low):.0f} C:{float(lc_close):.0f} ({chg_p:+.2f}%)",
...
def render_live_trading_ui(config):
    ...
    if st.session_state.get('trading_active', False):
        raw = (st.session_state.get('_debug_box') or ['—'])[0]
        st.info(f"📡 Last Dhan tick: {raw}")
