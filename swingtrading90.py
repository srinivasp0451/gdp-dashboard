    ...
    if not config.get('use_yfinance', True):
        ...
        def _dhan():
            try:
                ...
                feed = MarketFeed(ctx, [(seg, security_id, MarketFeed.Ticker)], version="v2")
                st.session_state['_ltp_feed_obj'] = feed
                feed.run_forever()          # connects WebSocket (returns quickly)
                while not stop_ev.is_set():
                    try:
                        data = feed.get_data()   # blocks until next tick
                        if data:
