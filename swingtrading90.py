...
def update_live_candle(ltp, now_dt, interval_secs, config):
    """
    Called on every LTP tick. Builds the current candle from live LTP data.

    • First call               → initialises a new candle
    • Same candle period       → updates H/L/C in-place
    • Period boundary crossed  → completes & appends the old candle to
                                 indicator_df (drops oldest row), starts new one

    Returns: 'init' | 'updated' | 'completed'
    """
    ...
    if prev_start is None:
        # ── First tick ever ───────────────────────────────────────────────
        st.session_state['live_candle_start']  = period_start
        st.session_state['live_candle_open']   = ltp
        st.session_state['live_candle_high']   = ltp
        st.session_state['live_candle_low']    = ltp
        st.session_state['live_candle_close']  = ltp
        st.session_state['live_candle_volume'] = 0
        return 'init'

    if period_start > prev_start:
        # ── Candle period boundary crossed → complete old candle ──────────
        completed = {
            'Datetime': prev_start,
            'Open':     float(st.session_state['live_candle_open']),
            'High':     float(st.session_state['live_candle_high']),
            'Low':      float(st.session_state['live_candle_low']),
            'Close':    float(st.session_state['live_candle_close']),
            'Volume':   int(st.session_state.get('live_candle_volume', 0)),
        }
        ...
        if df is not None and not df.empty:
            ...
            # Roll: drop oldest row, append completed candle
            ...
            try:
                ...
                add_log(f"⚠️ Indicator recalc error: {e}")
            st.session_state['indicator_df'] = df
            add_log(
                f"🕯️ Candle closed  O={completed['Open']:.2f}  "
                f"H={completed['High']:.2f}  L={completed['Low']:.2f}  "
                f"C={completed['Close']:.2f}  "
                f"({'🟢' if completed['Close'] >= completed['Open'] else '🔴'})"
            )
...
def live_trading_iteration():
    """
    Single iteration of the live trading loop (called every ~0.3 s).

    Data architecture:
    • fetch_ltp()          → fast LTP every tick (yfinance fast_info / Dhan LTP API)
    • indicator_df         → loaded ONCE from Dhan/yfinance on session start,
                             then rolled forward by update_live_candle()
    • update_live_candle() → uses LTP to grow the current candle in real-time;
                             when the period boundary is crossed the candle is
                             completed, appended to indicator_df (oldest dropped),
                             and indicators are recalculated — NO extra API call.
    """
    config = st.session_state.get('config', {})
    ...
    # ── Step 2: One-time initial candle load (skipped on subsequent ticks) ─────
    df = st.session_state.get('indicator_df')
    if df is None or df.empty:
        add_log(f"📥 Loading initial candles ({interval} / {period})…")
        df = fetch_data(
            ticker, interval, period,
            is_live_trading=True, custom_ticker=custom_ticker, config=config
        )
        if df is None or df.empty:
            add_log("❌ Initial candle load failed — will retry next tick")
            return
        df = calculate_all_indicators(df, config)
        st.session_state['indicator_df'] = df
        add_log(f"✅ {len(df)} historical candles loaded")

    # ── Step 3: Update live candle from LTP ────────────────────────────────────
    if ltp is not None and ltp > 0:
        candle_state = update_live_candle(ltp, now, interval_secs, config)
        if candle_state == 'completed':
            df = st.session_state.get('indicator_df', df)   # refreshed by update_live_candle
        # Log LTP + forming candle state
        forming_o = st.session_state.get('live_candle_open',  ltp)
        forming_h = st.session_state.get('live_candle_high',  ltp)
        forming_l = st.session_state.get('live_candle_low',   ltp)
        color     = '🟢' if ltp >= forming_o else '🔴'
        add_log(
            f"💹 LTP: ₹{ltp:.2f}  {color}  "
            f"O={forming_o:.2f} H={forming_h:.2f} L={forming_l:.2f} C={ltp:.2f}"
        )
    else:
        add_log("⚠️ LTP unavailable this tick — using last known price")

    # ── Step 4: Build current_data (last completed candle indicators + live LTP)
    idx           = len(df) - 1
    current_data  = df.iloc[idx].copy()
    current_price = (
        ltp if (ltp is not None and ltp > 0)
        else float(st.session_state.get('live_candle_close', current_data['Close']))
    )
    current_data['Close'] = current_price          # live price for SL/Target checks
