    ...
    if not config.get('use_yfinance', True):
        ...
        def _dhan():
            try:
                ...
                # Callback called as on_message(feed_obj, data) — two args
                def _on_tick(_, data):
                    try:
                ...
                feed = MarketFeed(ctx, [(seg, security_id, MarketFeed.Ticker)],
                                  version="v2", on_ticks=_on_tick)
                st.session_state['_ltp_feed_obj'] = feed
                # feed.start() runs _run_async() which handles ping/pong properly
                # giving per-tick updates instead of 15-second delays
                t = feed.start()
                # Block this thread until stop requested
