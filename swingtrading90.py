...
def live_trading_iteration():
    ...
    _ltp_t1 = _time.time()
    add_log(f"⏱ fetch_ltp took {(_ltp_t1-_ltp_t0)*1000:.0f}ms → LTP={ltp}")
...
def _live_trading_fragment(config):
    ...
    """
    # ══ DIAGNOSTIC TIMING — shows exactly what is slow ══
    ...
    _t2 = _time.time()
    st.caption(f"⏱ Fragment tick: {(_t2-_t0)*1000:.0f}ms total | iteration: {(_t2-_t1)*1000:.0f}ms | overhead: {(_t1-_t0)*1000:.0f}ms | clock: {__import__('datetime').datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
