...
# ─────────────────────────────────────────────────────────────────────────────
# BACKGROUND LTP THREAD
# fetch_ltp() has zero st.* calls — completely safe to run in a daemon thread.
# Communicates via queue.Queue only (never writes to session_state).
# This gives true sub-second LTP without blocking the display fragment.
# ─────────────────────────────────────────────────────────────────────────────

def _ltp_thread_worker(config_snap, ltp_q, stop_ev):
    """
    Fetches LTP every 1 s in background.
    Writes result to ltp_q (Queue). Main thread reads instantly — no HTTP wait.
    stop_ev (threading.Event) signals the thread to exit cleanly.
    """
    while not stop_ev.is_set():
        try:
            ltp = fetch_ltp(config_snap)
            if ltp is not None and ltp > 0:
                # Keep only the latest value — drain old entry first
                try:
                    ltp_q.get_nowait()
                except _queue.Empty:
                    pass
                try:
                    ltp_q.put_nowait(ltp)
                except _queue.Full:
                    pass
        except Exception:
            pass
        stop_ev.wait(timeout=1.0)   # sleep 1 s or wake early on stop


def start_ltp_thread(config):
    """Start background LTP thread. Call once when trading starts."""
    ...
    t = _th.Thread(
        target=_ltp_thread_worker,
        args=(config.copy(), ltp_q, stop_ev),
        daemon=True,
    )
    t.start()
...
def get_ltp_from_thread(config):
    """
    Read latest LTP from background thread queue (instant, no HTTP).
    Falls back to a direct fetch_ltp() call if thread not running.
    """
    ltp_q = st.session_state.get('_ltp_queue')
    if ltp_q is not None:
        try:
            ltp = ltp_q.get_nowait()
            st.session_state['_cached_ltp'] = ltp   # persist for next tick
            return ltp
        except _queue.Empty:
            # Thread hasn't produced a new value yet — use cached
            cached = st.session_state.get('_cached_ltp')
            if cached:
                return cached

    # Thread not running — direct fetch as last resort
