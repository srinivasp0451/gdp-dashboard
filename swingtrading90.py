...
def _live_trading_fragment(config):
    ...
    if logs:
        st.text_area("Trading Logs", label_visibility="collapsed", value="\n".join(reversed(logs[-50:])),
...
def _candle_fragment(config):
    """
    Runs every 1 s, but only hits the API when the selected interval has elapsed.
    Completely independent from _live_trading_fragment — Streamlit runs them on
    separate timers so a 2-second Dhan API call here never freezes the LTP display.
    """
    if not st.session_state.get('trading_active', False):
        return
...
def render_live_trading_ui(config):
    ...
    with col1:
        if st.button("▶️ Start Trading", type="primary"):
            ...
            start_ltp_thread(config)
            add_log("🔄 LTP background thread started")
    ...
    with col2:
        if st.button("⏹️ Stop Trading"):
            ...
            _ev = st.session_state.get("_ltp_stop_event")
            if _ev: _ev.set()
                ...
                if config.get('dhan_enabled', False):
                    ...
                    if bp and dhan_broker:
                        try:
                            ...
                            add_log(f"🏦 ⚠️ {e}")
                st.session_state['position'] = None
                _ev2 = st.session_state.get("_ltp_stop_event")
                if _ev2: _ev2.set()
                st.session_state['broker_position'] = None
                st.session_state.pop('current_data', None)
