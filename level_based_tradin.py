...
limit_price1 = st.number_input("Enter Limit1", min_value=0, step=50,value=80600)
strike_price1 = st.number_input("Select Strike Price for limit1", min_value=0, step=100,value=limit_price1)
limit_price2 = st.number_input("Enter Limit2", min_value=0, step=50,value=80600)
strike_price2 = st.number_input("Select Strike Price for limit2", min_value=0, step=100,value=limit_price2)
...
target_distance = st.number_input("Target Distance", min_value=0.0, step=0.1,format="%.2f",value=10.0)
quantity = st.number_input("Quantity", min_value=1, step=20, value=20)
profit_threshold = st.number_input("Profit Threshold", min_value=1.0, step=100.0,format="%.2f",value=5000.0)
loss_threshold = st.number_input("Loss Threshold", min_value=0.0, step=50.0,format="%.2f",value=350.0)
# timeframe = st.text_input("Time Frame",value=5)

# Dropdown for selecting whether to use trailing stop loss or not
use_trailing_stop_loss = st.selectbox("Use Trailing Stop Loss?", ["No","Yes"],index=0)

# Select backtesting or live trading
trade_mode = st.selectbox("Select Trade Mode", ["Live Trading","Backtesting"],index=1)
...
print('the secrit id is----',expiry_date)
...
def fetch_data():
    ...
    # index_name = tradingsymbol.split()[0]
    index_name = ''
    if selected_index == 'Nifty':
        index_name = 'NIFTY'
    elif selected_index == 'Sensex':
        ...
        index_name = 'BANKEX'
    # symbol = 'CE' if tradingsymbol.split()[-1]=="CALL" else "PE"
    ...
    # )
    while True:
        script_name, ltp = tsl.get_ltp_data(names = [index_name])
        if ltp[index_name]>=limit_price1:
            option_type = 'CE'
            df = load_csv_data()
            filtered_df = filter_data(df, selected_index, expiry_date, strike_price, option_type)
            security_id = filtered_df.iloc[0]['SEM_SMST_SECURITY_ID']
        ...
        if ltp[index_name]<=limit_price2:
            option_type = 'PE'
            df = load_csv_data()
            filtered_df = filter_data(df, selected_index, expiry_date, strike_price, option_type)
            security_id = filtered_df.iloc[0]['SEM_SMST_SECURITY_ID']
...
if st.button("Start"):
    if trade_mode == "Live Trading" and order_client_id and order_access_token:
        ...
        # Initial target based on entry price
        ...
        try:
            ...
            while True:
                ...
                ltp= tsl.get_ltp_data(names=[index_name])
                print(ltp[index_name])
                if ltp[index_name]>=limit_price1:
                    print('inside if')
                    option_type = 'CE'
                    strike_price = strike_price1
                    df = load_csv_data()
                    filtered_df = filter_data(df, selected_index, expiry_date, strike_price1, option_type)
                    security_id = filtered_df.iloc[0]['SEM_SMST_SECURITY_ID']
                    break
                ...
                if ltp[index_name]<=limit_price2:
                    ...
                    df = load_csv_data()
                    filtered_df = filter_data(df, selected_index, expiry_date, strike_price2, option_type)
                    security_id = filtered_df.iloc[0]['SEM_SMST_SECURITY_ID']
            ...
            while True:
                ...
                if 'LTP' in response.keys():
                    ...
                    if order_status == "placed":
                        ...
                        profit_placeholder.markdown(f"Current Profit/Loss: {round(profit_or_loss,2)}")
                        ...
                        if use_trailing_stop_loss=="Yes":
                            ...
                            print(f"Entry price: {entry_price} Trailing Stop Loss at: {trailing_stop_loss_price} highest price: {highest_price} target: {current_target} quantity: {quantity}")
                            trailing_placeholder.markdown(f"Entry price: {entry_price} Trailing Stop Loss at: {trailing_stop_loss_price} highest price: {highest_price} target: {current_target} quantity: {quantity}")
                        else:
                            trailing_stop_loss_price = entry_price-stop_loss_distance
                            current_target = entry_price+target_distance
                            trailing_placeholder.markdown(f"Entry price: {entry_price} Fixed Stop Loss at: {trailing_stop_loss_price} highest price: {highest_price} target: {current_target} quantity: {quantity}")
        ...
        try:
            ...
            while True:
                ...
                if ltp[index_name]<=limit_price2:
                    ...
                    filtered_df = filter_data(df, selected_index, expiry_date, strike_price2, option_type)
                    security_id = filtered_df.iloc[0]['SEM_SMST_SECURITY_ID']
                    print('security id will be',security_id)
                    break
