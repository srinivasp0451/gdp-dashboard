...
# Rates as published by a discount broker. brokerage_flat is a per-order fee that
# does not scale with turnover (options); brokerage_pct with a cap is the
# "0.03% or Rs 20, whichever is lower" structure. dp_charge is the depository fee
# levied once on a delivery SELL, per scrip — flat, so it dominates small trades.
COST_DEFAULTS = {
    "Equity intraday": dict(brokerage_pct=0.03, brokerage_cap=20.0, brokerage_flat=0.0,
                            stt_sell=0.025, stt_buy=0.0, txn=0.00297, stamp_buy=0.003,
                            sebi=0.0001, gst=18.0, dp_charge=0.0),
    "Equity delivery": dict(brokerage_pct=0.0, brokerage_cap=0.0, brokerage_flat=0.0,
                            stt_sell=0.1, stt_buy=0.1, txn=0.00297, stamp_buy=0.015,
                            sebi=0.0001, gst=18.0, dp_charge=13.5),
    "Futures":         dict(brokerage_pct=0.03, brokerage_cap=20.0, brokerage_flat=0.0,
                            stt_sell=0.02, stt_buy=0.0, txn=0.00173, stamp_buy=0.002,
                            sebi=0.0001, gst=18.0, dp_charge=0.0),
    "Options":         dict(brokerage_pct=0.0, brokerage_cap=0.0, brokerage_flat=20.0,
                            stt_sell=0.1, stt_buy=0.0, txn=0.03503, stamp_buy=0.003,
                            sebi=0.0001, gst=18.0, dp_charge=0.0),
}
...
class CostModel:
    ...
    def _brokerage(self, turnover: float) -> float:
        """Flat-fee brokerage does not scale with turnover; the capped
        percentage does until it hits the cap. Treating a flat fee as a capped
        percentage of zero returns zero, which silently made options free."""
        if self.brokerage_flat:
            return self.brokerage_flat
    ...
    def charges(self, entry: float, exit_: float, qty: int, side: int) -> dict:
        ...
        dp = self.dp_charge                      # once per delivery sell, per scrip
        gst = (brokerage + txn + sebi + dp) * self.gst / 100
...
def _cost_panel(s, instrument: str) -> CostModel:
    ...
    with s.container(border=True):
        ...
        with st.expander("Charge rates (%)"):
            ...
            c1, c2 = st.columns(2)
            bf = c1.number_input("Flat brokerage per order (₹)", 0.0, 1e4,
                                 d.get("brokerage_flat", 0.0), 1.0, key="c_bf",
                                 help="Used for options, where brokerage is a flat fee rather "
                                      "than a percentage.")
            dp = c2.number_input("DP charge per delivery sell (₹)", 0.0, 1e3,
                                 d.get("dp_charge", 0.0), 0.5, key="c_dp",
                                 help="Depository fee, charged once per scrip on a delivery sell. "
                                      "Flat, so it dominates small trades.")
    return CostModel(instrument=instrument, slippage_unit=unit, slippage=slip, brokerage_pct=bp,
                     brokerage_cap=bc, brokerage_flat=bf, dp_charge=dp, stt_sell=ss, stt_buy=sb,
                     txn=tx, stamp_buy=sd, sebi=se, gst=gs, enabled=True)
...
def render_backtest(cfg: dict, df: pd.DataFrame):
    ...
    # The verdict belongs at the top. Burying it below the charts meant the one
    # thing worth reading was the one thing nobody found.
    if not trades.empty:
        quick = reality_check(cfg, df, trades, summary, amb,
                              cached.get("best_net", summary["Net P&L"]), None,
                              time_consistency(trades, df), [])
        fails = sum(1 for r in quick if r["Verdict"] == "fail")
    ...
    render_reality_check(cfg, df, trades, summary, amb,
                         cached.get("best_net", summary.get("Net P&L", 0.0)))
...
def random_entry_benchmark(cfg: dict, df: pd.DataFrame, trades: pd.DataFrame,
                           trials: int = 200, seed: int = 0) -> dict | None:
    """Run the same exits from randomly chosen entry bars, many times over.

    This is the test that decides whether a strategy has any signal in it. Stop
    and target rules generate P&L on their own — a wide target in a rising
    market makes money from almost any entry — so a profitable backtest proves
    nothing until you know what random entries would have earned with the same
    exits on the same bars.

    Same number of signals, same long/short mix, same everything else. If the
    real result sits in the middle of the random distribution, the entry logic
    is decoration.
    """
    if trades.empty or len(df) < 50:
        return None
    n_sig = len(trades)
    longs = int(trades["Direction"].str.contains("LONG|CE").sum())
    p_long = longs / max(n_sig, 1)
    rng = np.random.default_rng(seed)
    t_all = pd.Series(True, index=df.index)
    nets, counts = [], []
    for _ in range(trials):
        pos = rng.choice(len(df) - 2, size=min(n_sig, len(df) - 2), replace=False)
        sides = rng.choice([1, -1], size=len(pos), p=[p_long, 1 - p_long])
        s = pd.Series(0, index=df.index, dtype=int)
        s.iloc[pos] = sides
        eng = Engine(df=df, signal=s, allow_long=t_all, allow_short=t_all,
                     sl_cfg=cfg["sl_cfg"], tgt_cfg=cfg["tgt_cfg"], qty=cfg["qty"],
                     side_mode="Both", flip=False, is_options=cfg.get("is_options", False),
                     allow_reverse=False, square_off=cfg.get("square_off"), costs=cfg["costs"])
        t, _ = eng.run()
        nets.append(float(t["Net P&L"].sum()) if len(t) else 0.0)
        counts.append(len(t))
    nets = np.array(nets, dtype=float)
    actual = float(trades["Net P&L"].sum())
    pct = float((nets < actual).mean() * 100)
    return {"actual": actual, "median": float(np.median(nets)), "mean": float(nets.mean()),
            "p95": float(np.percentile(nets, 95)), "p05": float(np.percentile(nets, 5)),
            "percentile": pct, "trials": trials, "avg_trades": float(np.mean(counts)),
            "beat": int((nets >= actual).sum())}


def parameter_stability(cfg: dict, df: pd.DataFrame) -> list[dict]:
    """Nudge the exit numbers and see whether the result survives.

    A setting that only works at exactly 10 points and collapses at 9 or 11 was
    fitted to this particular history. A real edge is a plateau, not a spike.
    """
    out = []
    for label, key, target in (("Stop", "sl_cfg", "sl"), ("Target", "tgt_cfg", "tgt")):
        base = cfg[key]
        field = next((f for f in ("points", "pct", "atr_mult", "rr") if f in base), None)
        if field is None:
            continue
        for mult in (0.8, 0.9, 1.0, 1.1, 1.2):
            probe = dict(cfg)
            newcfg = dict(base)
            newcfg[field] = round(base[field] * mult, 4)
            probe[key] = newcfg
            try:
                _, t, e = run_config(probe, df)
                m = summarise(t, e)
                out.append({"Setting": f"{label} {field}", "Value": newcfg[field],
                            "Change": f"{(mult - 1) * 100:+.0f}%", "Trades": m.get("Trades", 0),
                            "Net P&L": m.get("Net P&L", 0.0),
                            "Expectancy": m.get("Expectancy per trade", 0.0)})
            except Exception:
                continue
    return out


def time_consistency(trades: pd.DataFrame, df: pd.DataFrame, parts: int = 4) -> list[dict]:
    """Was the result earned steadily, or does one stretch carry everything?"""
    if trades.empty:
        return []
    edges = pd.date_range(df.index[0], df.index[-1], periods=parts + 1)
    out = []
    for i in range(parts):
        lo, hi = edges[i], edges[i + 1]
        sub = trades[(trades["Entry time"] >= lo) & (trades["Entry time"] < hi)]
        out.append({"Segment": f"{lo:%d-%b} to {hi:%d-%b}", "Trades": len(sub),
                    "Net P&L": round(float(sub["Net P&L"].sum()), 2) if len(sub) else 0.0})
    return out


def reality_check(cfg: dict, df: pd.DataFrame, trades: pd.DataFrame, summary: dict,
                  ambiguous: int, best_net: float, bench: dict | None,
                  segments: list[dict], stability: list[dict]) -> list[dict]:
    """The checklist a result has to survive before it deserves real money."""
    rows = []

    def add(name, verdict, detail):
        rows.append({"Check": name, "Verdict": verdict, "What it means": detail})

    n = summary.get("Trades", 0)
    if n == 0:
        return [{"Check": "Any trades", "Verdict": "fail",
    ...
    if bench:
        ...
        add("Beats random entries with the same exits",
            "pass" if pctl >= 95 else ("weak" if pctl >= 80 else "fail"),
            f"Better than {pctl:.0f}% of {bench['trials']} random-entry runs "
            f"(random median {bench['median']:,.2f} vs your {bench['actual']:,.2f}). "
            + ("Your entry signal is doing real work." if pctl >= 95 else
               "Random entries with your exit rules did about as well, which means the exits are "
               "producing the result and the signal is adding little."))

    share = 100 * ambiguous / max(n, 1)
    add("Does not rest on intrabar guesswork",
        "pass" if share < 15 else ("weak" if share < 40 else "fail"),
        f"{ambiguous} of {n} exits ({share:.0f}%) hit stop and target inside the same bar, where "
        f"the order is unknowable. Worst case {net:,.2f}, best case {best_net:,.2f}.")

    rr = summary.get("Planned reward:risk")
    if rr == rr:
        add("Sane reward against risk", "pass" if rr >= 1.0 else "fail",
            f"Target sits {rr:.2f}x the stop distance. Below 1.0 buys a high hit rate with a "
            f"payoff that one normal loss erases.")

    dd = abs(summary.get("Max drawdown", 0.0))
    add("Drawdown you could actually sit through",
        "pass" if dd < abs(net) * 0.5 else ("weak" if dd < abs(net) else "fail"),
        f"Worst peak-to-trough was {dd:,.2f} against a net of {net:,.2f}. You have to survive the "
        f"drawdown to collect the return.")

    if segments:
        pos = sum(1 for s_ in segments if s_["Net P&L"] > 0)
        add("Earned steadily, not in one lucky stretch",
            "pass" if pos >= len(segments) - 1 else ("weak" if pos >= len(segments) / 2 else "fail"),
            f"{pos} of {len(segments)} time segments were profitable. One winning quarter carrying "
            f"the whole result is a warning, not a strategy.")

    if stability:
        nets = [r["Net P&L"] for r in stability]
        pos = sum(1 for v in nets if v > 0)
        add("Not knife-edge fitted to one setting",
            "pass" if pos >= len(nets) * 0.8 else ("weak" if pos >= len(nets) * 0.5 else "fail"),
...
def reality_verdict(rows: list[dict]) -> tuple[str, str]:
    fails = sum(1 for r in rows if r["Verdict"] == "fail")
    weak = sum(1 for r in rows if r["Verdict"] == "weak")
    if fails == 0 and weak <= 1:
        return ("Worth paper trading", "This passed the checks that can be run on history. That "
                "is the most a backtest can ever say — it is a reason to forward test, not a "
                "reason to fund it.")
    if fails <= 2:
        return ("Not proven yet", "Some checks failed. Fix those before spending anything; each "
                "one is a specific way this result could be an illusion rather than an edge.")
    return ("Do not trade this", "Several checks failed. On the evidence here this is far more "
            "likely to be a pattern fitted to past noise than a real edge.")


def render_reality_check(cfg: dict, df: pd.DataFrame, trades: pd.DataFrame, summary: dict,
                         ambiguous: int, best_net: float):
    st.markdown("#### Is this actually profitable?")
    st.caption("A backtest showing a profit is the easy part. These are the checks that separate "
               "an edge from a result fitted to past noise. The most important one compares your "
               "signal against random entries using your exact exit rules.")
    c1, c2 = st.columns([1, 2])
    trials = c1.number_input("Random comparison runs", 50, 1000, 200, 50, key="rc_trials",
                             help="More runs give a steadier comparison and take longer.")
    if not c2.button("Run the full reality check", type="primary", width="stretch"):
        return
    with st.spinner("Running random-entry comparison, parameter sweep and time split…"):
        bench = random_entry_benchmark(cfg, df, trades, int(trials))
        segments = time_consistency(trades, df)
        stability = parameter_stability(cfg, df)
        rows = reality_check(cfg, df, trades, summary, ambiguous, best_net, bench,
                             segments, stability)
    verdict, blurb = reality_verdict(rows)
    tone = {"Worth paper trading": st.success, "Not proven yet": st.warning,
            "Do not trade this": st.error}[verdict]
    tone(f"**{verdict}.** {blurb}")

    icons = {"pass": "PASS", "weak": "WEAK", "fail": "FAIL"}
    table = pd.DataFrame([{"": icons[r["Verdict"]], "Check": r["Check"],
                           "What it means": r["What it means"]} for r in rows])
    st.dataframe(table, width="stretch", hide_index=True, height=min(60 + 62 * len(rows), 620))

    if bench:
        st.markdown("##### Your signal against random entries")
        fig = go.Figure(go.Histogram(x=np.round(np.linspace(bench["p05"], bench["p95"], 2), 2),
                                     visible=False))
        fig = go.Figure()
        fig.add_vline(x=bench["median"], line=dict(color="#5b8dee", width=2, dash="dot"),
                      annotation_text="random median")
        fig.add_vline(x=bench["actual"], line=dict(color="#26a172", width=2.5),
                      annotation_text="your strategy")
        fig.add_vrect(x0=bench["p05"], x1=bench["p95"], fillcolor="#5b8dee", opacity=0.12,
                      line_width=0, annotation_text="middle 90% of random results")
        fig.update_layout(template="plotly_dark", paper_bgcolor=BG, plot_bgcolor=BG, height=220,
                          margin=dict(l=8, r=8, t=36, b=8), font=dict(color=INK, size=11),
                          showlegend=False, yaxis=dict(visible=False),
                          xaxis_title="Net P&L", title=dict(text="Where your result falls", x=0,
                                                            font=dict(size=13)))
        fig.update_xaxes(gridcolor=GRID)
        st.plotly_chart(fig, width="stretch", config=PLOT_CFG)
        st.caption(f"{bench['trials']} runs used your exit rules from randomly chosen entry bars, "
                   f"matching your long/short mix. {bench['beat']} of them beat your strategy. "
                   f"If that number is large, your entries are not the reason you made money.")

    if segments:
        st.markdown("##### Was it earned steadily?")
        st.dataframe(pd.DataFrame(segments), width="stretch", hide_index=True)
    if stability:
        st.markdown("##### Does it survive a nudge to the numbers?")
        st.dataframe(pd.DataFrame(stability), width="stretch", hide_index=True, height=260)
        st.caption("Each row re-runs everything with one exit number moved slightly. A real edge "
                   "is a plateau; a spike that only works at your exact setting is curve fitting.")

    st.markdown("##### What this cannot tell you")
    st.markdown(
        "- **No backtest can tell you a strategy is profitable.** It can only tell you it was not "
        "obviously broken on data you already have. Every check above is a way of failing, not a "
        "proof of success.\n"
        "- The honest sequence is: pass these checks, then **paper trade it forward for at least "
        "a month** with no money and compare the live log against what the backtest expected. If "
        "they disagree, believe the live log.\n"
        "- Then trade the smallest size your broker allows for another month. Only scale after "
        "live results match expectations across a losing stretch, not a winning one.\n"
        "- Markets change. An edge that passed every check on two years of history can stop "
        "working the month you fund it, and nothing here can warn you in advance.")
...
def render_search(base_cfg: dict):
    ...
    c1, c2 = st.columns([1.2, 1])
    with c1:
        uni_name = st.selectbox("Universe", list(SCAN_UNIVERSES), index=0, key="sc_uni")
        default_syms = SCAN_UNIVERSES[uni_name]
        sync_universe_box("sc_uni", "sc_syms", uni_name)
        pasted = st.text_area(
            "Symbols — one per line or comma separated", height=110, key="sc_syms",
            help="Bare NSE tickers get .NS added automatically; indices keep their ^ prefix. "
                 "Edit freely — choosing a different universe refills this box.")
        symbols = [t.strip() for t in pasted.replace("\n", ",").split(",") if t.strip()]
        if st.session_state.get("sc_syms__n") != len(symbols):
            st.session_state["sc_lim"] = min(len(symbols), 500) if len(symbols) else 1
            st.session_state["sc_syms__n"] = len(symbols)
        limit = st.number_input(f"Test at most (of {len(symbols)} listed)", 1, 500, key="sc_lim",
                                help="Each symbol needs its own download, spaced by the gap set "
                                     "in the sidebar.")
...
def _render_scan_results(base_cfg):
    ...
    top = res.iloc[0]
    promising = res[(res["Flags"] == "—") & (res["Expectancy"] > 0)]
    st.markdown("#### In plain English")
    if promising.empty:
        st.error(f"**Nothing here looks worth trading.** {scan['tested']:,} combinations were "
                 f"tried and not one produced a positive average result without also tripping a "
                 f"warning flag. That is a real answer: on this data, with this strategy, there "
                 f"was no setting that worked. Try a different strategy, instrument or timeframe.")
    else:
        b = promising.iloc[0]
        st.success(
            f"**{len(promising):,} of {len(res):,} results made money per trade without tripping "
            f"a warning.** The best is **{b['Symbol']}** on the **{b['Timeframe']}** chart using "
            f"**{b['Strategy']}**, with a **{b['Stop rule'].lower()}** stop and a "
            f"**{b['Target rule'].lower()}** target. Over {int(b['Trades'])} trades it won "
            f"{b['Hit rate %']:.0f} times out of 100 and averaged {b['Expectancy']:,.2f} per "
            f"trade, {b['Net points']:,.1f} points in total.")
        st.markdown(f"**What to do with that:** send it to the sidebar at the bottom of this page, "
                    f"open the Backtest tab, press Run backtest, then run *Is this actually "
                    f"profitable?* there. That is where it gets tested properly — this table only "
                    f"shows what made money, not whether it will keep doing so.")
...
# ------------------------------------------------------------ Elliott wave
# Wave counting is subjective and two analysts will label the same chart
# differently. What is not subjective are Elliott's three hard rules, so this
# encodes those and nothing else:
#
...
#
# A 1-2-3-4 structure satisfying all three is located from confirmed pivots, and
# the trade is the break of the wave 3 extreme — the conventional wave 5 entry.
# Waves are counted only on closed, confirmed pivots, so nothing is labelled
# with hindsight. This is one mechanical reading of the theory, not the theory.

def find_impulse(df: pd.DataFrame, piv: list, bullish: bool, strict3: bool = True):
    """Locate the most recent valid 1-2-3-4 impulse and its wave 3 extreme."""
    if len(piv) < 5:
        return None
    lows = "L" if bullish else "H"
    highs = "H" if bullish else "L"
    seq = piv[-9:]
    for a in range(len(seq) - 4):
        p0, p1, p2, p3, p4 = seq[a:a + 5]
        if [p0[2], p1[2], p2[2], p3[2], p4[2]] != [lows, highs, lows, highs, lows]:
            continue
        w1 = (p1[1] - p0[1]) if bullish else (p0[1] - p1[1])
        w3 = (p3[1] - p2[1]) if bullish else (p2[1] - p3[1])
        w5min = abs(p4[1] - p3[1])
        if w1 <= 0 or w3 <= 0:
            continue
        # rule 1: wave 2 holds above the start of wave 1
        if (p2[1] <= p0[1]) if bullish else (p2[1] >= p0[1]):
            continue
        # rule 2: wave 3 is not the shortest
        if strict3 and (w3 < w1 and w3 < max(w5min, 1e-12)):
            continue
        # wave 3 must exceed wave 1's extreme, else it is not an impulse
        if (p3[1] <= p1[1]) if bullish else (p3[1] >= p1[1]):
            continue
        # rule 3: wave 4 stays clear of wave 1's territory
        if (p4[1] <= p1[1]) if bullish else (p4[1] >= p1[1]):
            continue
        return {"w0": p0, "w1": p1, "w2": p2, "w3": p3, "w4": p4,
                "trigger": p3[1], "confirmed_at": max(p4[3], p3[3])}
    return None


def p_elliott(st_, k):
    c1, c2 = st_.columns(2)
    piv = c1.number_input("Pivot sensitivity", 2, 20, 4, key=f"{k}_piv_n",
                          help="Larger values count bigger waves and find fewer of them.")
    life = c2.number_input("Signal valid for (bars)", 1, 200, 30, key=f"{k}_life",
                           help="How long after wave 4 confirms the wave 5 breakout still counts.")
    strict3 = st_.checkbox("Enforce 'wave 3 is never the shortest'", value=True, key=f"{k}_strict3")
    side = st_.selectbox("Count", ["Both directions", "Bullish impulses only",
                                   "Bearish impulses only"], key=f"{k}_bias")
    return {"piv_n": piv, "life": life, "strict3": strict3, "bias": side}


def c_elliott(df, p):
    """Signals the wave 5 breakout: close clears the wave 3 extreme after a
    valid 1-2-3-4 has confirmed."""
    n = len(df)
    longs = pd.Series(False, index=df.index)
    shorts = pd.Series(False, index=df.index)
    piv_all = pivot_series(df, int(p["piv_n"]), int(p["piv_n"]))
    c = df["Close"].to_numpy(float)
    want_long = p["bias"] != "Bearish impulses only"
    want_short = p["bias"] != "Bullish impulses only"
    life = int(p["life"])
    w3_line = np.full(n, np.nan)

    fired_long = fired_short = -1
    for i in range(int(p["piv_n"]) * 6, n):
        visible = [q for q in piv_all if q[3] <= i]      # only confirmed pivots
        if len(visible) < 5:
            continue
        for bullish in ((True,) if want_long and not want_short else
                        (False,) if want_short and not want_long else (True, False)):
            imp = find_impulse(df, visible, bullish, bool(p["strict3"]))
            if not imp or imp["confirmed_at"] > i or i - imp["confirmed_at"] > life:
                continue
            trig = imp["trigger"]
            w3_line[i] = trig
            broke = c[i] > trig if bullish else c[i] < trig
            prev = c[i - 1] > trig if bullish else c[i - 1] < trig
            if broke and not prev:
                if bullish and i != fired_long:
                    longs.iloc[i] = True
                    fired_long = i
                elif not bullish and i != fired_short:
                    shorts.iloc[i] = True
                    fired_short = i
    return Signals(_sig(df.index, longs, shorts),
                   overlays={"Wave 3 trigger": pd.Series(w3_line, index=df.index)},
                   note="Entry is the wave 5 breakout of the wave 3 extreme, after a 1-2-3-4 "
                        "satisfying all three of Elliott's hard rules.")


STRATEGIES["Elliott wave impulse"] = {"params": p_elliott, "compute": c_elliott}
STRATEGY_DEFAULTS["Elliott wave impulse"] = {"piv_n": 4, "life": 30, "strict3": True,
...
# Recomputed here, after every strategy has registered itself. Building this
# list earlier silently dropped whichever strategies were defined below it.
SWEEPABLE = [k for k in STRATEGY_DEFAULTS if not k.startswith("Simple")]

# VWAP resets at each session open and an opening range needs bars inside a
# session, so both are meaningless on daily or weekly candles — every bar is its
# own session. Sweeping them there burns tests and returns nothing.
INTRADAY_ONLY = {"VWAP reclaim", "Opening range breakout"}


def strategy_applies(name: str, interval: str) -> bool:
    return not (name in INTRADAY_ONLY and interval in ("1d", "1wk"))
...
def render_hit_rows(rows: pd.DataFrame, piv_n: int, horizon: int, key_prefix: str,
                    header: bool = False) -> None:
    """One hit per row with its levels and its three buttons.

    Used by both the results table and the paginated list, so the two can never
    drift apart.
    """
    ss = st.session_state
    widths = [2.6, 1.5, 1.0, 1.0, 1.0]
    if header:
        h = st.columns(widths)
        for col, label in zip(h, ["Hit", "Levels", "", "", ""]):
            col.markdown(f"<span style='font-size:.72rem;letter-spacing:.06em;"
                         f"text-transform:uppercase;opacity:.55'>{label}</span>",
                         unsafe_allow_html=True)
        st.markdown("<div style='height:1px;background:#2c3242;margin:.1rem 0 .5rem'></div>",
                    unsafe_allow_html=True)
    for pos, (_, r) in enumerate(rows.iterrows()):
        ...
        if entry == entry and stop == stop:
            cols[1].markdown(
                f"<span style='font-size:.82rem'>Entry <b>{entry:,.2f}</b><br>"
                f"Stop <b>{stop:,.2f}</b><br>Target <b>{tgt:,.2f}</b></span>",
                unsafe_allow_html=True)
        else:
            cols[1].caption("levels unavailable")
        uid = f"{key_prefix}_{row['Symbol']}_{row['Timeframe']}_{row['Pattern']}_{pos}"
        if cols[2].button("Chart", key=f"c_{uid}", width="stretch"):
            show_pattern_dialog(row, piv_n, horizon)
        if cols[3].button("Levels", key=f"l_{uid}", width="stretch"):
            show_plan_dialog(row, piv_n)
        if cols[4].button("Sidebar", key=f"s_{uid}", width="stretch"):
...
def render_patterns(base_cfg: dict):
    ...
    with c1:
        ...
        sync_universe_box("pt_uni", "pt_syms", uni)
        pasted = st.text_area("Symbols", height=110, key="pt_syms",
                              help="Edit freely. Choosing a different universe refills this box.")
        symbols = [t.strip() for t in pasted.replace("\n", ",").split(",") if t.strip()]
        if st.session_state.get("pt_syms__n") != len(symbols):
            st.session_state["pt_lim"] = min(len(symbols), 500) if len(symbols) else 1
            st.session_state["pt_syms__n"] = len(symbols)
        limit = st.number_input(f"Test at most (of {len(symbols)} listed)", 1, 500, key="pt_lim")
        symbols = symbols[:int(limit)]
    with c2:
    ...
    st.caption("These settings decide the levels quoted against every hit below, so you can copy "
               "them straight onto your order ticket. They are also what gets loaded when you "
               "press Sidebar on a row.")
    r1, r2, r3 = st.columns(3)
    rr_default = r1.slider("Reward : risk", 0.5, 6.0, 2.0, 0.5, key="pt_rr",
                           help="The target is set this many times the stop distance away.")
    atr_default = r2.slider("Fallback stop, in ATR", 0.5, 5.0, 1.5, 0.25, key="pt_atr",
                            help="Used when the pattern has no nearby structure to lean on, and "
                                 "as a floor so the stop never sits inside the noise.")
    r3.markdown("**Stop placement**  \nStructure first: the low a bottom is built on, the swing a "
                "breakout came from, or the extreme of the signal candle. ATR is the fallback and "
                "the floor.")
        ...
        for tf in tfs:
            ...
            for name in chosen:
                ...
                for i in fired:
                    ts = df.index[int(i)]
                    try:
                        plan = pattern_trade_plan(df, name, int(i), piv_n, rr_default,
                                                  atr_default)
                    except Exception:
                        plan = None
                    hits_rows.append({
                        ...
                        "Entry": plan["entry"] if plan else np.nan,
                        "Stop": plan["stop"] if plan else np.nan,
                        "Target": plan["target_rr"] if plan else np.nan,
                        "Risk": plan["risk"] if plan else np.nan,
                        "Reward:risk": rr_default if plan else np.nan,
                        "Stop is": plan["stop_reason"] if plan else "",
                    })
            done += 1
...
def _render_pattern_results():
    ...
    st.markdown("#### Live hits and their levels")
    if hits.empty:
        ...
        piv_n = int(res.get("piv_n", 3))
        horizon = int(res["horizon"])

        st.markdown("##### Recommended stop and target")
        rr = res.get("rr", 2.0)
        atr_m = res.get("atr", 1.5)
        with_levels = view[view["Stop"].notna()] if "Stop" in view.columns else view.iloc[0:0]
        if with_levels.empty:
            st.caption("No levels could be derived for these hits.")
        else:
            med_rr = rr
            be = 100 / (1 + med_rr)
            st.info(
                f"Every row below carries its own entry, stop and target, worked out from the "
                f"pattern itself at a **{med_rr:g}:1 reward-to-risk**. The stop goes on the "
                f"structure that would prove the pattern wrong — the low a bottom is built on, "
                f"the swing a breakout came from, or the extreme of the signal candle — with a "
                f"floor of {atr_m:g} x ATR so it never sits inside the noise. At {med_rr:g}:1 you "
                f"need to win **{be:.0f} out of every 100** just to break even, so check the "
                f"pattern's past win rate against that number before taking it.")
            st.caption("Change the reward-to-risk or ATR floor in the settings above and re-run "
        ...
        if not view.empty:
            ...
            st.caption("The same hits, ten at a time, for working through a long list.")
            per_page = 10
            total_hits = len(view)
            pages = max(1, int(np.ceil(total_hits / per_page)))
            st.session_state.setdefault("pt_page", 1)
            st.session_state["pt_page"] = min(max(1, st.session_state["pt_page"]), pages)
            page = st.session_state["pt_page"]
            if pages > 1:
                nav = st.columns([1, 1, 3])
                if nav[0].button("← Previous", width="stretch", disabled=page <= 1):
                    st.session_state["pt_page"] = page - 1
                    st.rerun()
                if nav[1].button("Next →", width="stretch", disabled=page >= pages):
                    st.session_state["pt_page"] = page + 1
                    st.rerun()
                first = (page - 1) * per_page + 1
                last = min(page * per_page, total_hits)
                nav[2].markdown(f"Showing hits **{first}–{last}** of **{total_hits}**, "
                                f"best first (page {page} of {pages})")
            chunk = view.iloc[(page - 1) * per_page: page * per_page]
            render_hit_rows(chunk, piv_n, horizon, f"act{page}")
    ...
    if best["Mean edge %"] > 0:
        st.success(
            f"**Best pattern here: {best['Pattern']}.** Across {int(best['Symbols'])} instruments "
            f"and {int(best['Total occurrences'])} occurrences it went the right way "
            f"{best['Mean win %']:.0f} times out of 100, against {best['Mean baseline %']:.0f} "
            f"times for any random bar on the same charts. So it is worth about "
            f"{best['Mean edge %']:.2f}% extra move per trade. Anything under about 0.2% will be "
            f"eaten by brokerage and slippage.")
    else:
        st.error("**No pattern beat what these instruments do anyway.** Every one of them scored "
                 "at or below the baseline, meaning you would have done as well entering on a "
                 "random bar. On this data these patterns are decoration.")
...
# ---------------------------------------------------------- trade ideas tab
# Answers "what should I trade right now" instead of "what worked in the past".
#
# An idea is only listed when two separate things are true at once: the setup
# has fired within the last few bars and has not already run to its stop or
# target, and that same setup has a track record on that instrument worth
# taking. Either half alone is worthless — a fresh signal from a rule that has
# never worked is noise, and a rule with a fine history is not a trade until it
# actually fires.

def _sl(mult, trail=False):
    return {"mode": "ATR multiple", "atr_len": 14, "atr_mult": mult, "atr_trail": trail,
            "be": {"on": False}}


def _tgt(rr):
    return {"mode": "Risk:reward multiple", "rr": rr}


INTRADAY_SETUPS = [
    {"name": "EMA 9/21 crossover", "tf": "5m", "strategy": "EMA crossover",
     "params": {"fast": 9, "slow": 21, "trend": 50}, "sl": _sl(2.0), "tgt": _tgt(2.0)},
    {"name": "EMA 9/21 on 15m", "tf": "15m", "strategy": "EMA crossover",
     "params": {"fast": 9, "slow": 21, "trend": 50}, "sl": _sl(2.0), "tgt": _tgt(2.0)},
    {"name": "Supertrend flip", "tf": "15m", "strategy": "Supertrend flip",
     "params": {"n": 10, "m": 3.0}, "sl": _sl(2.0, True), "tgt": _tgt(2.5)},
    {"name": "Opening range breakout", "tf": "5m", "strategy": "Opening range breakout",
     "params": {"mins": 15, "once": True}, "sl": _sl(1.5), "tgt": _tgt(2.0)},
    {"name": "VWAP reclaim", "tf": "5m", "strategy": "VWAP reclaim",
     "params": {"buf": 0.05}, "sl": _sl(1.5), "tgt": _tgt(2.0)},
    {"name": "Donchian 20 breakout", "tf": "15m", "strategy": "Donchian breakout",
     "params": {"n": 20}, "sl": _sl(2.0), "tgt": _tgt(2.0)},
]

SWING_SETUPS = [
    {"name": "EMA 20/50 crossover", "tf": "1d", "strategy": "EMA crossover",
     "params": {"fast": 20, "slow": 50, "trend": 200}, "sl": _sl(2.5), "tgt": _tgt(2.0)},
    {"name": "Supertrend flip", "tf": "1d", "strategy": "Supertrend flip",
     "params": {"n": 10, "m": 3.0}, "sl": _sl(3.0, True), "tgt": _tgt(2.5)},
    {"name": "Donchian 20 breakout", "tf": "1d", "strategy": "Donchian breakout",
     "params": {"n": 20}, "sl": _sl(2.5), "tgt": _tgt(2.5)},
    {"name": "MACD crossover", "tf": "1d", "strategy": "MACD crossover",
     "params": {"f": 12, "s": 26, "g": 9, "zero": True}, "sl": _sl(2.5), "tgt": _tgt(2.0)},
    {"name": "Bollinger breakout", "tf": "1d", "strategy": "Bollinger band",
     "params": {"n": 20, "k": 2.0, "mode": "Breakout"}, "sl": _sl(2.0), "tgt": _tgt(2.0)},
    {"name": "Trendline breakout + retest", "tf": "1d", "strategy": "Chart pattern",
     "params": {"family": "Trendline", "pattern": "Trendline breakout + retest", "piv_n": 3,
                "bias": "Pattern's own bias"}, "sl": _sl(2.0), "tgt": _tgt(2.5)},
]

HORIZON_PERIOD = {"5m": "1mo", "15m": "1mo", "1h": "6mo", "1d": "2y"}


def idea_levels(cfg: dict, df: pd.DataFrame, bar: int, side: int) -> dict | None:
    """Entry, stop and target for a signal that fired on `bar`, plus whether it
    is still live. Uses the same risk engine the backtest uses, so the numbers
    quoted here are the numbers that would have been traded."""
    ctx = build_context(df, cfg["sl_cfg"], cfg["tgt_cfg"])
    entry_bar = bar + 1
    if entry_bar >= len(df):
        entry_bar = len(df) - 1
    entry = float(df["Open"].iloc[entry_bar])
    pos = Position(side=side, qty=1, entry_price=entry, entry_time=df.index[entry_bar],
                   entry_reason="", entry_bar=entry_bar, peak=entry, trough=entry)
    init_stop(pos, ctx.iloc[bar], cfg["sl_cfg"])
    init_target(pos, ctx.iloc[bar], cfg["tgt_cfg"])
    if np.isnan(pos.sl):
        return None
    # Has it already run to stop or target since firing? Then the chance is gone.
    seg = df.iloc[entry_bar:]
    if side > 0:
        hit_sl = bool((seg["Low"] <= pos.sl).any())
        hit_tg = (not np.isnan(pos.target)) and bool((seg["High"] >= pos.target).any())
    else:
        hit_sl = bool((seg["High"] >= pos.sl).any())
        hit_tg = (not np.isnan(pos.target)) and bool((seg["Low"] <= pos.target).any())
...
def scan_trade_ideas(symbols: list[str], horizon: str, instrument: str, recent: int,
                     min_trades: int, min_hit: float, costs: CostModel, strict: bool = False,
                     progress=None) -> tuple[list[dict], list[tuple], dict]:
    """Find setups that are both live and proven, and record why the rest failed.

    The win-rate floor is applied to the observed rate. Applying it to the
    pessimistic end of the confidence range sounds more rigorous and is close to
    unusable on small samples — with 10 past trades, a 40% floor on the lower
    bound demands 8 wins out of 10. Strict mode restores that behaviour for
    anyone who wants it, but it is not the default because it silently returns
    nothing rather than explaining itself.

    The tally matters as much as the results. "No trades" is useless on its own;
    knowing that 95 candidates had negative expectancy and 30 never fired tells
    you which dial to turn.
    """
    setups = INTRADAY_SETUPS if horizon.startswith("Intraday") else SWING_SETUPS
    ideas, skipped = [], []
    tally = {"tested": 0, "no data": 0, "too few trades": 0, "lost money on average": 0,
             "win rate below floor": 0, "no recent signal": 0,
             "signal already hit stop or target": 0, "no valid stop": 0, "listed": 0}
    total = max(len(symbols), 1)
    for n_done, sym in enumerate(symbols, start=1):
        ysym = to_yf_symbol(sym)
        if progress:
            progress(n_done / total, f"{sym} ({n_done}/{total})")
        for setup in setups:
            tally["tested"] += 1
            tf = setup["tf"]
            period = HORIZON_PERIOD.get(tf, "1y")
            try:
                df = load_candles(ysym, tf, period)
            except RateLimitError:
                skipped.append((sym, "rate limited — scan stopped"))
                return ideas, skipped, tally
            except Exception as e:
                skipped.append((sym, f"{tf}: {str(e)[:80]}"))
                tally["no data"] += 1
                continue
            if len(df) < 120:
                tally["no data"] += 1
                continue
            cfg = dict(base_config_stub(), symbol=ysym, name=sym, interval=tf, period=period,
                       strategy=setup["strategy"], strat_params=setup["params"],
                       sl_cfg=setup["sl"], tgt_cfg=setup["tgt"], filters={},
                       sl_mode=setup["sl"]["mode"], tgt_mode=setup["tgt"]["mode"],
                       instrument=instrument,
                       is_options=instrument.startswith("Options"), costs=costs)
            try:
                sig, al, as_ = prepare(cfg, df)
            ...
            if m["Expectancy per trade"] <= 0:
                tally["lost money on average"] += 1
                continue
            lo, hi = wilson_interval(m["Wins"], m["Trades"])
            gate = lo if strict else m["Hit rate %"]
            if gate < min_hit:
                tally["win rate below floor"] += 1
                continue
            dsig = eng.directional_signal()
            fresh = np.flatnonzero(dsig.to_numpy()[-(recent + 1):-1] != 0)
            if len(fresh) == 0:
                tally["no recent signal"] += 1
                continue
            bar = len(df) - (recent + 1) + int(fresh[-1])
            side = int(dsig.iloc[bar])
            lv = idea_levels(cfg, df, bar, side)
            if lv is None:
                tally["no valid stop"] += 1
                continue
            if lv["resolved"]:
                tally["signal already hit stop or target"] += 1
                continue
            direction = ("Buy CE" if side > 0 else "Buy PE") if cfg["is_options"] else \
            ...
            ideas.append({
                ...
                "Risk": round(lv["risk"], 2), "Reward": round(lv["reward"], 2),
                "Reward:risk": round(lv["rr"], 2) if lv["rr"] == lv["rr"] else np.nan,
                "Already moved %": round(lv["moved_pct"], 2),
                "Past trades": m["Trades"], "Win rate %": m["Hit rate %"],
                "Win rate low %": round(lo, 1), "Win rate high %": round(hi, 1),
                "Avg per trade": m["Expectancy per trade"],
                "Profit factor": m["Profit factor"],
                "Sharpe": m.get("Sharpe (annualised)"),
                "Max drawdown": m["Max drawdown"],
                "Flags": degeneracy_flags(m),
                "_cfg": cfg,
            })
    return ideas, skipped, tally


def idea_reason(row: dict, horizon: str) -> str:
    """Why this is on the list, in the terms the trade will actually be taken."""
    d = row["Direction"].lower()
    ev = row["Avg per trade"]
    hold = "close it the same day" if horizon.startswith("Intraday") else \
           "hold it over days until the stop or target is reached"
    return (f"**{row['Symbol']} — {row['Direction']} · {row['Setup']} on the {row['Timeframe']} "
            f"chart.** The setup triggered {row['Bars ago']} bar(s) ago and price has not yet "
            f"reached either the stop or the target, so the trade is still available. "
            f"Enter near {row['Entry']:,.2f} (last traded {row['Last price']:,.2f}), stop at "
            f"{row['Stop']:,.2f}, target {row['Target']:,.2f}. You risk {row['Risk']:,.2f} to make "
            f"{row['Reward']:,.2f}, a reward-to-risk of {row['Reward:risk']:.2f} to 1. "
            f"On this instrument this exact setup has traded {row['Past trades']} times and won "
            f"{row['Win rate %']:.0f} out of every 100 — though with that many samples the honest "
            f"range is {row['Win rate low %']:.0f} to {row['Win rate high %']:.0f} out of 100. "
            f"Averaged over every past trade including the losers it made {ev:,.2f} per trade. "
            f"Plan to {hold}.")


def sync_universe_box(uni_key: str, syms_key: str, uni: str, limit: int = 200) -> None:
    """Refill the symbols box when the universe dropdown changes.

    A Streamlit widget that has a key takes its value from session state and
    ignores the `value=` argument from then on. Recomputing the default list
    therefore has no visible effect — the box silently keeps the first universe
    forever. Writing the key directly, and only when the selection actually
    changes, updates the box while still letting you edit the list by hand
    afterwards.
    """
    last = f"{uni_key}__last"
    if st.session_state.get(last) != uni:
        st.session_state[syms_key] = ", ".join(SCAN_UNIVERSES.get(uni, [])[:limit])
        st.session_state[last] = uni


def _render_idea_tally(tally: dict, empty: bool) -> None:
    """Show which gate stopped each candidate, and what to do about it."""
    if not tally:
        return
    order = ["too few trades", "lost money on average", "win rate below floor",
             "no recent signal", "signal already hit stop or target", "no valid stop", "no data"]
    advice = {
        "too few trades": "Lower *Minimum past trades*, or use a longer history.",
        "lost money on average": "Nothing to change — these setups lost money on this instrument "
                                 "after costs. This is usually the biggest bucket and it is the "
                                 "screener doing its job.",
        "win rate below floor": "Lower *Minimum win rate*. A 2:1 setup winning 40 out of 100 is "
                                "profitable, so a high floor rejects good setups.",
        "no recent signal": "Widen *Fired within last N bars*, or scan more symbols.",
        "signal already hit stop or target": "The setup fired but price already reached the stop "
                                             "or target, so the trade is gone. A wider bar window "
                                             "makes this MORE likely, not less — narrow it "
                                             "instead and scan more symbols.",
        "no valid stop": "Rare. The stop rule could not produce a level on that bar.",
        "no data": "Download failed or the history was too short.",
    }
    rows = [{"Rejected because": k, "Count": tally.get(k, 0), "What helps": advice[k]}
            for k in order if tally.get(k, 0) > 0]
    total = tally.get("tested", 0)
    st.caption(f"{total} setup-and-symbol combinations were tested; {tally.get('listed', 0)} "
               f"passed every check.")
    if rows:
        st.dataframe(pd.DataFrame(rows), width="stretch", hide_index=True)
    if empty:
        biggest = max((tally.get(k, 0), k) for k in order)[1] if total else None
        if biggest:
            st.info(f"The largest single reason was **{biggest}** "
                    f"({tally.get(biggest, 0)} of {total}). {advice[biggest]}")


def render_ideas(base_cfg: dict):
    ss = st.session_state
    st.markdown("#### What to trade")
    st.caption("Setups that have fired in the last few bars, have not yet run to their stop or "
               "target, and have a record on that instrument worth taking. Both halves have to be "
               "true — a fresh signal from a rule that never worked is noise.")

    c1, c2, c3 = st.columns([1.1, 1, 1])
    horizon = c1.selectbox("I want to trade", ["Intraday (close the same day)",
                                               "Swing / delivery (hold for days)"], key="id_hz")
    instrument = c2.selectbox("As", ["Equity intraday", "Equity delivery", "Futures",
                                     "Options (stock or index)"], key="id_instr")
    uni_names = list(SCAN_UNIVERSES)
    # Index options are written on indices, so default there rather than making
    # you notice the stock list is wrong for what you picked.
    if instrument.startswith("Options") and st.session_state.get("id_instr__last") != instrument:
        st.session_state["id_uni"] = "Indices and benchmarks"
        st.session_state.pop("id_uni__last", None)
    st.session_state["id_instr__last"] = instrument
    uni = c3.selectbox("Universe", uni_names, index=0, key="id_uni")
    sync_universe_box("id_uni", "id_syms", uni)
    pasted = st.text_area("Symbols", height=90, key="id_syms",
                          help="Edit freely. Choosing a different universe refills this box.")
    symbols = [t.strip() for t in pasted.replace("\n", ",").split(",") if t.strip()]
    m1, m2, m3, m4 = st.columns(4)
    if st.session_state.get("id_syms__n") != len(symbols):
        st.session_state["id_lim"] = min(len(symbols), 500) if len(symbols) else 1
        st.session_state["id_syms__n"] = len(symbols)
    limit = m1.number_input(f"Scan at most (of {len(symbols)} listed)", 1, 500, key="id_lim")
    recent = m2.number_input("Fired within last N bars", 1, 30, 3, key="id_recent",
                             help="1 means only signals from the most recently closed bar.")
    min_trades = m3.number_input("Minimum past trades", 5, 1000, 30, key="id_mint",
                                 help="How much history the setup needs before it is trusted.")
    min_hit = m4.number_input("Minimum win rate %", 0, 90, 30, key="id_minhit",
                              help="Applied to the observed win rate. A trend setup winning 40 "
                                   "out of 100 with a 2:1 reward is profitable, so do not set "
                                   "this high — expectancy is already checked separately.")
    symbols = symbols[:int(limit)]

    cc1, cc2 = st.columns(2)
    use_costs = cc1.checkbox("Include brokerage, taxes and slippage in the record", value=True,
                             key="id_costs")
    strict = cc2.checkbox("Strict: apply the win-rate floor to the pessimistic bound", value=False,
                          key="id_strict",
                          help="Far harsher than it sounds. With 10 past trades a 40% floor on "
                               "the pessimistic bound needs 8 wins out of 10.")
    inst_key = "Options" if instrument.startswith("Options") else instrument
    costs = CostModel(instrument=inst_key, enabled=use_costs, slippage_unit="Percent",
                      slippage=0.03, **{k: v for k, v in COST_DEFAULTS.get(
                          inst_key, COST_DEFAULTS["Equity intraday"]).items()})

    if instrument.startswith("Options"):
        st.warning("Everything below is measured on the **underlying's** price, not on option "
                   "premium. There are no greeks, no implied volatility and no time decay here. A "
                   "correct call on direction can still lose money in an option if volatility "
                   "falls or it takes too long. Treat the direction as the signal and price the "
                   "option yourself.", icon="⚠️")
    if instrument == "Futures":
        st.info("Levels are the underlying's. Futures trade at a premium or discount and move in "
                "fixed lot sizes, so convert before sizing.", icon="ℹ️")

    setups = INTRADAY_SETUPS if horizon.startswith("Intraday") else SWING_SETUPS
    e1, e2 = st.columns(2)
    e1.metric("Setups tested per symbol", len(setups))
    e2.metric("Downloads", f"{len(symbols) * len({s['tf'] for s in setups}):,}")

    if not symbols:
        st.info("Add at least one symbol.")
        return
    if st.button("Find trades", type="primary", width="stretch"):
        bar = st.progress(0.0)
        status = st.empty()
        with st.spinner("Testing every setup on every symbol…"):
            ideas, skipped, tally = scan_trade_ideas(
                symbols, horizon, inst_key, int(recent), int(min_trades), float(min_hit),
                costs, strict=strict,
                progress=lambda f, t: (bar.progress(min(f, 1.0)), status.write(t)))
        bar.progress(1.0)
        status.write(f"Finished. {len(ideas)} live setup(s) found.")
        ss["ideas"] = {"rows": ideas, "skipped": skipped, "horizon": horizon,
                       "instrument": instrument, "tally": tally}
    ...
    tally = res.get("tally", {})
    if not rows:
        st.error("**No trades right now.** Below is exactly where every candidate was rejected, "
                 "so you can see which setting is doing it rather than guessing.")
        _render_idea_tally(tally, empty=True)
        if res["skipped"]:
    ...
    df_rows = pd.DataFrame([{k: v for k, v in r.items() if k != "_cfg"} for r in rows])
    df_rows = df_rows.sort_values("Avg per trade", ascending=False).reset_index(drop=True)
    clean = int((df_rows["Flags"] == "—").sum())
    st.success(f"**{len(df_rows)} live setup(s) found**, {clean} of them without a warning flag. "
               f"Best first, ranked by average result per past trade — not by win rate, which can "
               f"be inflated by a tiny target.")

    with st.expander(f"Where the other {tally.get('tested', 0) - len(df_rows)} candidates were "
                     f"rejected"):
        _render_idea_tally(tally, empty=False)

    st.markdown("#### The trades")
    for i, r in df_rows.head(15).iterrows():
        cfg = next(x["_cfg"] for x in rows
                   if x["Symbol"] == r["Symbol"] and x["Setup"] == r["Setup"]
                   and x["Timeframe"] == r["Timeframe"])
        with st.container(border=True):
            st.markdown(idea_reason(r.to_dict(), res["horizon"]))
            k = st.columns(6)
            k[0].metric("Entry", f"{r['Entry']:,.2f}")
            k[1].metric("Stop", f"{r['Stop']:,.2f}", f"-{r['Risk']:,.2f}")
            k[2].metric("Target", f"{r['Target']:,.2f}", f"+{r['Reward']:,.2f}")
            k[3].metric("Reward : risk", f"{r['Reward:risk']:.2f}")
            k[4].metric("Won before", f"{r['Win rate %']:.0f}%",
                        f"{r['Win rate low %']:.0f}–{r['Win rate high %']:.0f}% honest range")
            k[5].metric("Avg per trade", f"{r['Avg per trade']:,.2f}")
            if r["Flags"] != "—":
                st.warning(f"Warning: {r['Flags']}")
            b1, b2 = st.columns([1, 3])
            if b1.button("Apply to sidebar", key=f"idea_{i}", width="stretch"):
                out = dict(cfg)
                out["_label"] = f"{r['Symbol']} · {r['Setup']} · {r['Timeframe']}"
                ss["pending_sidebar"] = out
                ss.pop("replay_cfg", None)
                ss.pop("bt_result", None)
                st.rerun()
            b2.caption("Loads this exact setup into the sidebar. Run it in the Backtest tab and "
                       "put it through *Is this actually profitable?* before risking anything.")

    st.markdown("#### All of them as a table")
    st.dataframe(df_rows, width="stretch", height=360, hide_index=True)
    st.download_button("Download as CSV", df_rows.to_csv(index=False).encode(),
                       "trade_ideas.csv", "text/csv")

    st.markdown("#### How to read the win rate")
    st.markdown(
        "- **The win rate is history, not a probability of this trade working.** It is how often "
        "this setup worked on this instrument before, and the honest range beside it shows how "
        "much that figure could move with more data.\n"
        "- **Average per trade matters more than win rate.** A setup winning 40 times out of 100 "
        "with a 2:1 reward beats one winning 70 times with a 0.3:1 reward, and the second will "
        "feel better right up until it takes everything back.\n"
        "- These setups use fixed, sensible exits. They are a starting point, not tuned to the "
        "instrument. Send one to the sidebar and test variations there.\n"
        "- The list being empty is a real answer. Most days there is no trade worth taking.")
    if res["skipped"]:
        with st.expander(f"{len(res['skipped'])} downloads skipped"):
            st.dataframe(pd.DataFrame(res["skipped"], columns=["Symbol", "Reason"]),
                         width="stretch", hide_index=True)
...
def scan_verified_live(symbols, strategies, tf_pairs, sl_grid, tg_grid, min_trades, recent,
                       costs, oos_frac, run_random, random_trials, cap, qty=1, progress=None):
    rows, configs, skipped = [], [], []
    tally = {"tested": 0, "too few trades": 0, "not profitable after costs": 0,
             ...
             "no better than random entries": 0, "needs intraday bars": 0, "listed": 0}
    ...
    for sym in symbols:
        ...
        for tf, period in tf_pairs:
            ...
            if len(df) < 200:
                ...
                continue
            for sname in strategies:
                if not strategy_applies(sname, tf):
                    tally["needs intraday bars"] += len(sl_grid) * len(tg_grid)
                    continue
                params = STRATEGY_DEFAULTS.get(sname, {})
                try:
                    sig = STRATEGIES[sname]["compute"](df, params)
                ...
                for slc in sl_grid:
                    for tgc in tg_grid:
                        ...
                        tally["tested"] += 1
                        by_strat[sname] = by_strat.get(sname, 0) + 1
                        cfg = dict(base_config_stub(), symbol=ysym, name=sym, interval=tf,
                                   period=period, strategy=sname, strat_params=params,
                                   sl_cfg=slc, tgt_cfg=tgc, filters={}, sl_mode=slc["mode"],
                                   tgt_mode=tgc["mode"], costs=costs, qty=qty)
                        try:
                            eng = make_engine(cfg, df, sig, al, as_)
                            trades, eq = eng.run()
                        except Exception:
                ...
                if stop_now:
                    break
    tally["_by_strategy"] = by_strat
    tally["_symbols_reached"] = symbols_done
    return rows, configs, skipped, tally
...
def render_verified_screener(base_cfg: dict):
    ...
    qty = qcol1.number_input("Quantity per trade", 1, 1_000_000, 1, key="vf_qty",
                             help="Charges are partly flat — brokerage is capped and the DP fee "
                                  "is fixed — so cost per share falls sharply with size. Testing "
                                  "at 1 share makes every setup look far worse than it is.")
    qcol2.caption("Set this to the size you would actually trade. At 1 share a delivery round "
                  "trip costs about 0.9% of turnover; at 100 shares it is 0.12%. A setup that "
                  "fails at quantity 1 can pass comfortably at a realistic size.")
    m1, m2, m3, m4 = st.columns(4)
    ...
    st.session_state["vf_qty_val"] = int(qty)
    ...
    e3.metric("Rough runtime", _runtime_estimate(min(total, int(cap)),
                                                 len(symbols) * max(len(tf_pairs), 1)))
    if total > cap:
        per_sym = max(total // max(len(symbols), 1), 1)
        reach = max(int(cap) // per_sym, 0)
        st.warning(
            f"The grid is {total:,} tests but the cap is {cap:,}, so the scan will stop after "
            f"roughly **{reach} of your {len(symbols)} tickers** — the rest are never looked at. "
            f"Raise the cap, or cut timeframes and strategies to get through the whole list.",
            icon="⚠️")
    if instrument.startswith("Options"):
    ...
    if st.button("Find verified live trades", type="primary", width="stretch"):
        ...
        with st.spinner("Testing, then verifying the survivors…"):
            rows, configs, skipped, tally = scan_verified_live(
                ...
                costs, oos, run_random, trials, int(cap), int(qty),
                progress=lambda f, t: (bar.progress(f), status.write(t)))
        bar.progress(1.0)
...
def _render_verified_results():
    ...
    n_tick = df_rows["Symbol"].nunique()
    st.success(f"**{len(df_rows)} verified setup(s) on {n_tick} ticker(s), firing right now.** "
               f"Each made money after costs, held up on bars it never saw, and risks less than "
               f"it stands to make."
               + (" Each also beat at least 80% of random-entry runs." if res["random"] else ""))
    st.caption(
        f"That is {len(df_rows)} survivors from {t['tested']:,} tests — not {len(df_rows)} out of "
        f"the tickers you scanned. Every ticker is tested against each strategy, timeframe, stop "
        f"and target, so one ticker alone accounts for hundreds of tests. A survival rate near "
        f"0.1% is what a strict filter looks like; if it were much higher the checks would not "
        f"be doing anything.")
    ...
    st.markdown("#### Which strategies were tested, and which produced anything")
    tested_by = t.get("_by_strategy", {})
    won_by = df_rows["Strategy"].value_counts().to_dict()
    if tested_by:
        brk = pd.DataFrame([{"Strategy": k, "Tests run": v, "Survivors": won_by.get(k, 0)}
                            for k, v in sorted(tested_by.items(), key=lambda kv: -kv[1])])
        st.dataframe(brk, width="stretch", hide_index=True)
        dead = [r["Strategy"] for _, r in brk.iterrows() if r["Survivors"] == 0]
        st.caption(
            f"Every strategy you ticked is tested against every timeframe, stop and target. "
            + (f"{len(dead)} produced no survivor here: {', '.join(dead[:6])}"
               + ("…" if len(dead) > 6 else "") + ". "
               if dead else "")
            + f"Tests reached {t.get('_symbols_reached', 0)} ticker(s) before the cap stopped it."
            + (f" {t['needs intraday bars']:,} tests were skipped because VWAP and opening-range "
               f"setups cannot work on daily or weekly candles."
               if t.get("needs intraday bars") else ""))
...
def render_edge_screener(base_cfg: dict):
    ...
    with c1:
        ...
        sync_universe_box("ed_uni", "ed_syms", uni, limit=100)
        pasted = st.text_area("Symbols", height=90, key="ed_syms",
                              help="Edit freely. Choosing a different universe refills this box.")
        symbols = [t.strip() for t in pasted.replace("\n", ",").split(",") if t.strip()][:60]
    with c2:
...
def render_edge_search(target_conf: int):
    ...
    with c1:
        ...
        sync_universe_box("es_uni", "es_syms", uni, limit=60)
        pasted = st.text_area("Symbols", height=90, key="es_syms",
                              help="Edit freely. Choosing a different universe refills this box.")
        symbols = [t.strip() for t in pasted.replace("\n", ",").split(",") if t.strip()][:60]
    with c2:
    ...
    st.markdown("##### In plain English")
    n_hold = int((r["Verdict"] == "edge holds").sum())
    n_lvl = int((r["Verdict"] == "clears the target").sum())
    if n_hold == 0 and n_lvl == 0:
        st.error(f"**Nothing worked.** {len(r):,} patterns were tested and none did better than "
                 f"what these instruments do anyway, once you allow for how many things were "
                 f"tried. That is the normal result and it has saved you from trading a "
                 f"coincidence.")
    else:
        st.success(f"**{n_hold} trading pattern(s) and {n_lvl} price level(s) survived the "
                   f"checks** out of {len(r):,} tested. Each is described below.")
