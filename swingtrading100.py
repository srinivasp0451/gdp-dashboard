    ...
    def run(self) -> tuple[pd.DataFrame, pd.Series]:
        ...
        for i in range(len(df)):
            ...
            if pos is not None:
                ...
                if px is None and not np.isnan(pos.sl):
...
def render_sidebar() -> dict:
    ...
    api_delay = r2.number_input("Gap between API calls (s)", 0.1, 10.0, YF_MIN_DELAY, step=0.1,
                                format="%.1f",
                                help="Minimum spacing between outbound requests. This is the "
                                     "rate-limit guard and is separate from the refresh rate.")
    if refresh < 5:
    ...
    return dict(source=source, group=g, name=name, symbol=symbol, interval=tf, period=period,
                ...
                refresh=float(refresh), api_delay=float(api_delay), dhan=dhan, gmail=gmail)
...
def render_backtest(cfg: dict, df: pd.DataFrame):
    ...
    if run_now:
        with st.spinner("Running…"):
            sig, trades, equity = run_config(cfg, df)
...
SCAN_COLS = ["Symbol", "Timeframe", "Period", "Filters", "Stop rule", "Stop settings",
             "Target rule", "Target settings", "Trades", "Hit rate %", "Net points",
             "Net P&L", "Profit factor", "Expectancy", "Sharpe", "Sortino", "Max drawdown",
             "Average R", "Average win", "Average loss", "Worst trade", "Flags", "Config"]
...
def render_search(base_cfg: dict):
    ss = st.session_state
    st.markdown("#### Scanner")
    st.caption(f"Sweeps instruments and exit settings against **{base_cfg['strategy']}** as "
               f"configured in the sidebar. Change the strategy or its parameters there.")
    ...
    with g2:
        ...
        tg_modes = st.multiselect("Target rules", TGT_MODES,
                                  default=["Fixed points", "Risk:reward multiple"], key="sc_tgm")
    ...
    rank_by = m1.selectbox("Rank by", ["Sharpe", "Net points", "Profit factor", "Expectancy",
    ...
    per_symbol = max(len(tf_pairs), 1) * max(len(fsets), 1) * max(len(sl_grid), 1) * max(len(tg_grid), 1)
...
def _execute_scan(base_cfg, symbols, tf_pairs, fsets, sl_grid, tg_grid, opt):
    ss = st.session_state
        ...
        for tf, period in tf_pairs:
            ...
            for fname, fset in fsets:
                cfg_base = dict(base_cfg, symbol=ysym, name=sym, interval=tf, period=period,
                                filters=fset, filter_logic="AND")
                sig = STRATEGIES[base_cfg["strategy"]]["compute"](df, base_cfg["strat_params"])
                ...
                for slc in sl_grid:
                    ...
                    for tgc in tg_grid:
                        ...
                        rows.append([sym, tf, period, fname, slc_s["mode"], describe(slc_s),
                                     ...
                                     m["Average R"], m["Average win"], m["Average loss"],
...
def _render_scan_results(base_cfg):
    ...
    labels = [f"#{i}  ·  {r['Symbol']} {r['Timeframe']}/{r['Period']}  ·  {r['Stop rule']} "
    ...
    if st.button("Load it into the backtest tab", type="primary", width="stretch"):
        row = res.iloc[labels.index(pick)]
        ss["replay_cfg"] = configs[int(row["Config"])]
        ss.pop("bt_result", None)
        st.success("Loaded. Open the Backtest tab and press Run backtest.")
...
def main():
    ...
    cfg = render_sidebar()
    tab_bt, tab_scan, tab_live, tab_hist = st.tabs(
        ["Backtest", "Search stocks", "Live trading", "Trade history"])
