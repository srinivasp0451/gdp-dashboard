...
def show_pattern_dialog(row: dict, piv_n: int, horizon: int):
    ...
    if st.button("Send ticker and timeframe to the sidebar", type="primary", width="stretch"):
        st.session_state["pending_sidebar"] = {
            "symbol": sym, "interval": row["Timeframe"],
            "period": TF_PERIODS[row["Timeframe"]][1], "source": "yfinance",
            "_label": f"{row['Symbol']} · {row['Timeframe']} · {row['Pattern']}"}
        st.rerun()
...
def _render_pattern_results():
    ...
    if hits.empty:
        ...
        if not view.empty:
            st.markdown("##### Inspect a hit")
            labels = [f"{r['Symbol']} · {r['Timeframe']} · {r['Pattern']} · "
                      f"{pd.Timestamp(r['Fired at']):%d-%b %H:%M} · {r['Bars ago']} bars ago"
                      for _, r in view.iterrows()]
            pick = st.selectbox("Row", labels, index=0, label_visibility="collapsed", key="pt_pick")
            row = view.iloc[labels.index(pick)].to_dict()
            b1, b2 = st.columns(2)
            if b1.button("Open chart with the pattern drawn", type="primary", width="stretch"):
                show_pattern_dialog(row, int(ss.get("pt_piv", 3)), int(res["horizon"]))
            if b2.button("Apply ticker and timeframe to the sidebar", width="stretch"):
                ss["pending_sidebar"] = {
                    "symbol": to_yf_symbol(row["Symbol"]), "interval": row["Timeframe"],
                    "period": TF_PERIODS[row["Timeframe"]][1], "source": "yfinance",
                    "_label": f"{row['Symbol']} · {row['Timeframe']} · {row['Pattern']}"}
                st.rerun()
        st.caption("Past win % and edge are measured on that same symbol and timeframe, over the "
                   f"{res['horizon']} bars after each historical occurrence. Edge is the pattern's "
                   f"average move minus the average move from every bar in the same window — the "
                   f"baseline matters, because in a rising market almost any bullish signal wins "
                   f"about 60% of the time.")
