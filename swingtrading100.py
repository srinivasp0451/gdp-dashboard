...
def scan_trade_ideas(symbols: list[str], horizon: str, instrument: str, recent: int,
                     min_trades: int, min_hit_lo: float, costs: CostModel,
    ...
    for n_done, sym in enumerate(symbols, start=1):
        ...
        for setup in setups:
            ...
            if m.get("Trades", 0) < min_trades:
                continue
            wins = m["Wins"]
            lo, hi = wilson_interval(wins, m["Trades"])
            if lo < min_hit_lo or m["Expectancy per trade"] <= 0:
                continue
...
def render_ideas(base_cfg: dict):
    ...
    symbols = [t.strip() for t in pasted.replace("\n", ",").split(",") if t.strip()]
    m1, m2, m3, m4 = st.columns(4)
    limit = m1.number_input("Scan at most", 1, 500, min(25, max(len(symbols), 1)), key="id_lim")
    ...
    min_hit = m4.number_input("Minimum win rate floor %", 0, 90, 40, key="id_minhit",
                              help="Applied to the pessimistic end of the confidence range, not "
                                   "the headline figure.")
    symbols = symbols[:int(limit)]

    use_costs = st.checkbox("Include brokerage, taxes and slippage in the record", value=True,
                            key="id_costs")
    ...
    if st.button("Find trades", type="primary", width="stretch"):
        ...
        with st.spinner("Testing every setup on every symbol…"):
            ideas, skipped = scan_trade_ideas(
                symbols, horizon, inst_key, int(recent), int(min_trades), float(min_hit),
                costs, progress=lambda f, t: (bar.progress(min(f, 1.0)), status.write(t)))
        bar.progress(1.0)
    ...
    rows = res["rows"]
    if not rows:
        st.error("**No trades right now.** Either nothing fired in the window you asked for, or "
                 "what fired came from setups without a good enough record on that instrument. "
                 "That is a normal outcome — most days there is nothing worth taking. Widen the "
                 "bar window, lower the win-rate floor, or scan more symbols.")
        if res["skipped"]:
