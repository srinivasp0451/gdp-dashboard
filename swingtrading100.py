...
def render_hit_rows(rows: pd.DataFrame, piv_n: int, horizon: int, key_prefix: str) -> None:
    ...
    for pos, (_, r) in enumerate(rows.iterrows()):
        row = r.to_dict()
        cols = st.columns([2.6, 1.5, 1.0, 1.0, 1.0])
...
def _render_pattern_results():
    ...
    if hits.empty:
        ...
        st.caption("Each row carries its levels and its own buttons. Chart draws the pattern, "
                   "Levels opens the full plan with its historical record, Sidebar loads the "
                   "whole setup ready to backtest.")
        show_n = st.slider("Rows to show here", 5, 60, 15, 5, key="pt_showrows")
        render_hit_rows(view.head(int(show_n)), piv_n, horizon, "fired")
        if len(view) > show_n:
            st.caption(f"{len(view) - int(show_n)} more below in *Act on a hit*, or raise the "
                       f"slider.")

        with st.expander("The same hits as a plain table", expanded=False):
