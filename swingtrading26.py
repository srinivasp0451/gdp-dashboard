...
# ---------- Helper functions placed early to avoid NameError ----------
...
def generate_summary_text(df):
    ...
    ma_len = min(200, len(df))
    long_ma = df['close'].rolling(window=ma_len).mean().iloc[-1] if len(df)>=1 else df['close'].iloc[-1]
        ...
        if best is None or best['stats'] is None:
            ...
            if not temp_trades.empty:
                ...
                if not is_live.empty:
                    ...
                    try:
                        sl_display = rec['entry_price'] - (rec['entry_price'] - rec['exit_price']) if rec['side']=='long' else rec['entry_price'] + (rec['exit_price'] - rec['entry_price'])
                    except Exception:
                        sl_display = rec['entry_price']
