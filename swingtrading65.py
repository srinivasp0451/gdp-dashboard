...
def render_backtesting(config):
    ...
    with tab3:
        ...
        if df is not None and not df.empty:
            ...
            st.dataframe(
                display_df[['Open', 'High', 'Low', 'Close', 'Change (Points)', 'Change (%)', 'Day of Week']].style.applymap(
                    color_change, subset=['Change (Points)', 'Change (%)']
                ),
...
if __name__ == "__main__":
    main() if position_type == 'LONG' else entry_price + 10
