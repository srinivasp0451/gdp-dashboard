...
def sidebar():
    with st.sidebar:
        ...
        days = st.number_input("History (calendar days)", 1, cap,
                               min(cap, 59 if cap > 59 else cap))
        if interval in ("1m", "2m", "5m", "15m", "30m"):
            st.caption(f"Yahoo caps {interval} history at {cap} days.")
    ...
    return dict(source="yfinance" if use_yf else "upload", symbol=symbol, upload=upload,
                interval=interval, days=int(days), dirs=dirs, cost_bps=cost_bps,
                slippage=slippage, intraday=intraday, n_iter=int(n_iter),
                min_trades=int(min_trades), train_frac=train_frac, tz=tz)
...
def tab_backtest(cfg, df, label):
    ...
    st.markdown("### Full-sample result")
    st.json(best["full_stats"])
    st.caption("Full-sample includes the training window, so it is descriptive only. "
               "Judge the strategy on the holdout column above.")
...
def tab_live(cfg):
    ...
    def render():
        ...
        st.caption(f"{status} · {cfg['symbol']} · last price {last:,.2f} · "
                   f"bar {pd.Timestamp(df['Date'].iloc[-1]):%d %b %H:%M} · "
                   f"updated {pd.Timestamp.now(tz=cfg['tz']):%H:%M:%S}")
        ...
        m[3].metric("Expectancy", f"{s['expectancy_points']:+.2f} pts")
        hold_stats = best["test_stats"] or best["train_stats"]
        m[4].metric("Backtest accuracy", f"{hold_stats['win_rate']*100:.1f}%",
        ...
        if ls["position"]:
            ...
            st.markdown("#### Flat — waiting for a signal")
...
def tab_history(cfg):
    ...
    if not best:
        st.caption("Run the optimisation to populate this.")
        return
    bt = best["full_trades"]
    if bt.empty:
        st.caption("The fitted parameters produced no trades.")
        return
    only_hold = st.checkbox("Show holdout trades only", value=False)
    show = best["test_trades"] if only_hold and not best["test_trades"].empty else bt
    st.dataframe(show.iloc[::-1], use_container_width=True)
    st.download_button("Download backtest trades (CSV)", show.to_csv(index=False),
