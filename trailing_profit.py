...
strike_price = st.number_input("Select Strike Price", min_value=0, step=50,value=23000)
            ...
            while True:
                ...
                if 'LTP' in response.keys():
                    ...
                    if order_status == "placed":
                        ...
                        if use_trailing_profit_threshold=="Yes":
                            # Calculate the trailing stop loss based on the highest price
                            trailing_profit_threshold = highest_profit - profit_threshold_distance
                            print(f"Entry price: {entry_price} Trailing Stop Loss at: {trailing_stop_loss_price} highest price: {highest_price} target: {current_target}")
                            trailing_placeholder.markdown(f"Entry price: {entry_price} Trailing Stop Loss at: {trailing_stop_loss_price} highest price: {highest_price} highest profit: {highest_profit} trailing profit: {trailing_profit_threshold} target: {current_target}")
                        ...
                        if profit_or_loss >= profit_threshold:
                        ...
                        elif profit_or_loss <=  trailing_profit_threshold :
                            print(f"trailing profit threshold reached! Exiting trade with loss of {profit_or_loss}.")
                            st.write(f"trailing profit  threshold reached! Exiting trade with loss of {profit_or_loss}.")
                            order_status = "trailing profit threshold hit"
                            # Exit the trade (place a sell order)
                            ...
                            profit_or_loss = profit_or_loss  # Initial profit/loss is zero
                            # profit_points = float(ltp) - entry_price  # Initial profit points
                            profit_points = float(ltp) - entry_price  # Initial loss points
                            # backtest = False  # Default to backtesting,  set to False for live trading
