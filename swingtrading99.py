...
def _fno_underlying_frame(instrument_code, underlying, exchange="NSE"):
    ...
    try:
        ...
        # Match on the UNDERLYING-NAME column first — it is exact and
        # unambiguous. Fall back to the trading symbol only with a delimiter
        # boundary.
        #
        # A bare startswith() was the previous behaviour and it silently
        # returned the wrong contract: "NIFTY" also matches NIFTYNXT50, and a
        # stock like "TATA" matches TATAMOTORS/TATASTEEL/TATACHEM. Since
        # dhan_lookup_future takes the first match, that is how a Nifty future
        # could come back priced at 1,595 instead of ~24,200.
        if c["name"]:
            mask |= (sub[c["name"]].astype(str).str.upper().str.strip() == u)
        if c["symbol"]:
            sym = sub[c["symbol"]].astype(str).str.upper().str.strip()
            # Trading symbols look like "NIFTY-Aug2026-FUT" or "NIFTY 28AUG2026 CE 24000"
            mask |= (sym == u) | sym.str.startswith(u + "-") | sym.str.startswith(u + " ")
        return sub[mask], c
    except Exception:
        return pd.DataFrame(), c
...
def record_chain_history(snap, futures=None, underlying_label=None):
    ...
    if hist and hist[-1].get("Time") == stamp:
        return
    # STALE-CHAIN GUARD. Outside market hours (and during any feed stall) Dhan
    # keeps returning the last traded chain with a fresh fetch timestamp. Those
    # are not new observations: recording them produces a series of identical
    # points, which is exactly why a PCR chart can look like a dead flat line
    # with two rows holding the same OI. If every OI figure matches the last
    # recorded row, the chain has not moved, so no new point is stored and the
    # UI is told why.
    if hist:
        prev = hist[-1]
        same = all(
            (prev.get(k) is None and snap.get(v) is None)
            or (prev.get(k) is not None and snap.get(v) is not None
                and abs(float(prev.get(k) or 0) - float(snap.get(v) or 0)) < 1e-9)
            for k, v in (("CE OI", "ce_oi"), ("PE OI", "pe_oi"),
                         ("CE Volume", "ce_volume"), ("PE Volume", "pe_volume"))
        )
        if same:
            now = ist_now()
            closed = now.weekday() >= 5 or not (dtime(9, 15) <= now.time() <= dtime(15, 30))
            st.session_state["chain_stale_note"] = (
                f"Chain unchanged since {prev.get('Time')} — "
                + ("the market is closed, so Dhan keeps returning the last traded chain. "
                   "New points will appear when trading resumes."
                   if closed else
                   "the feed has returned an identical snapshot. This is normal between updates; "
                   "Dhan refreshes the chain about once a minute.")
                + " No duplicate point was recorded, so the chart shows only genuine observations.")
            return
    st.session_state.pop("chain_stale_note", None)
...
# Midcap/smallcap names that, added to the Nifty 100, approximate the Nifty 200.
# IMPORTANT: index membership is decided by NSE and changes at each rebalance,
# and there is no offline source of truth for it here — so treat these as a
# curated large-cap/midcap watchlist rather than the exact index constituents.
# For an exact list, paste it into the Custom option.
NIFTY_MIDCAP_EXTRA = [
    "ASHOKLEY", "AUROPHARMA", "BALKRISIND", "BANDHANBNK", "BHARATFORG", "BHEL",
    "BIOCON", "CGPOWER", "COFORGE", "CONCOR", "CUMMINSIND", "DALBHARAT", "DEEPAKNTR",
    "DIXON", "ESCORTS", "EXIDEIND", "FEDERALBNK", "GLENMARK", "GMRINFRA", "GODREJPROP",
    "HINDPETRO", "IDFCFIRSTB", "INDHOTEL", "INDIANB", "INDUSTOWER", "IPCALAB",
    "IRCTC", "JUBLFOOD", "LICHSGFIN", "LUPIN", "MANAPPURAM", "MFSL", "MPHASIS",
    "MRF", "NATIONALUM", "NMDC", "OBEROIRLTY", "OFSS", "PAGEIND", "PERSISTENT",
    "PETRONET", "PIIND", "POLICYBZR", "PRESTIGE", "RAMCOCEM", "RBLBANK", "SAIL",
    "SJVN", "SONACOMS", "STARHEALTH", "SUNTV", "SUPREMEIND", "SYNGENE", "TATACOMM",
    "TATAELXSI", "TIINDIA", "TORNTPOWER", "TRIDENT", "UBL", "UNIONBANK", "VOLTAS",
    "YESBANK", "ZEEL", "ABCAPITAL", "ABFRL", "ALKEM", "APLAPOLLO", "ASTRAL",
    "ATGL", "AUBANK", "BATAINDIA", "BSOFT", "CANFINHOME", "CHAMBLFERT", "CROMPTON",
    "CYIENT", "DELHIVERY", "GNFC", "GRANULES", "GSPL", "HFCL", "IEX", "IGL",
    "INDIAMART", "IRB", "KPITTECH", "LALPATHLAB", "LAURUSLABS", "METROPOLIS",
    "MGL", "NAVINFLUOR", "PEL", "PVRINOX", "RECLTD", "SUNDARMFIN", "TATACHEM",
    "TVSMOTOR", "ZYDUSLIFE", "POONAWALLA", "KEI",
]
...
SCREENER_INDEX_SYMBOLS = [
    "^NSEI", "^NSEBANK", "^BSESN", "NIFTY_FIN_SERVICE.NS", "^CNXIT", "^CNXREALTY",
    "^CNXAUTO", "^CNXPHARMA", "^CNXFMCG", "^CNXMETAL", "^CNXENERGY", "^CNXPSUBANK",
    "^NSEMDCP50", "^INDIAVIX",
]
SCREENER_COMMODITY_SYMBOLS = ["GC=F", "SI=F", "CL=F", "NG=F", "HG=F", "PL=F", "ZC=F", "ZS=F"]
SCREENER_FOREX_SYMBOLS = ["USDINR=X", "EURINR=X", "GBPINR=X", "JPYINR=X",
                          "EURUSD=X", "GBPUSD=X", "USDJPY=X", "AUDUSD=X", "USDCAD=X", "DX-Y.NYB"]
SCREENER_CRYPTO_SYMBOLS = ["BTC-USD", "ETH-USD", "BNB-USD", "SOL-USD", "XRP-USD",
...
def fno_stock_universe():
    """
    Every stock with listed options, taken from Dhan's scrip master.

    This is offered instead of a hard-coded "Nifty 500" list: index membership
    changes at every rebalance and cannot be derived offline, whereas the F&O
    universe IS derivable and is what a derivatives screener actually wants.
    Returns [] without a scrip master (no Dhan token / offline).
    """
    try:
        sm = load_dhan_scrip_master()
        if sm.empty:
            return []
        c = _scrip_cols(sm)
        if not (c["instrument"] and c["name"]):
            return []
        sub = sm[sm[c["instrument"]].astype(str).str.strip() == "OPTSTK"]
        names = sorted(set(sub[c["name"]].astype(str).str.upper().str.strip()))
        return [n for n in names if n and n.isascii() and 1 < len(n) <= 20]
    except Exception:
        return []
...
def get_futures_price(uinfo, expiry=None):
    ...
    try:
        ...
        if not info:
            return None
        px = _dhan_future_ltp_cached(info["security_id"], f"{uinfo['exchange']}_FNO",
                                     hash(token) % 10_000_019)
        if px is None:
            return None
        # A future trades close to spot (basis is a fraction of a percent, not
        # a factor). If the resolved contract is wildly off, it is the wrong
        # instrument rather than a real price, so reject it instead of
        # publishing a nonsense number into the tables and charts.
        try:
            spot = dhan_get_ltp(uinfo["security_id"], uinfo["segment"])
            if spot and spot > 0 and not (0.85 <= float(px) / float(spot) <= 1.15):
                st.session_state["futures_mismatch_note"] = (
                    f"Futures lookup for {uinfo['label']} returned {px:,.2f} against a spot of {spot:,.2f} — "
                    "that is not the same instrument, so it has been discarded rather than shown. "
                    "Check the expiry and that the scrip master is current.")
                return None
        except Exception:
            pass
        st.session_state.pop("futures_mismatch_note", None)
        return px
    except Exception:
        return None
...
def screener_fetch(symbol, interval, period, source):
    ...
    """
    # Only bare NSE stock codes get the .NS suffix. Indices (^NSEI), futures
    # (GC=F), forex (USDINR=X) and crypto (BTC-USD) are already complete
    # yfinance symbols and must be passed through untouched.
    _sym = str(symbol).strip()
    if _sym.endswith((".NS", ".BO")) or _sym.startswith("^") or any(
            ch in _sym for ch in ("=", "-")) or "." in _sym:
        yf_symbol = _sym
    else:
        yf_symbol = f"{_sym}.NS"
    try:
...
with tab_screen:
    ...
    if scr_universe == "Custom list":
    ...
    elif scr_universe == "Nifty 200 (approx.)":
        _symbols = list(NIFTY200_SYMBOLS)
        st.caption("⚠️ NSE decides index membership and revises it at each rebalance, and there is no offline "
        ...
        if _symbols:
            st.caption(f"{len(_symbols)} stocks with listed options, read live from Dhan's scrip master. This is "
                       "offered instead of a hard-coded Nifty 500 because it is genuinely derivable and stays "
                       "current, whereas an index list would silently go stale.")
        else:
            st.warning("Could not read the scrip master — this needs a Dhan connection. Falling back to Nifty 100.")
            _symbols = list(NIFTY100_SYMBOLS)
    elif scr_universe == "Indices":
        _symbols = list(SCREENER_INDEX_SYMBOLS)
    elif scr_universe == "Commodities":
        _symbols = list(SCREENER_COMMODITY_SYMBOLS)
    elif scr_universe == "Forex":
        _symbols = list(SCREENER_FOREX_SYMBOLS)
    elif scr_universe == "Crypto":
        _symbols = list(SCREENER_CRYPTO_SYMBOLS)
    elif scr_universe.startswith("Indices +"):
        _symbols = (list(SCREENER_INDEX_SYMBOLS) + list(SCREENER_COMMODITY_SYMBOLS)
                    + list(SCREENER_FOREX_SYMBOLS) + list(SCREENER_CRYPTO_SYMBOLS))
    else:
        _symbols = list(NIFTY50_SYMBOLS)

    if scr_universe in ("Indices", "Commodities", "Forex", "Crypto") or scr_universe.startswith("Indices +"):
        st.caption("These are yfinance symbols and are scanned as-is, so they work without a Dhan token. "
...
with tab_pcr:
    ...
    def _pcr_intraday():
        ...
        st.caption(f"{len(merged)} rows on the {pcr_tf} grid · {_real} real snapshot(s) · "
                   ...
                   f"all times IST")
        if st.session_state.get("chain_stale_note"):
            st.warning("⏸ " + st.session_state["chain_stale_note"])
        if st.session_state.get("futures_mismatch_note"):
            st.warning("⚠️ " + st.session_state["futures_mismatch_note"])
        _uniq = int(pd.to_numeric(day.get("PCR"), errors="coerce").dropna().nunique()) if "PCR" in day.columns else 0
        if _uniq <= 1:
            st.info(f"📏 Only **{_uniq} distinct PCR value** has been recorded, so the line is necessarily flat — "
                    "there is nothing yet for it to move between. This is normal right after starting, and "
                    "expected outside market hours when the chain is frozen. Leave Auto-capture running during "
                    "the session and the curve will develop (Dhan supplies roughly one new value per minute).")
        ...
        _ys = pd.to_numeric(merged["PCR"], errors="coerce").dropna()
        _yaxis = dict(title="PCR (OI)")
        if len(_ys) and float(_ys.max() - _ys.min()) < 1e-9:
            # A constant series would otherwise render against an auto-scaled
            # axis that exaggerates meaningless noise; pin a readable band.
            _c = float(_ys.iloc[0])
            _yaxis["range"] = [_c - max(0.05, abs(_c) * 0.05), _c + max(0.05, abs(_c) * 0.05)]
        fig.update_layout(title=f"{_pcr_label} — PCR vs time ({pcr_tf}, IST)",
                          height=430, hovermode="x unified",
                          xaxis_title="Time (IST)", yaxis=_yaxis,
