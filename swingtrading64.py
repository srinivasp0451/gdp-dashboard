                ...
                if len(res['trades']) == 0:
                    ...
                    for idx, t in enumerate(res['trades']):
                        with st.expander(f"#{idx + 1} - {t['signal']} - P&L: {t['pnl']:.2f}"):
                            c1, c2 = st.columns(2)
                            with c1:
                                st.write(f"**Entry Time:** {t['entry_time'].strftime('%Y-%m-%d %H:%M:%S')}")
                                st.write(f"**Exit Time:** {t['exit_time'].strftime('%Y-%m-%d %H:%M:%S')}")
                                st.write(f"**Duration:** {t['duration']:.2f}h")
                                st.write(f"**Signal:** {t['signal']}")
                                st.write(f"**Entry Price:** {t['entry_price']:.2f}")
                            with c2:
                                st.write(f"**Exit Price:** {t['exit_price']:.2f}")
                                sl_str = f"{t['sl']:.2f}" if t['sl'] != 0 else "Signal Based"
                                st.write(f"**SL:** {sl_str}")
                                tgt_str = f"{t['target']:.2f}" if t['target'] != 0 else "Signal Based"
                            ...
                            if t['pnl'] >= 0:
                                ...
                                st.error(f"**P&L:** {t['pnl']:.2f}")
                            st.info(f"**Highest:** {t.get('highest', 0):.2f} | **Lowest:** {t.get('lowest', 0):.2f} | **Range:** {t.get('range', 0):.2f}")
        else:
