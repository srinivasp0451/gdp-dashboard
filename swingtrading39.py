...
def convert_to_ist(df):
    """Convert datetime index to IST and return a dataframe with a naive IST index."""
    try:
        ist = pytz.timezone('Asia/Kolkata')
        # If index is timezone-naive assume it's UTC and localize then convert
        ...
        # Make index naive (drop tzinfo) so downstream code is simpler
        df.index = df.index.tz_localize(None)
    except Exception:
        # If anything goes wrong, return original df
        return df
    return df
...
def find_pattern_similarity(data, window=20, top_n=5):
    ...
    # Require at least 2 windows + 1 future point
    ...
    for i in range(window, len(data) - window):
        ...
        if not np.isnan(correlation):
            # future return measured one period after the pattern (i + window)
            future_idx = i + window
            if future_idx < len(data):
                future_return = (data.iloc[future_idx] - data.iloc[i]) / data.iloc[i] * 100
                similarities.append({
                    'index': i,
                    'date': data.index[i],
...
# Helper to build safe small tables from series
def build_recent_stats_table(series, rows=10):
    s = series.dropna().iloc[-rows:]
...
# Helper to reshape arrays for heatmaps (pad if necessary)
def reshape_for_heatmap(arr, cols=5, max_cells=50):
    arr = np.asarray(arr)
    if arr.size == 0:
        return np.array([[0]])
    # limit to most recent values
    n = min(arr.size, max_cells)
    trimmed = arr[-n:]
    # if not divisible by cols, drop remainder at the start so we keep most recent values
    remainder = trimmed.size % cols
    if remainder != 0:
        trimmed = trimmed[remainder:]
    if trimmed.size == 0:
        # fallback to 1-row
        return np.array([arr[-min(arr.size, cols):]])
    return trimmed.reshape(-1, cols)
...
if fetch_button:
    with st.spinner("Fetching data from Yahoo Finance..."):
        try:
            ...
            # Convert to IST (and make index naive)
...
if st.session_state.data_fetched:
    ...
    with col1:
        ...
        pct_change1 = (change1 / prev_price1) * 100 if prev_price1 != 0 else 0
        ...
        # Build safe stats table
        stats1 = build_recent_stats_table(data1['Close'], rows=10)
    ...
    with col2:
        ...
        pct_change2 = (change2 / prev_price2) * 100 if prev_price2 != 0 else 0
        ...
        # Build safe stats table
        stats2 = build_recent_stats_table(data2['Close'], rows=10)
    ...
    # Build ratio_analysis safely by aligning on the data1 index
    ratio_analysis = pd.DataFrame(index=data1.index)
    ratio_analysis[f'{ticker1_name} Price'] = data1['Close']
    ratio_analysis[f'{ticker2_name} Price'] = data2['Close']
    ratio_analysis['Ratio'] = ratio
    ratio_analysis[f'RSI {ticker1_name}'] = rsi1
    ratio_analysis[f'RSI {ticker2_name}'] = rsi2
    ratio_analysis['RSI Ratio'] = rsi_ratio
    ...
    # Create ratio bins safely (dropna)
    ratio_bins = pd.qcut(ratio.dropna(), q=10, duplicates='drop')
    ratio_binned = pd.DataFrame({
        'DateTime': ratio.index,
        'Ratio': ratio.values
    }).dropna()
    ratio_binned['Bin'] = pd.cut(ratio_binned['Ratio'], bins=np.unique(np.quantile(ratio_binned['Ratio'], np.linspace(0,1,11))))
    ratio_binned['Price1'] = data1['Close'].values
    ratio_binned['Price2'] = data2['Close'].values

    # Calculate forward returns for bin analysis
    ratio_binned = ratio_binned.reset_index(drop=True)
    ...
    **📌 Bin Analysis Insights:** This table shows the average forward returns for each ratio bin. 
    ...
    # RSI plots
    fig.add_trace(go.Scatter(x=data1.index, y=rsi1, name=f'RSI {ticker1_name}'), row=4, col=1)
    fig.add_hline(y=70, line_dash="dash", row=4, col=1)
    fig.add_hline(y=30, line_dash="dash", row=4, col=1)

    fig.add_trace(go.Scatter(x=data2.index, y=rsi2, name=f'RSI {ticker2_name}'), row=5, col=1)
    fig.add_hline(y=70, line_dash="dash", row=5, col=1)
    fig.add_hline(y=30, line_dash="dash", row=5, col=1)

    fig.add_trace(go.Scatter(x=data1.index, y=rsi_ratio, name='RSI Ratio'), row=6, col=1)
    fig.add_hline(y=70, line_dash="dash", row=6, col=1)
    fig.add_hline(y=30, line_dash="dash", row=6, col=1)
    ...
    # Build export dataframe safely by aligning both dataframes on index
    export_df = pd.concat([
        data1[['Open','High','Low','Close','Volume']].rename(columns=lambda c: f'{ticker1_name}_{c}'),
        data2[['Open','High','Low','Close','Volume']].rename(columns=lambda c: f'{ticker2_name}_{c}')
    ], axis=1)
    export_df = export_df.assign(Ratio=ratio)
    export_df = export_df.reset_index().rename(columns={'index':'DateTime_IST'})
    ...
    points_data = export_df.copy()
    # add Points and % changes
    points_data[f'Points {ticker1_name}'] = points_data[f'{ticker1_name}_Close'].diff()
    points_data[f'Points {ticker2_name}'] = points_data[f'{ticker2_name}_Close'].diff()
    points_data['Points Ratio'] = points_data['Ratio'].diff()
    points_data[f'% Change {ticker1_name}'] = points_data[f'{ticker1_name}_Close'].pct_change() * 100
    points_data[f'% Change {ticker2_name}'] = points_data[f'{ticker2_name}_Close'].pct_change() * 100
    points_data['% Change Ratio'] = points_data['Ratio'].pct_change() * 100
    ...
    **📌 Volatility Insights:** 
    ...
    if f'Vol {ticker1_name}' not in vol_binned.columns:
        vol_binned[f'Vol {ticker1_name}'] = vol1.reindex(export_df['DateTime_IST']).values
    vol_binned[f'Vol_Bin_{ticker1_name}'] = pd.qcut(vol_binned[f'Vol {ticker1_name}'].fillna(0), q=5, labels=['Very Low', 'Low', 'Medium', 'High', 'Very High'], duplicates='drop')
    ...
    **📌 Volatility Binning Insights:** This shows how price movements correlate with volatility levels.
    ...
    # Day vs Week returns (percentage)
    ...
    if len(returns1_daily.dropna()) > 0:
        st.subheader(f"{ticker1_name} Returns Heatmap")
        returns1_matrix = reshape_for_heatmap(returns1_daily.dropna().values, cols=5, max_cells=50)
        ...
        returns2_matrix = reshape_for_heatmap(returns2_daily.dropna().values, cols=5, max_cells=50)
        ...
        ratio_returns_matrix = reshape_for_heatmap(ratio_returns_daily.dropna().values, cols=5, max_cells=50)
    ...
    if len(vol1.dropna()) > 0:
        vol1_matrix = reshape_for_heatmap(vol1.dropna().values, cols=5, max_cells=50)
        ...
        vol2_matrix = reshape_for_heatmap(vol2.dropna().values, cols=5, max_cells=50)
    ...
    if 'sim_df1' in locals() and 'sim_df_ratio' in locals():
    ...
    # 2. Ratio Analysis
    ratio_zscore = (current_ratio - mean_ratio) / (std_ratio + 1e-12)
    if ratio_zscore < -1:
        recommendation_score += 1.5
        reasons.append(f"✅ Ratio ({current_ratio:.4f}) is {abs(ratio_zscore):.2f} std below mean - {ticker1_name} undervalued vs {ticker2_name}")
    elif ratio_zscore > 1:
        recommendation_score -= 1.5
        reasons.append(f"⚠️ Ratio ({current_ratio:.4f}) is {ratio_zscore:.2f} std above mean - {ticker1_name} overvalued vs {ticker2_name}")

    # 3. Pattern Similarity
    if 'sim_df1' in locals():
    ...
    **⚠️ Disclaimer:** This analysis is based on historical patterns and statistical models. 
    ...
    with col1:
        ...
        volume_ratio = current_volume1 / avg_volume1 if avg_volume1 != 0 else 0
