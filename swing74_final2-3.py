    ...
    if df_plot is not None and not df_plot.empty:
        ...
        try:
            ...
            if all(v is not None for v in [lc_start, lc_open, lc_high, lc_low, lc_close]):
                ...
                # Robust timestamp comparison (avoids timezone format mismatch)
                try:
                    last_ts = pd.Timestamp(plot_df.iloc[-1]["Datetime"]).floor("min")
                    lc_ts   = pd.Timestamp(lc_start).floor("min")
                    same_candle = len(plot_df) > 0 and last_ts == lc_ts
                except Exception:
                    same_candle = len(plot_df) > 0 and str(plot_df.iloc[-1]["Datetime"])[:16] == str(lc_start)[:16]
                if same_candle:
