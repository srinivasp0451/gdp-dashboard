...
# -------- FIND SPOT GUESS --------
spot_guess = float(df_all.loc[df_all['CE VOLUME'] == df_all['CE VOLUME'].max(), 'STRIKE'].iloc[0])
...
def find_best(df_all, spot):
    ...
    # CE candidate = high CE volume, positive change in OI, decent IV
    ...
    # PE candidate = high PE volume, positive change in OI, decent IV
    ...
    if not best_ce.empty:
        ...
        logic = (f"High CE volume ({row['CE VOLUME']}) with OI up by {row['CE CHNGOI']} "
                 f"and IV {row['CE IV']}% — indicates upside momentum build-up.")
    ...
    if not best_pe.empty:
        ...
        logic = (f"High PE volume ({row['PE VOLUME']}) with OI up by {row['PE CHNGOI']} "
                 f"and IV {row['PE IV']}% — indicates downside momentum build-up.")
...
plot_bar("OI Change", df_all['CE CHNGOI'], df_all['PE CHNGOI'], df_all['STRIKE'])
