...
# ─────────────────────────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────────────────────────
# REAL-TIME LTP — Dhan MarketFeed WebSocket + yfinance fallback
#
# For Dhan: uses dhanhq.marketfeed.MarketFeed (WebSocket).
#   One persistent connection pushes ticks instantly — no polling delay.
#   Thread writes LTP to queue.Queue; fragment reads it in microseconds.
#
# For yfinance: polls fast_info every 1 s in a background thread.
#   Still faster than calling it on the main fragment thread.
# ─────────────────────────────────────────────────────────────────────────────

# Exchange segment map: sidebar label → marketfeed constant
_DHAN_SEG_MAP = {
    'IDX_I (Index)': 0,   # marketfeed.IDX
    'NSE_EQ':        1,   # marketfeed.NSE
    'BSE_EQ':        4,   # marketfeed.BSE
    'NSE_FNO':       2,   # marketfeed.NSE_FNO
    'BSE_FNO':       8,   # marketfeed.BSE_FNO
    'MCX_COMM':      5,   # marketfeed.MCX
}


def _dhan_ws_worker(client_id, access_token, security_id, exchange_segment_label,
                    ltp_q, stop_ev):
    """
    Background thread: opens Dhan MarketFeed WebSocket, receives ticks,
    writes LTP to ltp_q.  Never touches st.session_state.
    """
    try:
        from dhanhq import marketfeed, DhanContext
        seg_int = _DHAN_SEG_MAP.get(exchange_segment_label, 0)
        instruments = [(seg_int, str(security_id), marketfeed.Ticker)]
        ...
        def _on_ticks(data):
            ...
            try:
                ...
                if isinstance(data, dict):
                    ltp_val = data.get('LTP') or data.get('ltp') or data.get('last_price')
                elif isinstance(data, list) and data:
                    d = data[0]
                    ltp_val = d.get('LTP') or d.get('ltp') if isinstance(d, dict) else None
                if ltp_val is not None:
                    ltp_float = float(ltp_val)
                    if ltp_float > 0:
                        # Drain old value, put latest
                        try:    ltp_q.get_nowait()
                        except: pass
                        try:    ltp_q.put_nowait(ltp_float)
                        except: pass
            except Exception:
                pass

        feed = marketfeed.MarketFeed(ctx, instruments, on_ticks=_on_ticks)
        ...
        # WebSocket failed — fall back to REST polling in this thread
        _rest_ltp_worker_fallback(ltp_q, stop_ev, client_id, access_token,
                                   security_id, exchange_segment_label)


def _rest_ltp_worker_fallback(ltp_q, stop_ev,
                               client_id=None, access_token=None,
                               security_id=None, exchange_segment_label=None,
                               config_snap=None):
    """
    Fallback: poll REST API every 1 s.
    Used for yfinance, or if Dhan WebSocket fails.
    """
    while not stop_ev.is_set():
        try:
            if config_snap:
                ltp = fetch_ltp(config_snap)
            else:
                # Dhan REST fallback
                try:
                    from dhanhq import dhanhq as DhanHQ
                    dhan = DhanHQ(client_id, access_token)
                    seg_map = {v: k for k, v in {
                        'IDX_I (Index)': 'IDX_I', 'NSE_EQ': 'NSE_EQ', 'BSE_EQ': 'BSE_EQ',
                        'NSE_FNO': 'NSE_FNO', 'MCX_COMM': 'MCX_COMM'
                    }.items()}
                    seg = {'IDX_I (Index)': 'IDX_I', 'NSE_EQ': 'NSE_EQ',
                           'BSE_EQ': 'BSE_EQ', 'NSE_FNO': 'NSE_FNO',
                           'MCX_COMM': 'MCX_COMM'}.get(exchange_segment_label, 'NSE_EQ')
                    resp = dhan.get_ltp_data(securities={seg: [str(security_id)]})
                    ltp = None
                    if resp and resp.get('status') == 'success':
                        for sv in resp.get('data', {}).values():
                            for sv2 in (sv.values() if isinstance(sv, dict) else []):
                                if isinstance(sv2, dict):
                                    ltp = float(sv2.get('last_price') or sv2.get('ltp') or 0) or None
                except Exception:
                    ltp = None
...
def start_ltp_thread(config):
    """
    Start real-time LTP feed.
    Dhan → WebSocket (instant ticks).
    yfinance → REST poll every 1 s in background thread.
    """
    # Stop any existing thread
    old_ev = st.session_state.get('_ltp_stop_event')
    if old_ev:
        old_ev.set()

    stop_ev = _th.Event()
    ltp_q   = _queue.Queue(maxsize=1)
    st.session_state['_ltp_stop_event'] = stop_ev
    st.session_state['_ltp_queue']      = ltp_q
    ...
    if not use_yf:
        ...
        access_token = config.get('dhan_data_access_token') or config.get('dhan_access_token', '')
        security_id  = config.get('dhan_data_security_id', '')
        exch_seg     = config.get('dhan_data_exchange', 'IDX_I (Index)')
        t = _th.Thread(
            target=_dhan_ws_worker,
            args=(client_id, access_token, security_id, exch_seg, ltp_q, stop_ev),
            daemon=True,
        )
    else:
        # yfinance REST poll
        t = _th.Thread(
            target=_rest_ltp_worker_fallback,
            args=(ltp_q, stop_ev),
            kwargs={'config_snap': config.copy()},
            daemon=True,
        )

    t.start()
    return ltp_q, stop_ev


def get_ltp_from_thread(config):
    """Read latest LTP from background feed (microseconds). Falls back to direct fetch."""
    ltp_q = st.session_state.get('_ltp_queue')
    if ltp_q is not None:
        try:
            ltp = ltp_q.get_nowait()
            st.session_state['_cached_ltp'] = ltp
            return ltp
        except _queue.Empty:
            cached = st.session_state.get('_cached_ltp')
            if cached:
                return cached
    return fetch_ltp(config)
...
def live_trading_iteration():
    ...
    ltp = get_ltp_from_thread(config)   # instant — background thread
    if ltp is not None and ltp > 0:
        color = '🟢' if ltp >= (st.session_state.get('live_candle_open') or ltp) else '🔴'
        add_log(f"💹 LTP ₹{ltp:.2f} {color}")

    # ── Step 2: Build current_data from LTP immediately ─────────────────────
    #    Uses cached indicator_df so this is instant — no API call.
    #    SL/Target/PnL checks in entry/exit always use the freshest LTP.
    _df_cached = st.session_state.get('indicator_df')
    if _df_cached is not None and not _df_cached.empty:
        _cd       = _df_cached.iloc[-1].copy()
        _cp       = ltp if (ltp is not None and ltp > 0) else float(_df_cached.iloc[-1]['Close'])
        _cd['Close'] = _cp
        st.session_state['current_data'] = _cd

    # ── Step 3: Visual forming-candle from LTP (chart overlay only) ─────────
    if ltp is not None and ltp > 0:
        update_live_candle(ltp, now, interval_secs)


    # ── Step 4: Entry / Exit logic ────────────────────────────────────────────
    #    Runs with FRESH LTP (set in Step 2) — candle fetch comes AFTER this.
    _df_for_logic = st.session_state.get('indicator_df')
    if _df_for_logic is None or _df_for_logic.empty:
        add_log("⏳ Waiting for initial candle data…")
    else:
        df = _df_for_logic  # alias so entry/exit logic can reference 'df'
        idx           = len(_df_for_logic) - 1
        current_data  = st.session_state.get('current_data', _df_for_logic.iloc[idx].copy())
...
def _candle_refresh_logic(config):
    """
    Fetch candles from Dhan / yfinance on the interval schedule with stale detection.
    Called from _candle_fragment every second; does the API call only when the
    selected interval has elapsed (1m → every 60 s, 5m → every 300 s, etc.).
    Running this in a separate fragment ensures it never blocks the 0.3 s LTP display.
    """
    # ── Step 5: Candle fetch (throttled, runs LAST) ──────────────────────────
    # ── Resolve config variables (needed since this runs outside live_trading_iteration)
    position      = st.session_state.get('position')
    if position is not None and 'ticker' in position:
        ticker        = position['ticker']
        custom_ticker = position.get('custom_ticker')
    ...
    #    API candle fetch (throttled + stale-data detection) ────────
...
# ─── Fragment 1: METRICS ONLY — runs every 0.3 s ─────────────────────────────
# Nothing heavy here. Just reads numbers from session_state and shows them.
# Completes in < 20 ms every tick.
# ─────────────────────────────────────────────────────────────────────────────
@st.fragment(run_every=0.3)
def _metrics_fragment(config):
    if not st.session_state.get('trading_active', False):
        return

    # ── 1. Get LTP from background thread (microseconds) ─────────────────
    ltp = get_ltp_from_thread(config)
    if ltp and ltp > 0:
        st.session_state['_cached_ltp'] = ltp
    else:
        ltp = st.session_state.get('_cached_ltp', 0)
    current_price = float(ltp) if ltp else 0.0

    # ── 2. Update current_data price ──────────────────────────────────────
    df = st.session_state.get('indicator_df')
    if df is not None and not df.empty:
        cd = df.iloc[-1].copy()
        cd['Close'] = current_price
        st.session_state['current_data'] = cd

    # ── 3. Update visual forming candle ───────────────────────────────────
    if current_price > 0:
        now = datetime.now(pytz.timezone('Asia/Kolkata'))
        interval_secs = INTERVAL_FETCH_SECONDS.get(
            INTERVAL_MAPPING.get(config.get('interval', '1 day'), '1d'), 60)
        update_live_candle(current_price, now, interval_secs)

    # ── 4. SL / Target check (fast — no API, just comparisons) ───────────
    position = st.session_state.get('position')
    if position and current_price > 0:
        now = datetime.now(pytz.timezone('Asia/Kolkata'))
        _last_exit = st.session_state.get('last_exit_time')
        # Guard: skip if we just exited this same tick
        if not (_last_exit and (now - _last_exit).total_seconds() < 2):
            interval_secs2 = INTERVAL_FETCH_SECONDS.get(
                INTERVAL_MAPPING.get(config.get('interval', '1 day'), '1d'), 60)
            exit_reason = None
            exit_price  = current_price
            ...
            if sl:
                if position['type'] == 'LONG'  and current_price <= sl:
                    exit_reason, exit_price = 'SL Hit', sl
                elif position['type'] == 'SHORT' and current_price >= sl:
                    exit_reason, exit_price = 'SL Hit', sl

            if not exit_reason and tg:
                if position['type'] == 'LONG'  and current_price >= tg:
                    exit_reason, exit_price = 'Target Hit', tg
                elif position['type'] == 'SHORT' and current_price <= tg:
                    exit_reason, exit_price = 'Target Hit', tg

            if exit_reason:
                pnl = (exit_price - position['entry_price']) * position['quantity']                       if position['type'] == 'LONG'                       else (position['entry_price'] - exit_price) * position['quantity']
                brok    = calculate_brokerage(position['entry_price'], exit_price,
                                              position['quantity'], config)
                net_pnl = pnl - brok
                ...
                trade = {
                    ...
                    'price_change_pct': abs(exit_price - position['entry_price'])
                                        / position['entry_price'] * 100,
                    'duration_minutes': (now - position['entry_time']).total_seconds() / 60,
                    'price_range':  position.get('highest_price', exit_price)
                                    - position.get('lowest_price', exit_price),
                }
                st.session_state.setdefault('trade_history', []).append(trade)
                add_log(f"🚨 {exit_reason} @ ₹{exit_price:.2f}  P&L ₹{pnl:.2f}  Net ₹{net_pnl:.2f}")

                if config.get('dhan_enabled', False):
                    bp = st.session_state.get('broker_position')
                    db = st.session_state.get('dhan_broker')
                    if bp and db:
                        try:
                            db.exit_broker_position(bp, exit_price, exit_reason, add_log)
                        except Exception as ex:
                            add_log(f"🏦 ⚠️ Broker exit error: {ex}")
    ...
    # ── 5. Display metrics (the ONLY UI work this fragment does) ─────────
    cols = st.columns(6)
    ...
    with cols[0]:
        delta = None
        if pos:
            chg = current_price - pos['entry_price']
            delta = f"₹{chg:+.2f}"
        st.metric("💹 LTP", f"₹{current_price:.2f}", delta=delta)

    with cols[1]:
        if pos:
            pnl_live = (current_price - pos['entry_price']) * pos['quantity']                        if pos['type'] == 'LONG'                        else (pos['entry_price'] - current_price) * pos['quantity']
            st.metric("📈 P&L", f"₹{pnl_live:.2f}", delta=f"₹{pnl_live:.2f}")
        else:
            st.metric("📈 P&L", "—")

    with cols[2]:
        sl_val = pos.get('sl_price') if pos else None
        st.metric("🛑 SL", f"₹{sl_val:.2f}" if sl_val else "—")

    with cols[3]:
        tg_val = pos.get('target_price') if pos else None
        st.metric("🎯 Target", f"₹{tg_val:.2f}" if tg_val else "—")

    with cols[4]:
        hi = pos.get('highest_price') if pos else None
        st.metric("⬆ High", f"₹{hi:.2f}" if hi else "—")

    with cols[5]:
        lo = pos.get('lowest_price') if pos else None
        st.metric("⬇ Low", f"₹{lo:.2f}" if lo else "—")

    # Last 3 log lines — no heavy components
    logs = st.session_state.get('live_logs', [])
    if logs:
        st.caption("  |  ".join(logs[-3:]))


# ─── Fragment 2: CHART + DETAILS — runs every 5 s ─────────────────────────────
# Heavy chart is here. Updates every 5 s — no need faster for candle visuals.
# ─────────────────────────────────────────────────────────────────────────────
@st.fragment(run_every=5)
def _chart_fragment(config):
    if not st.session_state.get('trading_active', False):
        return
    ...
    current_price = st.session_state.get('_cached_ltp', float(df_plot.iloc[-1]['Close']))

    # Append live forming candle
    plot_df   = df_plot.tail(100).copy()
    ...
    if all(v is not None for v in [lc_start, lc_open, lc_high, lc_low, lc_close]):
        forming = pd.DataFrame([{
            'Datetime': lc_start, 'Open': float(lc_open),
            'High': float(lc_high), 'Low': float(lc_low), 'Close': float(lc_close)
        }])
        if len(plot_df) > 0 and str(plot_df.iloc[-1]['Datetime']) == str(lc_start):
            plot_df = pd.concat([plot_df.iloc[:-1], forming], ignore_index=True)
        else:
            plot_df = pd.concat([plot_df, forming], ignore_index=True)
    ...
    pos = st.session_state.get('position')
    if pos:
        fig.add_hline(y=pos['entry_price'], line_dash='dash', line_color='#00E676',
                      annotation_text=f"Entry ₹{pos['entry_price']:.2f}")
        if pos.get('sl_price'):
            fig.add_hline(y=pos['sl_price'], line_dash='dot', line_color='#FF1744',
                          annotation_text=f"SL ₹{pos['sl_price']:.2f}")
        if pos.get('target_price'):
            fig.add_hline(y=pos['target_price'], line_dash='dot', line_color='#00BCD4',
                          annotation_text=f"Target ₹{pos['target_price']:.2f}")

    fig.update_layout(xaxis_rangeslider_visible=False, height=380,
                      template='plotly_dark', margin=dict(l=0,r=60,t=30,b=0),
                      hovermode='x unified')
    st.plotly_chart(fig, width='stretch')
    ...
    if pos:
        ...
        c1.metric("Type",    pos['type'])
        c2.metric("Entry",   f"₹{pos['entry_price']:.2f}")
        c3.metric("Qty",     pos['quantity'])
        c4.metric("Strategy",pos.get('strategy',''))
    ...
    if hist:
        ...
        show = ['entry_time','exit_time','type','entry_price','exit_price','pnl','net_pnl','exit_reason']
        st.dataframe(df_h[[c for c in show if c in df_h.columns]],
                     width='content', height=160)

    # Full logs
    logs = st.session_state.get('live_logs', [])
    if logs:
        st.text_area("Trading Logs", label_visibility="collapsed",
                     value="\n".join(reversed(logs[-50:])),
                     height=150, disabled=True)


# ─── Legacy wrapper (called from render_live_trading_ui) ─────────────────────
def _live_trading_fragment(config):
    """Calls both fragments — kept for compatibility with render_live_trading_ui."""
    _metrics_fragment(config)
    _chart_fragment(config)


@st.fragment(run_every=1)
def _candle_fragment(config):
    """Runs every 1 s, fetches candles when interval elapsed, checks entry signals."""
    if not st.session_state.get('trading_active', False):
        return
    _candle_refresh_logic(config)

    # Entry signal check (only when we have data and no open position)
    ...
    config2 = st.session_state.get('config', config)
    interval = INTERVAL_MAPPING.get(config2.get('interval', '1 day'), '1d')
    interval_secs = INTERVAL_FETCH_SECONDS.get(interval, 60)
    now = datetime.now(pytz.timezone('Asia/Kolkata'))
    ...
    _last_exit = st.session_state.get('last_exit_time')
    if _last_exit:
        elapsed = (now - _last_exit).total_seconds()
        min_cool = max(interval_secs, config2.get('entry_cooldown_seconds', 0)
                       if config2.get('enable_entry_cooldown', False) else 0)
        if elapsed < min_cool:
            return

    if not is_within_trade_window(df.iloc[-1]['Datetime'], config2):
        return

    strategy_name = config2.get('strategy', 'EMA Crossover')
    strategy_func = STRATEGY_FUNCTIONS.get(strategy_name, check_ema_crossover_strategy)
    idx = len(df) - 1
    signal, entry_price = strategy_func(df, idx, config2, None)

    if signal and should_allow_trade_direction(signal, config2):
        current_data = st.session_state.get('current_data', df.iloc[idx].copy())
        current_price = float(current_data['Close'])
        entry_price = entry_price or current_price
        position_type = 'LONG' if signal in ('BUY', 'LONG') else 'SHORT'

        sl_price     = calculate_initial_sl(position_type, entry_price, df, idx, config2)
        target_price = calculate_initial_target(position_type, entry_price, df, idx, config2)
        ticker       = config2.get('asset', 'NIFTY 50')
        custom_ticker = config2.get('custom_ticker', None)

        position = {
            'type': position_type, 'entry_price': entry_price,
            'entry_time': now, 'sl_price': sl_price, 'target_price': target_price,
            'highest_price': entry_price, 'lowest_price': entry_price,
            'quantity': config2.get('quantity', 1),
            'ticker': ticker, 'custom_ticker': custom_ticker,
            'strategy': strategy_name,
            'ema_fast_period': config2.get('ema_fast', 9),
            'ema_slow_period': config2.get('ema_slow', 21),
            'ema_fast_entry': current_data.get('EMA_Fast'),
            'ema_slow_entry': current_data.get('EMA_Slow'),
            'ema_angle_entry': current_data.get('EMA_Fast_Angle'),
        }
        ...
        if config2.get('dhan_enabled', False):
            dhan_broker = st.session_state.get('dhan_broker')
            if dhan_broker:
                try:
                    if config2.get('clear_positions_before_entry', False):
                        st.session_state['clearing_in_progress'] = True
                        dhan_broker.clear_all_positions(add_log)
                        st.session_state['clearing_in_progress'] = False
                    bp = dhan_broker.enter_broker_position(signal, entry_price, config2, add_log)
                    st.session_state['broker_position'] = bp
                except Exception as ex:
                    add_log(f"🏦 ⚠️ {ex}")
