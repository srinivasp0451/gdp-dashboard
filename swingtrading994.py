...
def tab_optimization(cfg):
    ...
    if run_opt or run_all:
        ...
        _all_strat_results = {}; _fallback_results = {}
        data_cache = {}
        ...
        for _strat_idx, _cur_strat in enumerate(_strats_to_run):
            ...
            if _deep:
                _nt = 999999  # exhaustive — stops only when all combos tested
            elif _all_strats_mode:
                _nt = max(int(n_trials)//max(len(_strats_to_run),1), 8)
            else:
                _nt = int(n_trials)
            results = []; _fallback_rows = []; tested = set(); trial = 0
            ...
            while trial < min(_nt, max_combos):
                ...
                try:
                    ...
                    bt = run_backtest(df, sig, sl_type, actual_sl_p, tgt_type, actual_tgt_p,
                                      1, _cur_strat, conf)
                    # Always track results for fallback (ignore min_trades here)
                    if bt.empty: continue
                    _is_good_sample = len(bt) >= int(min_trades)
                    s = compute_stats(bt)
                    if not s: continue
                    ...
                    if _is_good_sample:
                        results.append(_row)  # meets min_trades
                    else:
                        _fallback_rows.append(_row)  # below min_trades but keep as fallback
                except Exception: continue
            _all_rows = results if results else _fallback_rows
            if _all_rows:
                _df = pd.DataFrame(_all_rows).sort_values("Score",ascending=False
                      ).reset_index(drop=True)
                _df.index += 1
                _all_strat_results[_cur_strat] = _df
            if "_fallback_results" not in dir():
                _fallback_results = {}
            _fallback_results[_cur_strat] = pd.DataFrame(_fallback_rows) if _fallback_rows else pd.DataFrame()

        prog.empty(); status.empty()
        # If Accuracy/Expectancy mode produced no results meeting min_trades,
        # fall back to ALL attempted combinations (ignore min_trades filter)
        # so the user always sees SOMETHING useful rather than a blank error.
        if not _all_strat_results:
            st.warning(
                f"⚠️ No combination met the min {min_trades} trades threshold with current settings. "
                f"Showing best results ignoring min-trade filter — treat with caution (small sample).")
            # Re-run without the min_trades gate
            for _sname, _sdf_raw in _fallback_results.items():
                if not _sdf_raw.empty:
                    _all_strat_results[_sname] = _sdf_raw
        if not _all_strat_results:
            st.error("No combinations produced any trades. Try wider timeframes or different strategies.")
            return
