...
def live_trading_loop(config):
    ...
    # Create persistent containers that don't flicker
    ...
    while st.session_state.get('trading_active', False):
        ...
        # Only update every 2-3 seconds to reduce flickering
        current_loop_time = time.time()
        if last_update_time and (current_loop_time - last_update_time) < 2:
            time.sleep(0.5)
            continue
