"""QuantAlgo Pro v4 — Auto-trading, no-button-load, 1s refresh"""
...
.mv{font-size:1.1rem;font-weight:900;color:#e6edf3}
.ms{font-size:.72rem;color:#8b949e}
.sbuy{background:rgba(63,185,80,.15);border:2px solid #3fb950;border-radius:8px;padding:10px;text-align:center;margin:4px 0}
.ssell{background:rgba(248,81,73,.15);border:2px solid #f85149;border-radius:8px;padding:10px;text-align:center;margin:4px 0}
.swait{background:rgba(139,148,158,.1);border:1px solid #30363d;border-radius:8px;padding:10px;text-align:center;margin:4px 0}
.pos-long{background:rgba(63,185,80,.1);border:1px solid #3fb950;border-radius:8px;padding:12px;margin:4px 0}
.pos-short{background:rgba(248,81,73,.1);border:1px solid #f85149;border-radius:8px;padding:12px;margin:4px 0}
...
.auto-on{background:rgba(63,185,80,.12);border:2px solid #3fb950;border-radius:8px;padding:10px;text-align:center}
...
# Strategies — Simple Buy and Simple Sell are SEPARATE and enter IMMEDIATELY at LTP
# All other strategies enter at NEXT candle open (N+1) in backtest / LTP at signal in live
STRATEGIES = [
    "EMA Crossover",
    "Simple Buy (Immediate)",    # enters at LTP instantly — no next-candle wait
    "Simple Sell (Immediate)",   # enters at LTP instantly — no next-candle wait
...
# Strategies where entry is immediate (no next-candle delay in backtest either)
IMMEDIATE_ENTRY_STRATEGIES = {"Simple Buy (Immediate)", "Simple Sell (Immediate)"}
# Simple strategies are manual only in live — auto-trading skips them
MANUAL_ONLY_STRATEGIES = {"Simple Buy (Immediate)", "Simple Sell (Immediate)"}
...
# ── SESSION STATE ─────────────────────────────────────────────
_DEFS = {
    "backtest_results": None,     # backtest-only
    "live_trades": [],            # live-only
    "live_position": None,        # open position dict
    "live_running": False,        # auto-trade loop active
    "current_data": None,
    "signals": None,
    "indicators": {},
    "last_data_key": "",          # tracks which ticker/TF was last loaded
    "last_strat_key": "",         # tracks which strategy was last run
    "last_signal_candle": None,   # prevents re-entering same signal twice
...
def _kalman(prices):
    a=prices.values; xk=np.zeros(len(a)); pk=np.ones(len(a)); Q,R=1e-5,0.01; xk[0]=a[0]
...
def _vpfr_compute(df,lb=50,n_bins=30):
    ...
    if df["Volume"].sum()==0: return pd.Series(sigs,index=df.index),{"no_volume":True}
    for i in range(lb,n):
...
# ── DATA FETCHING  (0.3 s API rate-limit guard per real call) ─
...
@st.cache_data(ttl=1, show_spinner=False)   # ← 1s LTP cache
...
def auto_load(tsym, interval, period, strat, sp):
    """Auto-fetch data on ticker/TF change; re-run strategy on strat change."""
    data_key  = f"{tsym}|{interval}|{period}"
    strat_key = f"{data_key}|{strat}|{sorted(sp.items())}"
    df = st.session_state.current_data

    # Re-fetch when instrument or timeframe changes
    if data_key != st.session_state.last_data_key:
        with st.spinner(f"Loading {tsym} {interval}/{period}…"):
            df = fetch_data(tsym, interval, period)
        if df is not None and not df.empty:
            st.session_state.current_data   = df
            st.session_state.last_data_key  = data_key
            st.session_state.last_strat_key = ""   # force strategy re-run
            st.session_state.backtest_results = None
        else:
            st.sidebar.error("❌ No data — check ticker")
            return

    # Re-run strategy when strategy or params change
    df = st.session_state.current_data
...
def run_strategy(df, strat, p) -> Tuple[pd.Series, dict]:
    ...
    try:
        if strat=="EMA Crossover":
        ...
        elif strat=="Simple Buy (Immediate)":
            # Always signals BUY on last candle — live entry is immediate at LTP
            sig.iloc[-1]=1
        elif strat=="Simple Sell (Immediate)":
            # Always signals SELL on last candle — live entry is immediate at LTP
            sig.iloc[-1]=-1
            ...
            if ind.get("no_volume"):
                st.warning("⚠️ No volume — VPFR needs volume data. Use equities/crypto, not index symbols.")
...
def strategy_proximity(df, strat, sp, sigs):
    ...
    if last_sig==1:  return "signal","🟢 BUY SIGNAL ACTIVE","Auto-enter will fire (or click BUY NOW)"
    if last_sig==-1: return "signal","🔴 SELL SIGNAL ACTIVE","Auto-enter will fire (or click SELL NOW)"
    ...
    try:
        if strat=="EMA Crossover":
            ...
            if pct<0.25: return "approaching",f"⚠️ APPROACHING ({bias})",f"Gap {gap:.2f} ({pct:.3f}%) — {conv}"
            return "far",f"⏳ FAR ({bias} bias)",f"EMA gap {gap:.2f} ({pct:.2f}%) — {conv}"
        elif strat=="RSI Strategy":
            ...
            sq=" ⚡SQUEEZE" if bw<=bw_s*1.05 else ""
            if closest<0.2: return "near",f"⚡ AT BAND{sq}",f"Upper {uv:.2f} Lower {lv:.2f}"
            ...
            if abs(z)>=thr*0.85: return "near","⚡ Z AT EDGE",f"Z={z:.3f} (±{thr})"
            if abs(z)>=thr*0.65: return "approaching","⚠️ Z BUILDING",f"Z={z:.3f} (±{thr})"
...
def exit_position(exit_price, exit_reason, cfg):
    ...
    st.session_state.last_signal_candle=None  # reset so next signal can fire
    clear_all_cache()   # ← fresh data after every exit

# ── BACKTEST ENGINE ───────────────────────────────────────────
def run_backtest(df, signals, sl_type, sl_p, tgt_type, tgt_p, qty, strat_name=""):
    """
    Signal on candle N → entry at candle N+1 OPEN (standard strategies).
    IMMEDIATE strategies (Simple Buy/Sell) → entry at candle N CLOSE (same candle).
    LONG: SL via Low first → Target via High.  SHORT: SL via High first → Target via Low.
    """
    immediate = strat_name in IMMEDIATE_ENTRY_STRATEGIES
    ...
    for i in range(1,len(df)):
        if not in_trade:
            ...
            if sv!=0:
                if immediate:
                    entry=float(df["Close"].iloc[i-1])   # same-candle close
                    check_start=i                          # check from this candle onward
                else:
                    entry=float(df["Open"].iloc[i])       # next-candle open
                    check_start=i
                ...
                trade=dict(entry_idx=check_start,entry_dt=df.index[i-1 if immediate else i],
...
def build_chart(df,sig,ind,trades_df=None,title="",chart_key="c0"):
    has_rsi="rsi" in ind; rows=3 if has_rsi else 2; rh=[0.60,0.20,0.20] if has_rsi else [0.72,0.28]
    ...
    for k,v in ind.items():
        ...
        if any(x in k for x in("ema","kalman","bb","vwap","atr_line","vpfr")):
            ...
            fig.add_trace(go.Scatter(x=df.index,y=v,name=k.replace("_"," ").title(),
                line=dict(color=_CMAP.get(k,"#aaa"),width=1 if "vpfr" in k else 1.5,dash=dash),
    ...
    for s in ind.get("support_levels",[]): fig.add_hline(y=s,line=dict(color="#26a69a",dash="dot",width=1),opacity=0.35,row=1,col=1)
    for r in ind.get("resistance_levels",[]): fig.add_hline(y=r,line=dict(color="#ef5350",dash="dot",width=1),opacity=0.35,row=1,col=1)
    for k,col_ in[("pivot_high","#ef5350"),("pivot_low","#26a69a")]:
        ...
        if not pts.empty:
            ...
            fig.add_trace(go.Scatter(x=pts.index,y=prices.values,mode="markers",
                marker=dict(symbol="circle",size=5,color=col_,opacity=0.6),showlegend=False),row=1,col=1)
    buys=sig[sig==1].index; sells=sig[sig==-1].index
    if len(buys): fig.add_trace(go.Scatter(x=buys,y=df["Low"].reindex(buys)*0.999,mode="markers",
        name="Buy",marker=dict(symbol="triangle-up",size=11,color="#3fb950",line=dict(color="white",width=1))),row=1,col=1)
    if len(sells): fig.add_trace(go.Scatter(x=sells,y=df["High"].reindex(sells)*1.001,mode="markers",
        name="Sell",marker=dict(symbol="triangle-down",size=11,color="#f85149",line=dict(color="white",width=1))),row=1,col=1)
    if trades_df is not None and not trades_df.empty:
        for _,t in trades_df.iterrows():
            ...
            fig.add_trace(go.Scatter(x=[t["Entry Date"]],y=[t["Entry Price"]],
                ...
                textfont=dict(size=7,color=c_),marker=dict(size=7,color=c_),showlegend=False),row=1,col=1)
            try:
    ...
    fig.add_trace(go.Bar(x=df.index,y=df["Volume"],marker_color=vcol,opacity=0.6,name="Volume"),row=vrow,col=1)
    ...
    fig.update_layout(template="plotly_dark",paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
        ...
        xaxis_rangeslider_visible=False,margin=dict(l=0,r=0,t=36,b=0),height=600,
...
def eq_fig(bt):
    ...
    fig.update_layout(template="plotly_dark",paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
        title="Equity Curve",height=240,margin=dict(l=0,r=0,t=28,b=0),font=dict(color="#8b949e"))
    return fig

# ── AUTO-TRADE FRAGMENT  (runs every 1 second) ────────────────
@st.fragment(run_every=1)
def auto_trade_fragment(cfg):
    """
    THE BRAIN of live trading.
    - Runs every 1 second via @st.fragment(run_every=1).
    - When live_running=True:  auto-monitors signals, auto-enters on new signal,
      auto-exits on SL/Target hit.
    - When live_running=False: only displays current position P&L (no action taken).
    - Simple Buy/Sell strategies are MANUAL ONLY — auto-entry skipped for them.
    """
    ticker=cfg["tsym"]; strat=cfg["strat"]; sp=cfg["sp"]
    running=st.session_state.live_running
    pos=st.session_state.live_position

    # ── Fetch latest data every 1s ───────────────────────────
    ...
    # ── STATUS BAR ───────────────────────────────────────────
    status_clr="#3fb950" if running else "#8b949e"
    status_txt="● LIVE AUTO-TRADE ACTIVE" if running else "○ MONITOR STOPPED"
    auto_label="✅ Auto-entry: ON" if running and strat not in MANUAL_ONLY_STRATEGIES else \
               ("⚠️ Manual-only strategy" if strat in MANUAL_ONLY_STRATEGIES else "Auto-entry: OFF")
    st.markdown(f"""<div style='display:flex;gap:16px;align-items:center;background:#161b22;
         border:1px solid #30363d;border-radius:8px;padding:8px 14px;margin-bottom:8px;font-size:.83rem'>
      ...
      <span style='color:#8b949e'>|</span>
      <span style='color:#8b949e'>{auto_label}</span>
      <span style='color:#8b949e;margin-left:auto;font-size:.7rem'>{datetime.now().strftime("%H:%M:%S")}</span>
    </div>""",unsafe_allow_html=True)

    # ── AUTO-ENTER LOGIC (only when running and no position) ─
    if running and pos is None and strat not in MANUAL_ONLY_STRATEGIES:
        is_new_candle = cur_candle != st.session_state.last_signal_candle
        if last_sig != 0 and is_new_candle:
            # Auto-enter position
            ...
            direction_txt="🟢 LONG" if last_sig==1 else "🔴 SHORT"
            st.success(f"🤖 AUTO-ENTERED {direction_txt} @ {pos_new['entry_price']:.2f} | SL {pos_new['sl']:.2f} | Tgt {pos_new['target']:.2f if pos_new.get('target') else '—'}")
            pos=st.session_state.live_position   # update local ref

    # ── AUTO-EXIT LOGIC (check SL/Target every second) ───────
    if running and pos is not None and ltp > 0:
        d=pos["direction"]
        exited=False
        # SL check
        if pos.get("sl"):
            if (d==1 and ltp<=pos["sl"]) or (d==-1 and ltp>=pos["sl"]):
                ...
                st.error(f"🛑 SL HIT — exited @ {pos['sl']:.2f} | LTP {ltp:.2f}")
                exited=True
        # Target check
        if not exited and pos.get("target") and "display only" not in cfg.get("tgt_type",""):
            if (d==1 and ltp>=pos["target"]) or (d==-1 and ltp<=pos["target"]):
                ...
                st.success(f"🎯 TARGET HIT — exited @ {pos['target']:.2f} | LTP {ltp:.2f}")
                exited=True
        if exited:
            pos=None   # update local ref

    # ── POSITION P&L PANEL ───────────────────────────────────
    pos=st.session_state.live_position   # re-read after potential exit
    if pos:
        ...
        st.markdown(f"""<div class='{"pos-long" if d==1 else "pos-short"}'>
          <div style='display:flex;flex-wrap:wrap;gap:18px;align-items:center'>
            ...
            <div><div class='ml'>LTP NOW</div><div class='mv' style='color:{pnl_clr}'>{ltp:,.2f}</div></div>
            <div><div class='ml'>STOP LOSS</div><div class='mv' style='color:#f85149'>{pos['sl']:,.2f}</div></div>
            ...
            <div><div class='ml'>OPEN P&L</div>
              <div style='color:{pnl_clr};font-weight:900;font-size:1.3rem'>
            ...
            <div><div class='ml'>DAY HI/LO</div>
              <div style='font-size:.8rem;color:#8b949e'>{day_hi:,.2f} / {day_lo:,.2f}</div></div>
            ...
            for col,(nm,val,clr) in zip(st.columns(len(chunk)),chunk):
                with col:
                    st.markdown(f"""<div class='mc'><div class='ml'>{nm}</div>
                      <div class='mv' style='color:{clr};font-size:.88rem'>{val:,.2f}</div></div>""",
    ...
    if "ema_fast" in inds and "ema_slow" in inds and len(inds["ema_fast"])>=2:
        ...
        if ef_v>es_v and ef_p<=es_p:
            st.markdown("<div class='sbuy' style='padding:7px'><b style='color:#3fb950'>▲ BULLISH CROSSOVER — EMA Fast crossed ABOVE EMA Slow!</b></div>",unsafe_allow_html=True)
        elif ef_v<es_v and ef_p>=es_p:
            st.markdown("<div class='ssell' style='padding:7px'><b style='color:#f85149'>▼ BEARISH CROSSOVER — EMA Fast crossed BELOW EMA Slow!</b></div>",unsafe_allow_html=True)
        else:
            bias="Bullish" if ef_v>es_v else "Bearish"; gap=abs(ef_v-es_v)
            bclr="#3fb950" if bias=="Bullish" else "#f85149"
            st.markdown(f"""<div class='swait' style='padding:6px;font-size:.82rem'>
              <span style='color:{bclr}'>No crossover · {bias} bias</span> · Gap: <b>{gap:.2f}</b></div>""",
              unsafe_allow_html=True)

    # ── LIVE CHART ────────────────────────────────────────────
    fig=build_chart(df,sigs,inds,title=f"LIVE {ticker} | {strat} | {cfg['interval']}/{cfg['period']}",chart_key="af")
    # Right-edge EMA label annotations
    for k,nm_,clr in[("ema_fast",f"EMA {sp.get('ema_fast',9)}","#f0883e"),
        ...
        if k in inds and isinstance(inds[k],pd.Series):
            ...
            fig.add_annotation(x=df.index[-1],y=v,text=f"  {nm_}: {v:,.2f}",
                showarrow=False,xanchor="left",font=dict(size=9,color=clr),
                bgcolor="rgba(13,17,23,.75)")
    st.plotly_chart(fig,use_container_width=True,key=f"atf_{ticker}_{cfg['interval']}")

# ── STATIC POSITION DISPLAY (shown when monitor is STOPPED) ───
def static_position_display():
    pos = st.session_state.live_position
    if pos:
        d = pos["direction"]
        st.markdown(f"""<div class='{"pos-long" if d==1 else "pos-short"}'>
          <div style='display:flex;flex-wrap:wrap;gap:18px;align-items:center'>
            ...
            <div><div class='ml'>TARGET</div>
              <div class='mv' style='color:#3fb950'>{"%.2f" % pos["target"] if pos.get("target") else "—"}</div></div>
            <div><div class='ml'>QTY</div><div class='mv'>{pos['qty']}</div></div>
            <div><div class='ml'>ENTRY TIME</div>
              <div class='ms'>{pos['entry_dt'].strftime('%H:%M:%S')}</div></div>
          </div>
          <div style='margin-top:8px;color:#f0883e;font-size:.8rem'>
            ⚠️ Monitor STOPPED — P&L not updating. Click START to resume live monitoring.</div>
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""<div class='pos-none'>
          No open position · Monitor stopped · Click ▶ START to begin live trading
        </div>""", unsafe_allow_html=True)
...
def sidebar() -> dict:
    with st.sidebar:
        st.markdown("""<div style='text-align:center;padding:8px 0 14px'>
          ...
          <div style='font-size:.6rem;color:#8b949e;letter-spacing:2px'>JANE STREET INSPIRED · v4.0</div>
        </div>""", unsafe_allow_html=True)
        st.divider()
        ...
        tsym = (st.text_input("Symbol", "RELIANCE.NS",
                               ...
                               key="custom_sym").upper().strip()
                if tname == "✏️ Custom" else TICKERS[tname])

        # ── Timeframe  (default 1m / 5d) ────────────────────
        st.markdown("**⏱️ TIMEFRAME**")
        c1, c2 = st.columns(2)
        all_ivs = list(TIMEFRAME_PERIODS.keys())
        with c1:
            interval = st.selectbox("Interval", all_ivs,
                                     index=all_ivs.index("1m"),
                                     key="interval", label_visibility="collapsed")
        periods_list = TIMEFRAME_PERIODS[interval]
        def_p = "5d" if "5d" in periods_list else periods_list[0]
        with c2:
            period = st.selectbox("Period", periods_list,
                                   index=periods_list.index(def_p),
        ...
        strat = st.selectbox("Strategy", STRATEGIES, key="strat",
        ...
        with st.expander("⚙️ Strategy Params"):
            if strat == "EMA Crossover":
                ...
                st.caption(f"{'▲ BUY' if 'Buy' in strat else '▼ SELL'} — enters at LTP instantly. No next-candle delay.")
            elif strat == "Threshold Price Cross":
        ...
        with st.expander("⚙️ Target Params"):
            if tgt_type == "Custom Points":
                ...
                st.caption("📊 Shown on chart — never triggers exit")
            elif tgt_type == "🤖 Autopilot Target":
        ...
        # ── LOAD DATA button ─────────────────────────────────
        # ← Data only fetched when this button is clicked.
        # ← Strategy re-runs only on Load click or when strat params change.
        if st.button("📥 LOAD DATA", use_container_width=True, key="load_btn"):
            ...
            if df is not None and not df.empty:
                ...
                st.session_state.indicators       = inds
                st.session_state.backtest_results = None
                st.session_state.last_data_key    = f"{tsym}|{interval}|{period}"
        ...
        if st.session_state.get("last_data_key"):
            st.caption(f"Loaded: {st.session_state.last_data_key}")
...
# ── TAB 1 — BACKTEST ──────────────────────────────────────────
# Only runs when "▶ Run Backtest" button is clicked. Nothing executes otherwise.
def tab_backtest(cfg):
    ...
    if df is None or df.empty:
        st.info("👈 Configure settings in the sidebar → click **📥 LOAD DATA** first.")
        return
    ...
    with ci:
        ...
        entry_note = ("Immediate entry (signal candle close)"
                      if cfg["strat"] in IMMEDIATE_ENTRY_STRATEGIES
                      else "Standard entry (next candle open)")
        st.markdown(f"""<div style='background:#161b22;border:1px solid #30363d;border-radius:6px;
             padding:9px 14px;font-size:.82rem;display:flex;gap:14px;align-items:center'>
          <span style='color:#8b949e'>{len(df):,} candles</span>
          <span style='color:#3fb950'>▲ {bc} BUY</span>
          <span style='color:#f85149'>▼ {sc} SELL</span>
          <span style='color:#8b949e'>| {cfg["strat"][:28]}</span>
          <span style='color:#58a6ff;font-size:.75rem'>· {entry_note}</span>
        </div>""", unsafe_allow_html=True)

    # Only run backtest on button click — nothing else triggers this
    if run_bt:
        with st.spinner("Backtesting…"):
            bt = run_backtest(df, sigs, cfg["sl_type"], cfg["sl_p"],
                              cfg["tgt_type"], cfg["tgt_p"],
                              cfg["qty"], cfg["strat"])
        st.session_state.backtest_results = bt

    # Chart — always show loaded data, signals, and (if run) trade markers
    ...
    if bt_res is None:
        st.caption("Click ▶ Run Backtest to see results.")
        return
    if bt_res.empty:
    ...
    for col, lbl, val, clr in zip(cols,
        ...
        with col:
            st.markdown(f"""<div class='mc' style='border-top:3px solid {clr}'>
              ...
              <div class='mv' style='color:{clr};font-size:.98rem'>{val}</div>
    ...
    with ca:
        st.plotly_chart(eq_fig(bt_res), use_container_width=True, key="bt_eq")
    with cb:
    ...
    st.markdown("**📋 Trade Log (Backtest only — isolated from live)**")
    def _hl(row):
...
# ── TAB 2 — LIVE TRADING ──────────────────────────────────────
# auto_trade_fragment runs every 1 s ONLY when live_running=True.
# When stopped, only static_position_display() is shown — zero API calls.
def tab_live(cfg):
    df   = st.session_state.current_data
    sigs = st.session_state.signals
    pos  = st.session_state.live_position
    ...
    for col,(lbl,val,sub) in zip([c1,c2,c3,c4],[
        ...
        ("Risk",     cfg["sl_type"][:24], cfg["tgt_type"][:24]),
    ]):
        with col:
            st.markdown(f"""<div class='icard'><div class='ml'>{lbl}</div>
              <div style='color:#e6edf3;font-size:.87rem;font-weight:700;margin-top:3px'>{val}</div>
              <div class='ms'>{sub}</div></div>""", unsafe_allow_html=True)
    st.divider()

    if df is None or df.empty:
        st.info("👈 Click **📥 LOAD DATA** in the sidebar first.")
        return

    # ── START / STOP / SQUARE OFF ──────────────────────────
    ...
    with b1:
        if not running:
            ...
            st.markdown("""<div class='auto-on'>
              <div style='color:#3fb950;font-weight:900;font-size:.9rem'>● LIVE — RUNNING</div>
              <div style='color:#8b949e;font-size:.72rem'>Auto-monitoring signals</div>
            </div>""", unsafe_allow_html=True)
    ...
    with b2:
        if running:
            ...
            st.button("⏹ STOP", use_container_width=True, disabled=True, key="stop_dis")

    with b3:
        if pos:
            if st.button("🔴 SQUARE OFF", use_container_width=True, key="sq_btn"):
                ltp_now,_,_ = fetch_ltp(cfg["tsym"])
                ep = ltp_now if ltp_now > 0 else pos["entry_price"]
                exit_position(ep, "Manual Square Off", cfg)
                st.success(f"✅ Squared off at {ep:.2f}")
                st.rerun()
        else:
            st.button("🔴 SQUARE OFF", use_container_width=True,
                      disabled=True, key="sq_dis")

    with b4:
        sc = "#3fb950" if running else "#8b949e"
        pos_txt = (f"{'LONG' if pos['direction']==1 else 'SHORT'} @ {pos['entry_price']:.2f}"
        ...
        st.markdown(f"""<div class='icard' style='text-align:center'>
          <div style='color:{sc};font-weight:700;font-size:.82rem'>
            {"● LIVE" if running else "○ IDLE"}</div>
          <div style='color:#e6edf3;font-size:.78rem;margin-top:3px'>{pos_txt}</div>
        </div>""", unsafe_allow_html=True)
    st.divider()

    # ── MANUAL BUY / SELL (immediate entry, always available) ─
    st.markdown("### ⚡ Manual Instant Entry")
    ltp_cur,_,_ = fetch_ltp(cfg["tsym"])
    cur = ltp_cur if ltp_cur > 0 else float(df["Close"].iloc[-1])
    ...
    with mb:
        st.markdown(f"""<div style='background:#1a3d26;border:1px solid #3fb950;border-radius:8px;
          padding:6px;margin-bottom:5px;text-align:center;font-size:.76rem'>
          <b style='color:#3fb950'>▲ BUY LONG</b><br>
          <span style='color:#8b949e'>Entry {cur:,.2f} | SL {psl_b:,.2f} | T {"%.2f"%ptgt_b if ptgt_b else "—"} | R:R {rr_b:.1f}:1</span>
        </div>""", unsafe_allow_html=True)
        if st.button("▲ BUY NOW", use_container_width=True, key="buy_now",
                     disabled=(pos is not None)):
            st.session_state.live_position = enter_position(1, cfg, df)
            clear_all_cache()
            st.rerun()
    with ms_:
        st.markdown(f"""<div style='background:#3d1a1a;border:1px solid #f85149;border-radius:8px;
          padding:6px;margin-bottom:5px;text-align:center;font-size:.76rem'>
          <b style='color:#f85149'>▼ SELL SHORT</b><br>
          <span style='color:#8b949e'>Entry {cur:,.2f} | SL {psl_s:,.2f} | T {"%.2f"%ptgt_s if ptgt_s else "—"} | R:R {rr_s:.1f}:1</span>
        </div>""", unsafe_allow_html=True)
        if st.button("▼ SELL NOW", use_container_width=True, key="sell_now",
            ...
            st.rerun()
    with mi:
        dis = "⚠️ Close current position first" if pos else "✅ Ready to trade"
        dis_c = "#f0883e" if pos else "#3fb950"
        st.markdown(f"""<div class='icard'>
          <div style='color:{dis_c};font-size:.79rem;font-weight:700;margin-bottom:6px'>{dis}</div>
          <div style='color:#8b949e;font-size:.77rem;line-height:1.8'>
            LTP: <b style='color:#e6edf3'>{cur:,.2f}</b><br>
            ATR(14): <b style='color:#80cbc4'>{a_v:.2f}</b> &nbsp;|&nbsp;
            Qty: <b style='color:#e6edf3'>{cfg['qty']}</b>
          </div></div>""", unsafe_allow_html=True)
    st.divider()
    ...
    with sc1:
        if last_sig == 1:
            st.markdown("""<div class='sbuy'><div style='font-size:1.4rem'>▲</div>
              <div style='font-size:1rem;font-weight:900;color:#3fb950'>BUY SIGNAL</div>
              <div style='color:#8b949e;font-size:.77rem'>Auto-enters when monitor ON</div>
            </div>""", unsafe_allow_html=True)
        elif last_sig == -1:
            st.markdown("""<div class='ssell'><div style='font-size:1.4rem'>▼</div>
              <div style='font-size:1rem;font-weight:900;color:#f85149'>SELL SIGNAL</div>
              <div style='color:#8b949e;font-size:.77rem'>Auto-enters when monitor ON</div>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""<div class='swait'><div style='font-size:1.4rem'>⏳</div>
              <div style='font-size:1rem;font-weight:900;color:#8b949e'>NO SIGNAL</div>
              <div style='color:#8b949e;font-size:.77rem'>Watching for setup</div>
            </div>""", unsafe_allow_html=True)
    with sc2:
        cm = {"signal":"#3fb950","near":"#f0883e","approaching":"#ffe082","far":"#8b949e"}
        bm = {"signal":"rgba(63,185,80,.1)","near":"rgba(240,136,62,.1)",
              "approaching":"rgba(255,235,130,.06)","far":"rgba(139,148,158,.05)"}
        bdr = cm.get(prx_lvl,"#8b949e"); bg_ = bm.get(prx_lvl,"transparent")
        st.markdown(f"""<div style='background:{bg_};border:1px solid {bdr};
             border-left:4px solid {bdr};border-radius:8px;padding:12px'>
          <div style='color:{bdr};font-weight:700;font-size:.88rem'>{prx_h}</div>
          <div style='color:#c9d1d9;font-size:.8rem;margin-top:5px'>{prx_d}</div>
        </div>""", unsafe_allow_html=True)
    st.divider()

    # ── LIVE TRADING ENGINE ────────────────────────────────────
    # Fragment runs every 1 s ONLY when live_running=True.
    # When stopped → static display only (zero API calls, zero CPU waste).
    st.markdown("### 📊 Live Monitor & P&L")
    if running:
        auto_trade_fragment(cfg)
    else:
        static_position_display()
        # Show last known chart (no API call — uses session state data)
        if df is not None and sigs is not None:
            st.markdown("### 📈 Last Loaded Chart")
            inds = st.session_state.indicators
            fig = build_chart(df, sigs, inds,
                              title=f"{cfg['tname']} | {cfg['strat']} (snapshot — click START to go live)",
                              chart_key="live_snap")
            st.plotly_chart(fig, use_container_width=True, key="live_snap_chart")
...
def tab_history():
    ...
    if not live:
        ...
        st.markdown("""<div class='icard' style='margin-top:12px'>
          <div style='color:#58a6ff;font-weight:700;margin-bottom:6px'>ℹ️ How it works</div>
          <div style='color:#8b949e;font-size:.82rem;line-height:1.9'>
            • Click ▶ START in Live Trading → strategy auto-enters on signals<br>
            • Or click <b style='color:#3fb950'>▲ BUY NOW</b> / <b style='color:#f85149'>▼ SELL NOW</b> for manual immediate entry<br>
            • Trades close via SL hit, Target hit, or Square Off button (all automatic)<br>
            • Completed trades appear here — <b style='color:#e6edf3'>100% isolated from backtest results</b><br>
            • Cache is cleared on every exit for clean fresh data
          </div></div>""", unsafe_allow_html=True)
        return

    df_h = pd.DataFrame(live); s = compute_stats(df_h)
    pf_txt = f"{s['pf']:.2f}" if s["pf"] != float("inf") else "∞"
    ...
    with ca:
        ...
        bar.update_layout(template="plotly_dark", paper_bgcolor="#0d1117",
            plot_bgcolor="#0d1117", height=240, title="P&L Per Trade",
            margin=dict(l=0,r=0,t=28,b=0), font=dict(color="#8b949e"))
        st.plotly_chart(bar, use_container_width=True, key="hist_bar")
    with cb:
        st.plotly_chart(eq_fig(df_h), use_container_width=True, key="hist_eq")
    ...
    st.dataframe(filt.style.apply(_hl, axis=1),
                 use_container_width=True, height=380, key="hist_df")
    dl_c, clr_c = st.columns(2)
    with dl_c:
...
# ── RECOMMENDATIONS ───────────────────────────────────────────
def recommendations():
    with st.expander("💡 Strategy Guide & Risk Rules", expanded=False):
        st.markdown("""<div class='icard'>
<h4 style='color:#58a6ff'>How the App Works</h4>
<div style='color:#8b949e;font-size:.82rem;line-height:2'>
  1. <b style='color:#e6edf3'>Sidebar</b>: Pick instrument, timeframe, strategy, SL/Target → click <b>📥 LOAD DATA</b><br>
  2. <b style='color:#e6edf3'>Backtest tab</b>: Click <b>▶ Run Backtest</b> — nothing runs until you click<br>
  3. <b style='color:#e6edf3'>Live tab → ▶ START</b>: Auto-trade loop activates (1s refresh).
     Auto-enters on signal, auto-exits on SL/Target. Click <b>⏹ STOP</b> to freeze.<br>
  4. <b style='color:#e6edf3'>Manual BUY NOW / SELL NOW</b>: Immediate entry at LTP — no waiting<br>
  5. <b style='color:#e6edf3'>🔴 SQUARE OFF</b>: Closes position at LTP immediately<br>
  6. <b style='color:#e6edf3'>Simple Buy/Sell strategies</b>: Manual-only (auto-trading skips them)
</div>
<h4 style='color:#58a6ff;margin-top:14px'>Top Strategy Combos</h4>
<table style='width:100%;border-collapse:collapse;font-size:.81rem'>
<tr style='border-bottom:1px solid #30363d'>
  <th style='color:#8b949e;padding:4px;text-align:left'>Strategy</th>
  <th style='color:#8b949e;padding:4px'>Asset</th>
  <th style='color:#8b949e;padding:4px'>TF</th>
  <th style='color:#8b949e;padding:4px'>Edge</th>
</tr>
<tr style='border-bottom:1px solid #21262d'>
  <td style='padding:4px;color:#e6edf3'>★ Volatility Regime Momentum</td>
  <td style='padding:4px;color:#8b949e;text-align:center'>BankNifty·Nifty50</td>
  <td style='padding:4px;color:#8b949e;text-align:center'>15m/1mo</td>
  <td style='padding:4px;color:#3fb950;text-align:center'>65–75%</td>
</tr>
<tr style='border-bottom:1px solid #21262d'>
  <td style='padding:4px;color:#e6edf3'>★ Order Flow Imbalance</td>
  <td style='padding:4px;color:#8b949e;text-align:center'>BTC·ETH·BankNifty</td>
  <td style='padding:4px;color:#8b949e;text-align:center'>5m/1d</td>
  <td style='padding:4px;color:#3fb950;text-align:center'>60–70%</td>
</tr>
<tr>
  <td style='padding:4px;color:#e6edf3'>★ Kalman Mean Reversion</td>
  <td style='padding:4px;color:#8b949e;text-align:center'>Gold·Silver·USDINR</td>
  <td style='padding:4px;color:#8b949e;text-align:center'>1h/3mo</td>
  <td style='padding:4px;color:#3fb950;text-align:center'>Sharpe ~2.0</td>
</tr>
</table>
<h4 style='color:#f0883e;margin-top:14px'>⚠️ Risk Rules</h4>
<ul style='color:#8b949e;font-size:.81rem;line-height:2;margin:0'>
  <li>Max <b style='color:#e6edf3'>1–2% capital</b> per trade · Min <b style='color:#e6edf3'>1:2 R:R</b></li>
  <li>Position size: <b style='color:#58a6ff'>Qty = (Capital × 1%) / (ATR × 1.5)</b></li>
  <li>Backtest 200+ trades before live capital · Volume confirms every breakout</li>
</ul>
</div>""", unsafe_allow_html=True)
...
def main():
    st.markdown("""<div style='display:flex;align-items:center;gap:12px;
         padding:3px 0 10px;border-bottom:1px solid #21262d;margin-bottom:10px'>
      <span style='font-size:1.9rem'>📊</span>
      <div>
        <div style='font-size:1.5rem;font-weight:900;color:#e6edf3;letter-spacing:1px'>QuantAlgo Pro</div>
        <div style='font-size:.64rem;color:#8b949e;letter-spacing:2px'>PROFESSIONAL ALGORITHMIC TRADING · v4.0</div>
      </div>
      <div style='margin-left:auto;display:flex;gap:8px;align-items:center'>
        ...
        <div style='background:#1c2128;color:#58a6ff;padding:3px 10px;border-radius:20px;font-size:.68rem'>14 Strategies · 1m/5d default</div>
      </div>
    </div>""", unsafe_allow_html=True)
