...
for _k,_v in {
    ...
    "last_data_key":"","last_strat_key":"","last_signal_candle":None,"last_exit_time":None,
...
def detailed_signal_status(df, strat, sp, sigs):
    """Returns rich HTML card showing:
    - Last signal status
    - ALL current indicator values
    - Exact conditions required for BUY and SELL signals
    - How far current values are from triggering
    """
    if df is None or len(df) < 2: return ""
    close = df["Close"]; cur = float(close.iloc[-1])
    last_sig = int(sigs.iloc[-1]) if sigs is not None and not sigs.empty else 0

    rows = []  # list of (label, current_val, needed_for_buy, needed_for_sell, gap_buy, gap_sell)

    try:
        if strat == "EMA Crossover":
            fast = sp.get("ema_fast",9); slow = sp.get("ema_slow",15)
            ef = float(_ema(close,fast).iloc[-1]); es = float(_ema(close,slow).iloc[-1])
            gap = ef - es; gap_pct = gap/es*100
            rows.append(("EMA Fast",  f"{ef:,.2f}", f"> EMA Slow (cross above)", f"< EMA Slow (cross below)", None, None))
            rows.append(("EMA Slow",  f"{es:,.2f}", "—", "—", None, None))
            rows.append(("EMA Gap",   f"{gap:+.2f} ({gap_pct:+.3f}%)",
                f"Gap must flip from {'-' if gap<0 else '+'} to + (cross)",
                f"Gap must flip from {'-' if gap<0 else '+'} to − (cross)",
                f"{'✅ Bullish bias' if gap>0 else f'Need +{abs(gap):.2f} pts shift'}",
                f"{'✅ Bearish bias' if gap<0 else f'Need −{abs(gap):.2f} pts shift'}"))

        elif strat == "RSI Strategy":
            period = sp.get("rsi_period",14); ob = sp.get("rsi_ob",70); os_ = sp.get("rsi_os",30)
            r = float(_rsi(close,period).iloc[-1])
            gap_buy = r - os_; gap_sell = ob - r
            rows.append(("RSI",        f"{r:.2f}", f"Cross BELOW {os_} (oversold)", f"Cross ABOVE {ob} (overbought)", None, None))
            rows.append(("Oversold (OS)", f"{os_}", "← Need RSI here to BUY", "—", None, None))
            rows.append(("Overbought (OB)", f"{ob}", "—", "← Need RSI here to SELL", None, None))
            rows.append(("Gap to BUY",  f"{gap_buy:+.2f} pts",
                "Need RSI to fall by this much" if gap_buy>0 else "✅ Already in BUY zone",
                "—", None, None))
            rows.append(("Gap to SELL", f"{gap_sell:+.2f} pts",
                "—",
                "Need RSI to rise by this much" if gap_sell>0 else "✅ Already in SELL zone",
                None, None))

        elif strat == "Bollinger Bands":
            n = sp.get("bb_period",20); k = sp.get("bb_std",2.)
            up,mid,lo = _boll(close,n,k)
            uv = float(up.iloc[-1]); mv = float(mid.iloc[-1]); lv = float(lo.iloc[-1])
            bw = (uv-lv)/mv*100
            rows.append(("Close Price", f"{cur:,.2f}", f"Must cross ABOVE Lower Band", f"Must cross BELOW Upper Band", None, None))
            rows.append(("Upper Band",  f"{uv:,.2f}", "—", f"← Need price at {uv:,.2f}", None, None))
            rows.append(("Mid Band",    f"{mv:,.2f}", "—", "—", None, None))
            rows.append(("Lower Band",  f"{lv:,.2f}", f"← Need price at {lv:,.2f}", "—", None, None))
            rows.append(("Gap to Lower",f"{cur-lv:+.2f} pts",
                f"Price is {cur-lv:+.2f} above lower — must reach {lv:,.2f}" if cur>lv else "✅ At/below lower band",
                "—", None, None))
            rows.append(("Gap to Upper",f"{uv-cur:+.2f} pts", "—",
                f"Price is {uv-cur:.2f} below upper — must reach {uv:,.2f}" if cur<uv else "✅ At/above upper band",
                None, None))
            rows.append(("Band Width %",f"{bw:.2f}%", "Squeeze: watch for expansion", "Squeeze: watch for expansion", None, None))

        elif "Kalman" in strat:
            thr = sp.get("kf_thr",1.5)
            kf = _kalman(close); dev = close-kf
            z_ser = dev/dev.rolling(30).std().replace(0,np.nan)
            z = float(z_ser.iloc[-1]); kf_v = float(kf.iloc[-1])
            gap_buy = z-(-thr); gap_sell = thr-z
            rows.append(("Close Price",  f"{cur:,.2f}", "—", "—", None, None))
            rows.append(("Kalman Fair",  f"{kf_v:,.2f}", "—", "—", None, None))
            rows.append(("Deviation",    f"{cur-kf_v:+.2f} pts", "—", "—", None, None))
            rows.append(("Z-Score",      f"{z:.3f}",
                f"Must cross BELOW −{thr} (currently {z:+.3f})",
                f"Must cross ABOVE +{thr} (currently {z:+.3f})", None, None))
            rows.append(("Gap to BUY",   f"{gap_buy:+.3f} z",
                f"Need z to drop {gap_buy:.3f} more to reach −{thr}" if gap_buy>0 else "✅ In BUY zone",
                "—", None, None))
            rows.append(("Gap to SELL",  f"{gap_sell:+.3f} z", "—",
                f"Need z to rise {gap_sell:.3f} more to reach +{thr}" if gap_sell>0 else "✅ In SELL zone",
                None, None))

        elif "Volatility Regime" in strat:
            fast=sp.get("vrm_fast",10); slow=sp.get("vrm_slow",30)
            a=_atr(df,14); regime=a/a.rolling(20).mean().replace(0,np.nan)
            ef=_ema(close,fast); es=_ema(close,slow)
            rv=float(regime.iloc[-1]); ef_v=float(ef.iloc[-1]); es_v=float(es.iloc[-1])
            mf=float(close.pct_change(fast).iloc[-1]*100); ms=float(close.pct_change(slow).iloc[-1]*100)
            rows.append(("ATR Regime",   f"{rv:.3f}", f"Must be > 1.1 (currently {rv:.3f})", f"Must be > 1.1", None, None))
            rows.append(("EMA Fast",     f"{ef_v:,.2f}", f"Must be > EMA Slow (currently {'✅' if ef_v>es_v else '❌'})", "Must be < EMA Slow", None, None))
            rows.append(("EMA Slow",     f"{es_v:,.2f}", "—", "—", None, None))
            rows.append(("Momentum Fast",f"{mf:+.2f}%", "Must be > 0 (bullish)", "Must be < 0 (bearish)", None, None))
            rows.append(("Momentum Slow",f"{ms:+.2f}%", "Must be > 0 (bullish)", "Must be < 0 (bearish)", None, None))
            cond_buy  = rv>1.1 and mf>0 and ms>0 and ef_v>es_v
            cond_sell = rv>1.1 and mf<0 and ms<0 and ef_v<es_v
            rows.append(("ALL conds BUY", f"{'✅ ALL MET' if cond_buy else '❌ NOT ALL MET'}",
                "Regime>1.1 + MomFast>0 + MomSlow>0 + EMAFast>EMASlow",
                "—", None, None))
            rows.append(("ALL conds SELL",f"{'✅ ALL MET' if cond_sell else '❌ NOT ALL MET'}",
                "—",
                "Regime>1.1 + MomFast<0 + MomSlow<0 + EMAFast<EMASlow", None, None))

        elif "Liquidity Hunt" in strat:
            sw=sp.get("lh_swing_w",10); sp_pct=sp.get("lh_sweep_pct",0.05); mw=sp.get("lh_min_wick",0.30)
            ph,pl=_pivots(df,sw)
            sh=float(df["High"].where(ph).ffill().iloc[-1]) if ph.any() else 0
            sl_=float(df["Low"].where(pl).ffill().iloc[-1]) if pl.any() else 0
            a=float(_atr(df).iloc[-1])
            dist_h=abs(cur-sh); dist_l=abs(cur-sl_)
            rows.append(("Close Price",     f"{cur:,.2f}", "—", "—", None, None))
            rows.append(("Last Swing High", f"{sh:,.2f}", "—", f"Price wick above {sh:,.2f}, then close below → SHORT", None, None))
            rows.append(("Last Swing Low",  f"{sl_:,.2f}", f"Price wick below {sl_:,.2f}, then close above → LONG", "—", None, None))
            rows.append(("Dist to Swing H", f"{dist_h:,.2f} pts", "—", f"{'🎯 NEAR!' if dist_h<a*0.5 else f'Need {dist_h-a*0.5:.2f} pts closer'}", None, None))
            rows.append(("Dist to Swing L", f"{dist_l:,.2f} pts", f"{'🎯 NEAR!' if dist_l<a*0.5 else f'Need {dist_l-a*0.5:.2f} pts closer'}", "—", None, None))
            rows.append(("ATR (context)",   f"{a:,.2f}", f"Min sweep = {sh*sp_pct/100:.2f} pts beyond swing level", "—", None, None))
            rows.append(("Min Wick Ratio",  f"{mw:.0%}", "Rejection wick must be ≥ this % of candle range", "Same condition", None, None))

        else:
            # Generic fallback for any strategy
            atr_v = float(_atr(df).iloc[-1])
            rows.append(("Close Price", f"{cur:,.2f}", "See signal indicators below", "See signal indicators below", None, None))
            rows.append(("ATR(14)",     f"{atr_v:,.2f}", "Volatility context", "Volatility context", None, None))
            rows.append(("Signal",      "▲ BUY" if last_sig==1 else "▼ SELL" if last_sig==-1 else "NONE",
                "Signal active → will auto-enter on START", "Signal active → will auto-enter on START", None, None))

    except Exception as e:
        return f"<div class=\'ms\'>Signal status unavailable: {e}</div>"

    if not rows: return ""

    # Build HTML table
    sig_badge = (
        "<span style=\'background:#238636;color:#fff;padding:2px 8px;border-radius:4px;font-size:.75rem;font-weight:700\'>▲ BUY SIGNAL ACTIVE</span>"
        if last_sig==1 else
        "<span style=\'background:#da3633;color:#fff;padding:2px 8px;border-radius:4px;font-size:.75rem;font-weight:700\'>▼ SELL SIGNAL ACTIVE</span>"
        if last_sig==-1 else
        "<span style=\'background:#30363d;color:#8b949e;padding:2px 8px;border-radius:4px;font-size:.75rem\'>○ NO SIGNAL YET</span>"
    )

    header = f"""<div style=\'background:#161b22;border:1px solid #30363d;border-radius:8px;padding:12px;font-size:.8rem\'>
      <div style=\'display:flex;align-items:center;gap:10px;margin-bottom:8px\'>
        <b style=\'color:#e6edf3\'>{strat}</b> {sig_badge}
      </div>
      <table style=\'width:100%;border-collapse:collapse;font-size:.78rem\'>
      <tr style=\'border-bottom:1px solid #21262d\'>
        <th style=\'color:#58a6ff;padding:4px 6px;text-align:left;width:22%\'>Indicator</th>
        <th style=\'color:#e6edf3;padding:4px 6px;text-align:center;width:16%\'>Current Value</th>
        <th style=\'color:#3fb950;padding:4px 6px;text-align:left;width:31%\'>▲ BUY Condition Needed</th>
        <th style=\'color:#f85149;padding:4px 6px;text-align:left;width:31%\'>▼ SELL Condition Needed</th>
      </tr>"""

    body = ""
    for lbl, cur_v, buy_cond, sell_cond, *_ in rows:
        body += (f"<tr style=\'border-bottom:1px solid #21262d\'>"
                 f"<td style=\'color:#8b949e;padding:3px 6px\'>{lbl}</td>"
                 f"<td style=\'color:#ffe082;padding:3px 6px;text-align:center;font-weight:700\'>{cur_v}</td>"
                 f"<td style=\'color:#c9d1d9;padding:3px 6px\'>{buy_cond}</td>"
                 f"<td style=\'color:#c9d1d9;padding:3px 6px\'>{sell_cond}</td>"
                 f"</tr>")
...
def enter_position(direction,cfg,df=None):
    ...
    return dict(entry_price=entry,direction=direction,sl=sl,target=tgt,trail_sl=sl,
                qty=cfg["qty"],entry_dt=now_ist(),order_id=order_id,
...
def exit_position(exit_price,exit_reason,cfg):
    ...
    _dfn=st.session_state.current_data
    st.session_state.last_signal_candle=_dfn.index[-1] if (_dfn is not None and not _dfn.empty) else None
...
def run_backtest(df,signals,sl_type,sl_p,tgt_type,tgt_p,qty,strat_name="",conf=None):
    ...
    for i in range(1,len(df)):
        ...
        if not in_trade:
            ...
            # ── Max duration profit ───────────────────────────────
            max_prof_dur=conf.get("max_profit_duration_min",9999) if conf.get("max_profit_duration_enabled") else 9999
            if ep is None and max_prof_dur<9999 and cur_pnl>0:
                if hasattr(trade["entry_dt"],"timestamp") and hasattr(df.index[i],"timestamp"):
                    _dur_m=(df.index[i]-trade["entry_dt"]).total_seconds()/60
                    if _dur_m>=max_prof_dur: ep=float(c["Close"]); er=f"Max Profit Duration ({max_prof_dur}m)"
            # ── Max duration (losing) ──────────────────────────
...
class DhanBroker:
    ...
    def place_order(self,sym,qty,side,price=0,sl=0,tgt=0):
        ...
        return {"order_id":f"DHAN_{now_ist():%Y%m%d%H%M%S}","symbol":sym,"qty":qty,
...
def auto_trade_fragment(cfg,conf):
    ...
    today=now_ist().date(); ds=st.session_state.daily_stats
    ...
    st.markdown(f"""<div style='display:flex;gap:12px;align-items:center;background:#161b22;border:1px solid #30363d;border-radius:8px;padding:7px 12px;margin-bottom:6px;font-size:.81rem'>
      ...
      <span style='color:#8b949e;margin-left:auto;font-size:.68rem'>{now_ist().strftime("%H:%M:%S IST")}</span>
    ...
    if pos is not None and ltp>0:
        ...
        if not exited and conf.get("max_duration_enabled") and cur_pnl<0:
            dur_min=(now_ist()-pos["entry_dt"]).total_seconds()/60
        ...
        # Step Trail SL update
        if _entry_age >= 5 and not exited and cfg["sl_type"]=="Step Trail SL (N pts lock K pts)":
        ...
        # Drawdown Recovery
        if _entry_age >= 5 and not exited and cfg["sl_type"]=="Drawdown Recovery Exit (loss+recovery%)":
        ...
        # Max profit duration — exits long-running profitable trades
        if _entry_age >= 5 and not exited and conf.get("max_profit_duration_enabled") and cur_pnl>0:
            dur_min=(now_ist()-pos["entry_dt"]).total_seconds()/60
            if dur_min>=conf.get("max_profit_duration_min",60):
                exit_position(ltp,f"Max Profit Duration ({conf['max_profit_duration_min']}m)",cfg)
                st.info(f"⏱️ Profit trade exited after {dur_min:.1f}m @ {ltp:.2f}")
                exited=True; st.rerun()
        if _entry_age >= 5 and not exited and cfg["tgt_type"]=="Profit Erosion Exit (peak-erosion%)":
        ...
        # Standard SL / Target
        if _entry_age >= 5 and not exited:
    ...
    if pos:
        ...
        pnl_clr="#3fb950" if pnl_pts>=0 else "#f85149"; duration=str(now_ist()-pos["entry_dt"]).split(".")[0]
...
def sidebar():
    with st.sidebar:
        ...
        tsym=(st.text_input("Symbol","KAYNES.NS",key="csym").upper().strip() if tname=="✏️ Custom" else TICKERS[tname])
        ...
        with st.expander("⏱️ Trade Duration & Daily Limits", expanded=False):
            ...
            if conf["max_duration_enabled"]:
                conf["max_duration_min"]=st.slider("Max minutes in loss",1,60,15,key="mdm")
                st.caption(f"Auto-exits losing trade if held > {conf['max_duration_min']} minutes.")
            st.divider()
            conf["max_profit_duration_enabled"]=st.checkbox("Max Duration for Profitable Trade",value=False,key="mpd_en")
            if conf["max_profit_duration_enabled"]:
                conf["max_profit_duration_min"]=st.number_input("Max minutes in profit",1,1440,60,key="mpdm")
                st.caption(f"Auto-exits profitable trade after {conf['max_profit_duration_min']} minutes — locks in gains, stops holding winners too long.")
            st.divider()
...
def tab_live(cfg):
    ...
    st.markdown("### 🔔 Signal")
    # Detailed signal status card
    _status_html = detailed_signal_status(df, cfg["strat"], cfg["sp"], sigs)
    if _status_html:
        st.markdown(_status_html.replace("\'","'"), unsafe_allow_html=True)
        st.markdown("<div style='margin:4px'></div>", unsafe_allow_html=True)
...
def tab_history():
    ...
    dc,cc=st.columns(2)
    with dc: st.download_button("📥 Export CSV",filt.to_csv(index=False),file_name=f"live_{now_ist():%Y%m%d_%H%M}_IST.csv",mime="text/csv",key="hist_dl")
    with cc:
...
def _opt_score_accuracy(s, target_acc=70.0, min_trades=3):
    """Accuracy-First: hard gate on win rate, then rank by PF x expectancy x sample size."""
    import math
    mt = max(int(min_trades), 1)
    if not s or s["total"] < mt: return -999
    acc = s["acc"]; pf = min(s["pf"],10) if s["pf"]!=float("inf") else 10
    exp = s.get("exp",0); sw = min(math.log(s["total"]+1)/math.log(101),1.5)
    if acc < target_acc:
        return round(-100 + acc/max(target_acc,1)*50, 4)   # below gate = negative
    acc_bonus = (acc - target_acc)/100
    exp_f = max(exp,0)/(abs(s.get("aw",1) or 1)+1e-9) + 1
    return round((acc/100)*pf*(1+acc_bonus)*exp_f*sw, 4)

def _opt_score_expectancy(s, max_dd_pct=30.0, min_trades=3):
    """Expectancy+Risk: maximise avg profit/trade, penalise drawdown, reward sample size."""
    import math
    mt = max(int(min_trades), 1)
    if not s or s["total"] < mt: return -999
    exp = s.get("exp",0)
    if exp <= 0: return round(-100 + exp/10, 4)
    pf  = min(s["pf"],10) if s["pf"]!=float("inf") else 10
    dd_pct = abs(s["dd"])/(abs(s["tp"])+1e-9)*100
    if dd_pct > max_dd_pct:
        return round(exp*pf*0.5*(1-(dd_pct-max_dd_pct)/100), 4)
    sw = min(math.log(s["total"]+1)/math.log(max(int(min_trades),1)+1), 2.0)
    dd_q = max(1-(dd_pct/(max_dd_pct*2)), 0.2)
    return round(exp*pf*sw*dd_q, 4)
...
def tab_optimization(cfg):
    ...
    n_trials  = oc1.number_input("Combinations to test", 5, 10000, 40, step=10, key="opt_n")
    ...
    if "Accuracy" in opt_mode:
        ...
        target_acc=_c1.number_input("Target Win Rate %",1.,99.,70.,step=1.,key="opt_tacc")
        ...
        target_acc=_c1.number_input("Min Win Rate %",1.,99.,55.,step=1.,key="opt_macc")
    ...
    deep_search = st.checkbox(
        "🔬 DeepSearch — exhaustively test ALL combinations (ignores 'Combinations to test' input, may take several minutes)",
        value=False, key="opt_deep",
        help="Tests every single combination of TF × SL × Target × Filter × Direction. "
             "With default pools: ~2,000–5,000 combinations. Slow but finds the absolute best.")
    if deep_search:
        st.caption("⚠️ DeepSearch active — will test ALL combinations. "
                   "Progress bar shows estimated completion. Best results guaranteed.")
    ...
    if run_opt or run_all:
        ...
        for _strat_idx, _cur_strat in enumerate(_strats_to_run):
            ...
            if _deep:
                _nt = 999999
            elif _all_strats_mode:
                _nt = max(int(n_trials)//max(len(_strats_to_run),1), 8)
            else:
                _nt = int(n_trials)
            max_combos = (len(tf_pool)*len(_OPT_SL)*len(_OPT_TGT)
                          *len(_OPT_FILTERS)*len(dir_pool))

            while trial < min(_nt, max_combos):
                ...
                try:
                    ...
                    n_sigs = (sig!=0).sum()
                    if n_sigs == 0: continue   # truly no signals — skip

                    bt = run_backtest(df, sig, sl_type, actual_sl_p, tgt_type, actual_tgt_p,
                                      1, _cur_strat, conf)
                    # ── ALWAYS include if any trades at all (min_trades is soft) ──
                    if bt.empty: continue
                    s = compute_stats(bt)
                    if not s: continue

                    # Score with selected mode (min_trades used as soft penalty in score fn)
                    ...
                    results.append({
                        "Score":          round(score,4),
                        "Opt Mode":       ("Accuracy" if "Accuracy" in _om
                                           else "Expectancy" if "Expectancy" in _om
                                           else "Composite"),
        ...
        # ── Always show whatever was found — NEVER block with an error ────
        if not _all_strat_results:
            st.warning("⚠️ No completed trades found in any tested combination. "
                       "Try: wider timeframes (1d/1y or 1d/2y), enable 5y/10y data, "
                       "or increase 'Combinations to test'.")
            return
        ...
        # Warn if all scores are negative (criteria too strict — showing anyway)
        if df_opt["Score"].iloc[0] < 0:
            st.warning(
                f"⚠️ No combination met the strict criteria "
                f"(target accuracy / min trades). Showing {len(df_opt)} results anyway, "
                f"ranked by best-available score. Consider relaxing the accuracy target "
                f"or reducing min trades.")
        else:
