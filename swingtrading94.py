...
def tab_optimization(cfg):
    ...
    opt_tfs   = oc3.multiselect("Timeframes to include",
        ["1m","5m","15m","1h","1d"],
        default=["5m","15m","1h","1d"], key="opt_tfs")
    seed      = oc4.number_input("Random seed", 0, 9999, 42, key="opt_seed")
    ...
    if run_opt:
        ...
        # Filter TF combos to selected intervals
        tf_pool = [(iv,pd) for iv,pd in _OPT_TF_COMBOS if iv in opt_tfs]
        if not tf_pool: st.error("Select at least one timeframe."); return

        results = []; tested = set()
        data_cache = {}   # avoid re-fetching same ticker/tf/period
        prog = st.progress(0, "Optimizing…")
        status = st.empty()
        trial = 0

        while trial < int(n_trials) and trial < len(tf_pool)*len(_OPT_SL)*len(_OPT_TGT)*len(_OPT_FILTERS):
            ...
            key = f"{tf}|{period}|{sl_type}|{tgt_type}|{str(sorted(conf.items()))}"
            if key in tested: continue
            tested.add(key); trial += 1
            prog.progress(trial / int(n_trials), f"Trial {trial}/{int(n_trials)}  [{tf}/{period}]")

            try:
                dk = f"{cfg['tsym']}|{tf}|{period}"
                if dk not in data_cache:
                    status.caption(f"Fetching {cfg['tsym']} {tf}/{period}…")
                    df = fetch_data(cfg["tsym"], tf, period)
                    data_cache[dk] = df
                df = data_cache[dk]
                ...
                results.append({
                    ...
                    "SL Type":      sl_type[:22],
                    "Target":       tgt_type[:22],
                    "Filters":      (", ".join(k.replace("_enabled","") for k,v in conf.items() if v is True) or "None"),
                    "Win Rate":     f"{s['acc']:.1f}%",
                    "PF":           pf_v,
        ...
        df_opt = pd.DataFrame(results).sort_values("Score",ascending=False).reset_index(drop=True)
        df_opt.index += 1
        st.session_state["opt_results"] = df_opt
        st.session_state["opt_best"]    = df_opt.iloc[0].to_dict()
        st.success(f"✅ Tested {trial} combinations. Best score: {df_opt['Score'].iloc[0]:.4f}  |  Win Rate: {df_opt['Win Rate'].iloc[0]}  |  PF: {df_opt['PF'].iloc[0]}")
    ...
    with ac1:
        if st.button("✅ Apply BEST Combo to Sidebar", type="primary",
            ...
            st.success("✅ Best params applied to sidebar! Now click Backtest or Start Live Trading.")
            st.rerun()
    with ac2:
        ...
        if st.button("Apply Selected Rank", use_container_width=True, key="opt_apply_rank"):
            chosen_row = df_opt.iloc[chosen_rank-1].to_dict()
            _apply_opt_to_sidebar(chosen_row)
            st.success(f"✅ Rank #{chosen_rank} params applied!")
            st.rerun()
    with ac3:
        st.markdown("""<div class='icard' style='padding:8px'>
          <div style='color:#8b949e;font-size:.79rem;line-height:1.7'>
          After applying: the sidebar updates automatically. Go to
          <b style='color:#e6edf3'>📈 Backtest</b> → click ▶ Run Backtest, or
          <b style='color:#e6edf3'>📡 Live Trading</b> → click ▶ START.
          </div></div>""", unsafe_allow_html=True)

    st.divider()

    # ── Full results table ────────────────────────────────────────
    st.markdown(f"#### 📊 All Results ({len(df_opt)} combinations tested, ranked by Score)")
