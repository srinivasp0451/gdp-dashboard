    ...
    if _all_sr and len(_all_sr) > 1:
        ...
        for _si,(_sname,_sdf) in enumerate(_sorted_strats):
            ...
            with st.expander(
                ...
                with _ab1:
                    if st.button(f"✅ Apply Best for this Strategy", key=f"opt_sapply_{_si}"):
                        _rd = _best.to_dict(); _rd["_strat"] = _sname
                        _apply_opt_to_sidebar(_rd)
                        st.success(f"Applied: {_sname[:30]}"); st.rerun()
                with _ab2:
                    st.caption(f"{len(_sdf)} combinations · Best score {_best['Score']}")
        st.divider()
