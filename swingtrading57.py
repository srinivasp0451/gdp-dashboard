...
with col3:
    if st.session_state.live_running:
        ...
        st.metric("Status", "🟢 RUNNING", f"Last update: {int(time_since_fetch)}s ago")
    else:
...
with tab1:
    if st.session_state.live_running and st.session_state.live_data is not None:
        ...
        if st.session_state.last_fetch_time:
            ...
            if time_diff >= random.uniform(1.0, 1.5):
...
with tab2:
    if len(st.session_state.live_trades) > 0:
        ...
        for idx, trade in enumerate(st.session_state.live_trades):
            with st.expander(f"Trade #{idx + 1} - {trade['Type']} - P&L: {trade['PnL']:.2f}"):
