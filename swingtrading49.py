...
class TechnicalIndicators:
    ...
    def calculate_atr(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14) -> pd.Series:
        ...
        return atr.fillna(method='bfill').fillna(1)
...
class TradingAnalyzer:
    ...
    def fetch_data_with_retry(self, ticker: str, period: str, interval: str, max_retries: int = 3) -> Optional[pd.DataFrame]:
        ...
        - RSI and Volatility NaN values fixed with proper min_periods
        - Ratio calculation handles different data lengths properly
        - RSI Divergence detection implemented
        - Backtesting fixed with proper data handling
        - Auto-refresh positions every 2 seconds
        - Proper P&L calculation and updates
        - Entry, Target, and Stop-Loss clearly displayed
        
        ✅ **Visual Analysis:**
        - Price vs RSI with divergence highlighting
        - Price vs Volatility charts
        - Price vs Fibonacci levels visualization
        - Technical indicators (MACD, ADX, EMAs)
        - Ratio analysis with separate charts
        - Ticker1 vs Ticker2 price comparison (dual-axis)
        
        ✅ **RSI Divergence:**
        - Bullish divergence (price lower lows, RSI higher lows)
        - Bearish divergence (price higher highs, RSI lower highs)
        - Strength calculation and detailed description
        - Visual highlighting on charts
        
        ✅ **Paper Trading:**
        - Auto-refresh every 2 seconds
        - Real-time P&L updates
        - Entry, SL, and 3 targets (based on ATR)
        - Risk/Reward ratio calculation
        - Current parameter tracking (RSI, Vol, Z-Score)
        
        ✅ **Backtesting:**
        - Detailed trade history table
        - Entry/Exit dates and prices
        - Stop-loss and target levels
        - Exit reasons (Target/Stop/RSI)
        - Win/Loss tracking
        - Downloadable CSV results
        
        ✅ **Ratio Analysis:**
        - Handles tickers with different data lengths
        - Uses common date intersection
        - Dual-axis price comparison charts
        - Complete ratio metrics and Z-scores
...
if __name__ == "__main__":
    main()warning(f"Failed to fetch {ticker} ({interval}/{period}): {str(e)}")
                    return None
                time.sleep(2)
    ...
    def calculate_targets_stops(self, df: pd.DataFrame, action: str) -> Dict:
        ...
        return {
            ...
            'rr_ratio1': abs(target1 - current_price) / abs(current_price - stop_loss) if abs(current_price - stop_loss) > 0 else 1
        }
        ...
        if len(all_signals) > 0:
            ...
            with chart_tabs[2]:
                ...
                fig_tech = make_subplots(rows=3, cols=1, shared_xaxes=True,
                                        ...
                                        subplot_titles=(f'{st.session_state.ticker1_name} with EMAs', 'MACD', 'ADX'))
            ...
            if enable_ratio and ticker2:
                ...
                if ratio_keys:
                    ...
                    # Properly align data with different lengths
                    ...
                    if len(common_index) > 0:
                        ...
                        st.warning("No common dates found between the two tickers. They may have different trading schedules.")
                    ...
                    if len(df_bt) >= 100:
                        ...
                        if trades_bt:
                            ...
                            st.download_button(
                                ...
                                data=csv,
                                file_name=f"backtest_{st.session_state.ticker1_name}_{datetime.now(IST).strftime('%Y%m%d')}.csv",
                                mime="text/csv"
                            )
                        else:
                            st.info("No trades generated in backtest period")
                    else:
                        st.error("Insufficient data for backtesting. Need at least 100 data points.")

    else:
        st.info("👆 Configure your analysis and click 'Start Complete Analysis' to begin.")
        
        st.bar = st.progress(0)
