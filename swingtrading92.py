...
def run_trap_strategy(df, lookback=20):
    ...
    for i in range(lookback + 5, len(df)):
        ...
        # Wide Range Body Rule (> 1.2x ATR/Avg Range)
...
# --- FETCH DATA WITH RATE LIMIT SAFETY ---
def fetch_ticker_data(ticker, period, interval):
    time.sleep(1.0) # Hard rule: 1 second delay to avoid Yahoo Finance API crashes
    try:
...
# Interactive Rules Matrix mapping based on user criteria selection
intervals_configs = {
    "1m": ["1d", "5d", "7d"],
    "5m": ["1d", "5d", "7d", "1mo"],
    "15m": ["1d", "5d", "7d", "1mo"],
...
custom_sl_input = st.sidebar.number_input("Custom/Trailing SL Points Offset", min_value=1.0, max_value=1000.0, value=10.0, step=1.0)
...
custom_tp_input = st.sidebar.number_input("Custom Target Points Offset", min_value=1.0, max_value=5000.0, value=20.0, step=1.0)
        ...
        with st.spinner("Processing architectural market arrays..."):
            ...
            if raw_data.empty:
                ...
                if not found_signals:
                    ...
                    # Parse Backtest Outputs
                    records = []
                    for timestamp, sig_type, pattern, entry_p, conf in found_signals:
                        ...
                        records.append({
                            "Timestamp": timestamp, "Signal Type": sig_type, "Pattern Core": pattern,
                            "Entry Reference": entry_p, "Stop Loss Assignment": sl, "Target (TP1 70%)": tp, "Confidence Score": f"{conf}%"
                        })
                    ...
                    for rec in records:
                        ...
                        fig.add_trace(go.Scatter(
                            x=[rec['Timestamp']], y=[rec['Entry Reference']], mode='markers',
...
with tab2:
    ...
    st.caption("Auto-refresh cycle uses optimized isolated components to update variables without interface lag.")
    ...
    def live_streaming_fragment():
        ...
        # Network call using 1-second rule restriction
        ...
        if found_signals:
            ...
            # Ensure signal occurred within the last 3 candles to represent an active execution state
            if processed_df.index.get_loc(last_sig[0]) >= len(processed_df) - 3:
                current_signal = last_sig[1]
                pattern_detected = last_sig[2]
                entry_price = float(last_sig[3])
                confidence = last_sig[4]
                
                idx_pos = processed_df.index.get_loc(last_sig[0])
        ...
        # Display Execution Matrix Data
        with col2:
            st.subheader("Dynamic Execution Matrix Matrix")
            ...
            # Simulated Ledger Engine Update Logic
            if current_signal in ["BUY", "SELL"]:
                trade_key = f"{ticker_symbol}_{last_sig[0]}"
                if trade_key not in st.session_state.active_trades:
                    ...
                    new_trade_entry = {
                        ...
                        "Hard Stop": calculated_sl,
                        "Primary Target": calculated_tp,
                        "70% Scaling Point (TP1)": round(calculated_tp, 2),
            ...
            # Live Plot Display
            fig_live = go.Figure()
            # Tail visual window to enhance responsiveness
