# ╔══════════════════════════════════════════════════════════════════╗
# ║       SMART INVESTING  –  Full Auto Algo Trading  v6            ║
# ║                                                                  ║
# ║  THE FUNDAMENTAL FIX:                                           ║
# ║  Streamlit re-executes the ENTIRE script on every rerun.        ║
# ║  Module-level variables (_D = {...}) are RE-INITIALIZED each    ║
# ║  rerun, so any thread state stored there gets wiped.            ║
# ║                                                                  ║
# ║  Solution: Use a SINGLETON pattern — the dict is only created   ║
# ║  ONCE via a module-level guard, and survives reruns because      ║
# ║  Python modules are only imported once per process.             ║
# ║  The thread reads/writes this singleton. The UI syncs from it.  ║
# ╚══════════════════════════════════════════════════════════════════╝
...
# ════════════════════════════════════════════════════════════════════
# SINGLETON LIVE-STATE STORE
# This block runs once per Python process, NOT once per Streamlit rerun.
# The 'if "_SI" not in globals()' guard ensures _SI is created only once.
# The background thread reads/writes _SI safely via _SI_LOCK.
# The Streamlit main thread reads _SI (never writes running/trades/pos).
# ════════════════════════════════════════════════════════════════════
if "_SI" not in globals():
...
def _g(k, d=None):
    with _SI_LOCK: return _SI.get(k, d)

def _s(k, v):
    with _SI_LOCK: _SI[k] = v
...
# ════════════════════════════════════════════════════════════════════
# SESSION STATE — UI-only keys (survive reruns via session_state)
# ════════════════════════════════════════════════════════════════════
for _k, _v in {
...
.ltptick{color:#90cdf4;font-size:11px}.ltpval{color:#e2e8f0;font-size:24px;font-weight:700}
.ltpup{color:#48bb78;font-size:13px;font-weight:600}.ltpdn{color:#fc8181;font-size:13px;font-weight:600}
...
.bdg{display:inline-block;padding:3px 12px;border-radius:16px;font-size:11px;font-weight:700;color:white;margin-bottom:6px}
...
# ════════════════════════════════════════════════════════════════════
# DATA FETCH  (called from background thread only)
# ════════════════════════════════════════════════════════════════════
def _rate_wait():
    with _SI_LOCK:
        gap = time.time() - _SI["_rl"]
        if gap < 1.5:
            time.sleep(1.5 - gap)
        _SI["_rl"] = time.time()
...
def _fetch_thread(symbol, interval, period):
    """yfinance fetch — only call from background thread."""
    _rate_wait()
    try:
...
def fetch_ui(symbol, interval, period):
    """yfinance fetch for backtest / optimization — main thread, no rate-wait."""
    try:
        raw = yf.Ticker(symbol).history(
            period=MAX_FETCH.get(interval, period),
            interval=interval, auto_adjust=True, prepost=False)
...
def ema_sig(df, idx, cfg):
    ...
    if cfg.get("use_angle", False):
        if ema_angle(df["ef"].iloc[:idx + 1]) < float(cfg.get("min_angle", 0)):
            return None, None
...
# ════════════════════════════════════════════════════════════════════
# ELLIOTT WAVE — wave-by-wave detail + EW rules + confidence score
# ════════════════════════════════════════════════════════════════════
    ...
    for p in pts:
        ...
        if cl[-1]["t"] == p["t"]:
            if (p["t"] == "H" and p["px"] > cl[-1]["px"]) or \
               (p["t"] == "L" and p["px"] < cl[-1]["px"]): cl[-1] = p
        else: cl.append(p)
    return cl

def _fib_q(ratio, targets):
    if ratio <= 0: return 0
    return max(0, 1 - min(abs(ratio - t) / t for t in targets) / 0.15)

def detect_ew(df):
    BLANK = {"ok": False, "status": "Insufficient data", "direction": "UNCLEAR",
             "wave_type": "—", "waves": [], "rules": [], "confidence": 0,
             "current_wave": "—", "wave_detail": [], "next_targets": {},
             "after_msg": "Need ≥25 candles", "current_price": 0,
             "trade_bias": "", "recommendation": None}
    if len(df) < 25: return {**BLANK, "current_price": float(df["close"].iloc[-1])}
    cp = float(df["close"].iloc[-1])
    swings = _swings(df, win=max(3, len(df) // 40))
    if len(swings) < 4:
        return {**BLANK, "ok": True, "status": "Forming", "current_price": cp,
                "after_msg": "Wait for more swing pivots"}
    ...
    if len(swings) >= 6:
        for st0, label, sign in [("L", "BULLISH", 1), ("H", "BEARISH", -1)]:
            ...
            P = [pt["px"] for pt in s]
            W1 = sign * (P[1] - P[0]); W2 = sign * (P[2] - P[1])
            W3 = sign * (P[3] - P[2]); W4 = sign * (P[4] - P[3])
            W5 = sign * (P[5] - P[4])
            if W1 <= 0 or W3 <= 0: continue
            w2r = abs(W2) / W1; w3x = W3 / W1; w4r = abs(W4) / W3

            rules = [
                {"rule": "W3 not shortest",       "passed": W3 >= min(W1, max(W5, 0.01)), "detail": f"W1={W1:.1f} W3={W3:.1f} W5={W5:.1f}"},
                {"rule": "W2 retraces < 100% W1", "passed": w2r < 1.0,                    "detail": f"W2 retrace = {w2r*100:.1f}%"},
                {"rule": "W4 no overlap W1",      "passed": (P[4] > P[1]) if sign == 1 else (P[4] < P[1]), "detail": f"W4 end={P[4]:.0f} W1 end={P[1]:.0f}"},
                {"rule": "W3 ≥ 61.8% of W1",      "passed": W3 >= W1 * 0.618,             "detail": f"W3/W1 = {w3x:.2f}x"},
                {"rule": "W4 retrace < 100% W3",  "passed": w4r < 1.0,                    "detail": f"W4 retrace = {w4r*100:.1f}%"},
            ]
            n_pass = sum(1 for r in rules if r["passed"])
            if n_pass < 3: continue

            fib_q = (_fib_q(w2r, [0.382, 0.5, 0.618]) +
                     _fib_q(w3x, [1.0, 1.382, 1.618, 2.0, 2.618]) +
                     _fib_q(w4r, [0.236, 0.382, 0.5])) / 3
            confidence = min(100, int(n_pass / 5 * 60 + fib_q * 40))
            ...
            wave_detail = [
                ...
                {"label": "5", "type": "Impulse (forming)", "dir": "UP" if sign == 1 else "DOWN",
                 ...
                 "fib_note": "Currently forming — targets shown below", "color": "#76e4f7"},
            ]
            projs = {
                f"61.8% of W1": round(P[4] + sign * W1 * 0.618, 2),
                f"100% of W1":  round(P[4] + sign * W1, 2),
                f"161.8% W1":   round(P[4] + sign * W1 * 1.618, 2),
            }
            sl_s = round(P[4] - sign * abs(W1) * 0.05, 2)
            tgt_s = round(P[4] + sign * W1, 2)
            rec = {"signal": "BUY" if sign == 1 else "SELL", "entry": cp,
                   "wave_context": f"Wave 5 of {label.title()} 5-Wave Impulse",
                   "sl_suggestion": sl_s, "tgt_suggestion": tgt_s,
                   "sl_basis": f"Below W4 end ({P[4]:.2f})",
                   "tgt_basis": "W5 = 100% of W1", "confidence": confidence}
            ...
            return {
                ...
                "current_wave": "Wave 5 (forming)", "wave_detail": wave_detail,
                "rules": rules, "confidence": confidence, "next_targets": projs,
                "after_msg": "Expect ABC correction after Wave 5 completes",
                "current_price": cp, "trade_bias": "BUY" if sign == 1 else "SELL",
                "recommendation": rec,
            }
    ...
    if len(swings) >= 4:
        for st0, label, sign in [("H", "CORRECTIVE_DOWN", -1), ("L", "CORRECTIVE_UP", 1)]:
            ...
            rules = [
                {"rule": "B retraces A (20%–100%)", "passed": 0.2 < bret < 1.0,
                 "detail": f"B = {bret*100:.1f}% of A"},
                {"rule": "C moves in A's direction", "passed": C > 0,
                 "detail": f"C = {C:.1f}pts"},
            ]
            confidence = min(100, int(
                _fib_q(bret, [0.382, 0.5, 0.618, 0.786]) * 50 +
                _fib_q(cpct, [0.618, 1.0, 1.272, 1.618]) * 50))
            wave_detail = [
                {"label": "A", "type": "Impulse", "dir": "DOWN" if sign == -1 else "UP",
                 "start": P[0], "end": P[1], "size": round(abs(P[1]-P[0]),2),
                 "pct": round(A/P[0]*100,2) if P[0] else 0,
                 "fib_note": "Wave A — first leg of correction", "color": "#fc8181"},
                {"label": "B", "type": "Correction", "dir": "UP" if sign == -1 else "DOWN",
                 "start": P[1], "end": P[2], "size": round(abs(P[2]-P[1]),2),
                 "pct": round(bret*100,1),
                 "fib_note": f"{bret*100:.1f}% retrace of A  (ideal: 38.2%–78.6%)", "color": "#48bb78"},
                {"label": "C", "type": "Impulse (forming)", "dir": "DOWN" if sign == -1 else "UP",
                 "start": P[2], "end": P[3], "size": round(abs(P[3]-P[2]),2),
                 "pct": round(cpct*100,1),
                 "fib_note": f"Currently {cpct*100:.1f}% of A  (ideal: 61.8%–161.8%)", "color": "#fc8181"},
            ]
            c_tgt = round(P[2] - sign * A, 2)
            projs = {
                "61.8% of A":  round(P[2] - sign * A * 0.618, 2),
                "100% of A":   c_tgt,
                "127.2% of A": round(P[2] - sign * A * 1.272, 2),
            }
            rec = {"signal": "SELL" if sign == -1 else "BUY", "entry": cp,
                   "wave_context": f"Wave C  {'Bearish' if sign==-1 else 'Bullish'} ABC Correction",
                   "sl_suggestion": round(P[2] + sign * A * 0.1, 2),
                   "tgt_suggestion": c_tgt, "sl_basis": "Above Wave B high",
                   "tgt_basis": "C = A (equal move)", "confidence": confidence}
            ...
            return {
                ...
                "current_wave": "Wave C (forming)", "wave_detail": wave_detail,
                "rules": rules, "confidence": confidence, "next_targets": projs,
                "after_msg": "Expect new impulse wave after ABC completes",
                "current_price": cp, "trade_bias": "SELL" if sign == -1 else "BUY",
                "recommendation": rec,
            }

    return {**BLANK, "ok": True, "status": "Structure forming", "current_price": cp,
            "after_msg": "Not enough confirmed swing pivots yet"}
...
def run_backtest(df, cfg):
    ...
    for i in range(1, len(df)):
        ...
        if pos:
            ...
            ep = None; er = None; violated = False
            if pt == "buy":
                if row["low"] <= sl:
                    ep = sl; er = f"SL Low≤{sl:.2f}"
                    violated = tg_t != "Trailing Target" and row["high"] >= tgt
                elif tg_t != "Trailing Target" and row["high"] >= tgt:
                    ep = tgt; er = f"Target High≥{tgt:.2f}"
                elif ema_ex: ep = row["open"]; er = "EMA Rev exit"
            else:
                if row["high"] >= sl:
                    ep = sl; er = f"SL High≥{sl:.2f}"
                    violated = tg_t != "Trailing Target" and row["low"] <= tgt
                elif tg_t != "Trailing Target" and row["low"] <= tgt:
                    ep = tgt; er = f"Target Low≤{tgt:.2f}"
                elif ema_ex: ep = row["open"]; er = "EMA Rev exit"
            if ep is None and i == len(df) - 1: ep = row["close"]; er = "End of data"
            if ep is not None:
                pnl = round(((ep - pos["entry"]) if pt == "buy" else (pos["entry"] - ep)) * qty, 2)
                t = {"#": tnum, "Type": pt.upper(), "Entry Time": fdt(pos["et"]),
                     "Exit Time": fdt(row.name), "Entry": round(pos["entry"], 4),
                     "Exit": round(ep, 4), "SL": round(pos["si"], 4), "Target": round(pos["ti"], 4),
                     "Bar High": round(row["high"], 4), "Bar Low": round(row["low"], 4),
                     "Entry Reason": pos["reason"], "Exit Reason": er,
                     "PnL": pnl, "Qty": qty, "Violated": violated}
                trades.append(t)
                if violated: viol.append(t)
                pos = None
        if pos is None and pending:
            sig, rsn = pending; pending = None
            ep = float(row["open"]); sl, tgt = calc_sl_tgt(ep, sig, row, cfg); tnum += 1
            pos = {"type": sig, "entry": ep, "et": row.name,
                   "sl": sl, "si": sl, "tgt": tgt, "ti": tgt, "reason": rsn}
        if pos is None:
            if strat == "EMA Crossover":
                ...
                pos = {"type": "buy", "entry": ep, "et": row.name,
                       "sl": sl, "si": sl, "tgt": tgt, "ti": tgt, "reason": "Simple Buy"}
            elif strat == "Simple Sell":
                ep = float(row["close"]); sl, tgt = calc_sl_tgt(ep, "sell", row, cfg); tnum += 1
                pos = {"type": "sell", "entry": ep, "et": row.name,
                       "sl": sl, "si": sl, "tgt": tgt, "ti": tgt, "reason": "Simple Sell"}
            elif strat == "Elliott Wave" and i >= 20:
                ew = detect_ew(df.iloc[:i + 1]); bias = ew.get("trade_bias", "")
                if bias:
                    sig = "buy" if bias == "BUY" else "sell"
                    pending = (sig, f"EW: {ew.get('current_wave', '')}")
    return trades, viol
...
def run_opt(df, base_cfg, grid, metric="Accuracy%", min_tr=5, cb=None):
    ...
    for idx, combo in enumerate(combos):
        ...
        try:
            ...
            score = acc * 0.4 + min(rr, 5) * 4 * 0.3 + (20 if pnl > 0 else 0) * 0.3
...
# LIVE THREAD
# ════════════════════════════════════════════════════════════════════
def live_thread(cfg, symbol, dhan_client):
    """
    Pure Python thread — zero Streamlit calls.
    Reads/writes _SI only. Survives reruns because _SI is a singleton.
    """
    ...
    try:
        _rate_wait()
        raw2 = yf.Ticker(symbol).history(period="5d", interval="1d",
                                          auto_adjust=True, prepost=False)
        df2  = _clean(raw2)
    ...
    while _g("running", False):
        try:
            df = _fetch_thread(symbol, cfg["timeframe"], cfg["period"])
            if df is None or df.empty:
                _log("[WARN] no data — retrying")
                time.sleep(2)
                continue
            ...
            ef_p  = float((df.iloc[-2].get("ef",  float("nan")) if len(df) >= 2 else float("nan")) or float("nan"))
            es_p  = float((df.iloc[-2].get("es",  float("nan")) if len(df) >= 2 else float("nan")) or float("nan"))
            _s("ind", {"ef": ef_v, "es": es_v, "atr": atr_v,
                       "ef_p": ef_p, "es_p": es_p, "fast": fast, "slow": slow,
                       "ltp": ltp, "bt": df.index[-1],
            ...
            if pos:
                ...
                if ep is not None:
                    pnl = round(((ep - pos["entry"]) if pt == "buy" else (pos["entry"] - ep)) * qty, 2)
                    t = {"#": len(_g("trades", [])) + 1, "Type": pt.upper(),
                         "Entry Time": fdt(pos["et"]),
                    ...
                    _s("pos", None)
                    if use_dhan:
                        _log(f"EXIT order: {place_order(dhan_client, cfg, pt, ltp, is_exit=True)}")
            ...
            if _g("pos") is None:
                    ...
                    if trd:
                        try:
                            le = datetime.strptime(
                                trd[-1].get("Exit Time", "2000-01-01 00:00:00"),
                                "%Y-%m-%d %H:%M:%S")
                            le = IST.localize(le)
                ...
                # ── SIMPLE BUY/SELL: fire on FIRST tick, no boundary ──
                if strat == "Simple Buy" and cd_ok:
                    sl, tgt = calc_sl_tgt(ltp, "buy", lr, cfg)
                    _s("pos", {"type": "buy", "entry": ltp, "et": now,
                                "sl": sl, "si": sl, "tgt": tgt, "ti": tgt,
                                "reason": "Simple Buy"})
                    if use_dhan:
                        _log(f"ENTRY order: {place_order(dhan_client, cfg, 'buy', ltp)}")
                    _log(f"▲ BUY @{ltp:.2f} | SL:{sl:.2f} | Tgt:{tgt:.2f}")

                elif strat == "Simple Sell" and cd_ok:
                    sl, tgt = calc_sl_tgt(ltp, "sell", lr, cfg)
                    _s("pos", {"type": "sell", "entry": ltp, "et": now,
                                "sl": sl, "si": sl, "tgt": tgt, "ti": tgt,
                                "reason": "Simple Sell"})
                    if use_dhan:
                        _log(f"ENTRY order: {place_order(dhan_client, cfg, 'sell', ltp)}")
                ...
                # ── EMA/EW: execute pending from previous boundary ──
                    ...
                    _s("pos", {"type": sig, "entry": ep, "et": now,
                                "sl": sl, "si": sl, "tgt": tgt, "ti": tgt,
                                "reason": rsn})
                    if use_dhan:
                        _log(f"ENTRY order: {place_order(dhan_client, cfg, sig, ep)}")
                    ...
                    if at_bdy and cd_ok:
                        ...
                        if strat == "EMA Crossover":
                            ...
                            if sig and df.index[-1] != last_sig_bar:
                                ...
                                _log(f"◆ SIGNAL {sig.upper()} @ {fdt(df.index[-1])} → next candle open")
                        elif strat == "Elliott Wave":
                            ew   = _g("ew", {}); bias = ew.get("trade_bias", "")
                            if bias:
                                s2 = "buy" if bias == "BUY" else "sell"
                                pending = (s2, f"EW: {ew.get('current_wave', '')}")
                                _log(f"◆ EW SIGNAL {s2.upper()} → next candle open")

            time.sleep(1.5)

        except Exception as e:
            _log(f"[ERR] {e}")
            time.sleep(2)   # brief pause, keep running
...
# CHARTS
# ════════════════════════════════════════════════════════════════════
def make_chart(df, trades=None, title="", fast=9, slow=15, pos=None, h=520):
    ...
    if trades:
        for t in trades[:60]:
            try:
                ...
                ep_v = t.get("Entry", t.get("Entry Price", 0))
                xp_v = t.get("Exit",  t.get("Exit Price",  0))
                ...
                fig.add_trace(go.Scatter(x=[et], y=[ep_v], mode="markers+text",
                    ...
                    text=[f"E:{ep_v:.0f}"], textposition="top center",
                    textfont=dict(size=8, color=c), showlegend=False), row=1, col=1)
                fig.add_trace(go.Scatter(x=[xt], y=[xp_v], mode="markers+text",
                    marker=dict(symbol="x", size=9, color=pc),
                    text=[f"X:{xp_v:.0f}"], textposition="bottom center",
    ...
    if pos:
        try:
            ...
            fig.add_trace(go.Scatter(x=[pos["et"]], y=[pos["entry"]], mode="markers+text",
                marker=dict(symbol=sym, size=16, color=c, line=dict(color="white", width=2)),
                text=[f"ENTRY {pos['entry']:.2f}"], textposition="top center"), row=1, col=1)
            fig.add_hline(y=pos["sl"], line=dict(color="#fc8181", width=2, dash="dot"),
                annotation_text=f"SL {pos['sl']:.2f}", annotation_font=dict(color="#fc8181"), row=1, col=1)
            fig.add_hline(y=pos["tgt"], line=dict(color="#48bb78", width=2, dash="dot"),
                annotation_text=f"Tgt {pos['tgt']:.2f}", annotation_font=dict(color="#48bb78"), row=1, col=1)
        except Exception: pass
...
def pnl_chart(trades):
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
        height=240, margin=dict(l=0, r=0, t=20, b=0), font=dict(color="#e2e8f0"))
    return fig
...
def sidebar():
    ...
    with st.sidebar:
        ...
        pr  = TF_PERIODS[tf]
        period = st.selectbox("Period", pr, index=min(1, len(pr)-1), key="s_period")
        cfg.update(timeframe=tf, period=period)

        strat = st.selectbox("🧠 Strategy",
            ["EMA Crossover","Simple Buy","Simple Sell","Elliott Wave"], key="s_strat")
        cfg["strategy"] = strat
        if strat == "EMA Crossover":
            c1, c2 = st.columns(2)
            ...
            ct = st.selectbox("Crossover Type", ["Simple","Custom Candle","ATR Candle"], key="s_ct")
        ...
        cfg["sl_points"]   = (st.number_input("SL Points", 0.1, value=slp, step=0.5, key="s_slp")
                               if sl_t != "ATR Based" else slp)
        cfg["atr_sl_mult"] = (st.number_input("ATR Mult SL", 0.1, 10.0, 1.5, 0.1, key="s_asm")
                               if sl_t == "ATR Based" else 1.5)

        tg_t = st.selectbox("🎯 Target Type",
        ...
        if tg_t == "ATR Based Target":
            ...
            cfg["atr_tgt_mult"]  = 3.0; st.caption("Display only — no auto exit")
        else:
        ...
        if en_dhan:
            ...
            if opts:
                ...
                cfg["ce_id"]         = st.text_input("CE ID", key="s_ceid")
                cfg["pe_id"]         = st.text_input("PE ID", key="s_peid")
...
def ltp_banner(cfg):
    ...
    st.markdown(
        ...
        f'<div class="ltpmeta">{cfg.get("timeframe","")} · {cfg.get("period","")} · '
        f'{cfg.get("strategy","")} | {ts_s}</div>'
...
def cfg_box(cfg):
    st.markdown(
        ...
        f'<b>EMA:</b> Fast={cfg.get("fast_ema",9)} Slow={cfg.get("slow_ema",15)} | '
...
def style_df(df):
    ...
    def rc(row):
        pnl = row.get("PnL", 0)
        c   = ("rgba(72,187,120,0.10)" if pnl > 0
               else "rgba(252,129,129,0.10)" if pnl < 0 else "")
        return [f"background-color:{c}"] * len(row)
    styled = df.style.apply(rc, axis=1)
    if "PnL" in df.columns:
        styled = styled.map(
            lambda v: (f"color:{'#48bb78' if v > 0 else '#fc8181'};font-weight:700"
                       if isinstance(v, (int, float)) else ""), subset=["PnL"])
    if "Violated" in df.columns:
        styled = styled.map(
            lambda v: ("background-color:#2d1515;color:#fc8181;font-weight:700"
                       if v is True else ""), subset=["Violated"])
    return styled
...
def trade_stats(trades):
    ...
    al   = loss["PnL"].mean() if len(loss) else 0
    cols = st.columns(6)
    for col, lbl, val, color in [
        (cols[0], "Trades",    len(df),           "#e2e8f0"),
        (cols[1], "Winners",   len(wins),          "#48bb78"),
        (cols[2], "Losers",    len(loss),          "#fc8181"),
        (cols[3], "Accuracy",  f"{acc:.1f}%",      "#f6ad55"),
        (cols[4], "Total P&L", f"₹{tot:+.2f}",    "#48bb78" if tot >= 0 else "#fc8181"),
        (cols[5], "Avg W/L",   f"₹{aw:.1f}/₹{al:.1f}", "#76e4f7"),
    ]:
        col.markdown(
            f'<div class="mc"><div style="color:#a0aec0;font-size:11px">{lbl}</div>'
            f'<div style="color:{color};font-size:16px;font-weight:700;margin-top:2px">{val}</div>'
            f'</div>', unsafe_allow_html=True)
...
def wave_panel(ew):
    ...
    for w in detail:
        lbl   = w["label"]; wtype = w["type"]; wdir = w["dir"]
        s_px  = w["start"]; e_px  = w["end"];  size = w["size"]
        pct   = w.get("pct", 0); fib = w.get("fib_note", ""); color = w.get("color", "#a0aec0")
        darr  = "🔼" if "UP" in wdir else "🔽"
        ibg   = "#1a3a1a" if "Impulse" in wtype else "#3a1a1a"
        ibadge = "⚡ Impulse" if "Impulse" in wtype else "↩ Correction"
        st.markdown(
            f'<div class="wrow">'
            f'<span style="color:{color};font-weight:700;font-size:14px">Wave {lbl}</span> &nbsp;'
            f'<span style="background:{ibg};color:{color};padding:2px 8px;border-radius:4px;font-size:11px">'
            f'{ibadge} {darr}</span> &nbsp;'
            f'<span style="color:#e2e8f0;font-size:13px">{s_px:.2f} → {e_px:.2f}</span> &nbsp;'
            f'<span style="color:#a0aec0;font-size:12px">({size:.1f} pts, {pct:.1f}%)</span><br>'
            f'<span style="color:#718096;font-size:11px">📐 {fib}</span>'
            f'</div>', unsafe_allow_html=True)

def rules_panel(ew):
    rules = ew.get("rules", []); conf = ew.get("confidence", 0)
    if not rules: return
    conf_c = "#48bb78" if conf >= 70 else "#f6ad55" if conf >= 40 else "#fc8181"
    bar    = "█" * int(conf / 5) + "░" * (20 - int(conf / 5))
    st.markdown("##### ✅ EW Rules Check + Confidence")
    st.markdown(
        f'<div class="mc">'
        f'<span style="color:#a0aec0;font-size:11px">Confidence  </span>'
        f'<span style="color:{conf_c};font-size:20px;font-weight:700">{conf}%</span>'
        f'<div style="color:{conf_c};font-size:12px;letter-spacing:1px;margin:2px 0">{bar}</div>'
        f'<span style="color:#718096;font-size:10px">'
        f'≥70% = high-quality pattern | Use alongside EMA/price action for best results</span>'
        f'</div>', unsafe_allow_html=True)
    cols = st.columns(2)
    for i, r in enumerate(rules):
        icon = "✅" if r["passed"] else "❌"; rc2 = "#48bb78" if r["passed"] else "#fc8181"
        cols[i % 2].markdown(
            f'<div style="background:#0e1117;border-radius:6px;padding:6px 10px;margin:3px;font-size:11px">'
            f'{icon} <b style="color:{rc2}">{r["rule"]}</b><br>'
            f'<span style="color:#718096">{r["detail"]}</span>'
            f'</div>', unsafe_allow_html=True)

def rec_panel(ew, cfg):
    rec = ew.get("recommendation")
    if not rec: return
    sig   = rec["signal"]; entry = rec["entry"]
    sl_s  = rec["sl_suggestion"]; tgt_s = rec["tgt_suggestion"]
    ...
    st.markdown(
        ...
        f'<div style="color:#718096;font-size:10px">{tg_pts:.0f}/{sl_pts:.0f} pts</div></div></div>'
        f'<div style="background:#0e1117;border-radius:6px;padding:8px 12px;font-size:12px;color:#a0aec0">'
        f'💡 <b style="color:#f6ad55">Your SL:</b> {strat_sl}pts ({strat_sl_t}) &nbsp;|&nbsp; '
...
def tab_backtest(cfg):
    ...
    if st.button("▶ Run Backtest", type="primary", key="btn_bt"):
        ...
        with st.spinner("Running backtest…"):
            trades, viol = run_backtest(df, cfg)
    ...
    if viol:
        ...
        with st.expander("View Violations"):
            st.dataframe(style_df(pd.DataFrame(viol)), use_container_width=True)
    else:
        st.success("✅ No violations!")

    st.plotly_chart(make_chart(df.tail(500), trades=trades, title=f"Backtest — {cfg.get('ticker_name')}",
                               fast=fast, slow=slow, h=560), use_container_width=True, key="bt_chart")
    st.plotly_chart(pnl_chart(trades), use_container_width=True, key="bt_pnl")

    st.markdown(f"### 📋 Trade Log ({len(trades)} trades)")
    tdf = pd.DataFrame(trades)
    co  = ["#","Type","Entry Time","Exit Time","Entry","Exit","SL","Target",
           "Bar High","Bar Low","Entry Reason","Exit Reason","PnL","Qty","Violated"]
...
def tab_live(cfg):
    ...
    if start and not _g("running", False):
        ...
        dhan_client = st.session_state.get("dhan_client")   # captured here in main thread
        t = threading.Thread(
            target=live_thread,
            args=(cfg, cfg["symbol"], dhan_client),
            daemon=True)
        with _SI_LOCK: _SI["_thread"] = t
        t.start()
    ...
    if sq and _g("pos"):
        ...
        pnl = round(((ltp - pos["entry"]) if pos["type"] == "buy"
                     else (pos["entry"] - ltp)) * int(cfg.get("quantity", 1)), 2)
        t2  = {"#": len(_g("trades", [])) + 1, "Type": pos["type"].upper(),
               "Entry Time": fdt(pos["et"]),
               "Exit Time": datetime.now(IST).strftime("%Y-%m-%d %H:%M:%S"),
               "Entry": round(pos["entry"], 4), "Exit": round(ltp, 4),
               "SL": round(pos["si"], 4), "Target": round(pos["ti"], 4),
    ...
    # Status badge
    status = _g("status", "STOPPED")
    bc     = "#48bb78" if status == "RUNNING" else "#fc8181"
    ...
    if ind:
        ...
        spr_str = f"{spread:.2f}"
        spr_clr = "#48bb78" if spread >= 0 else "#fc8181"
        ...
        ec[2].markdown(f'<div class="ec"><div style="color:#a0aec0;font-size:11px">Spread</div><div style="color:{spr_clr};font-size:20px;font-weight:700">{spr_str}</div></div>', unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            f'📡 <b style="color:#76e4f7">Last Bar [{fdt(bt)}]</b>: '
            f'O:{ind.get("bo",0):.2f} H:{ind.get("bh",0):.2f} '
            f'L:{ind.get("bl",0):.2f} C:{ltp_v:.2f} | '
            f'Fetched: <b style="color:#48bb78">{ts_str} IST</b>'
            f'</div>', unsafe_allow_html=True)
    elif _g("running", False):
        st.info("⏳ Fetching first tick… (~2 seconds)")
    ...
    if pos:
        ...
        upnl = round(((ltp - pos["entry"]) if pos["type"] == "buy"
                      else (pos["entry"] - ltp)) * int(cfg.get("quantity", 1)), 2)
        bc2  = "#48bb78" if upnl >= 0 else "#fc8181"
        ptc  = "#48bb78" if pos["type"] == "buy" else "#fc8181"
        ...
        st.markdown(
            ...
            f'Entry: <b>{pos["entry"]:.2f}</b> | '
            f'LTP: <b style="color:{bc2}">{ltp:.2f}</b> | '
            f'SL: <b style="color:#fc8181">{pos["sl"]:.2f}</b> | '
            f'Target: <b style="color:#48bb78">{pos["tgt"]:.2f}</b> | '
            f'Unrealised P&L: <b style="color:{bc2}">₹{upnl:+.2f}</b> | '
            f'Time: {fdt(pos["et"])}<br>'
            f'<span style="color:#a0aec0;font-size:11px">Reason: {pos["reason"]}</span>'
            f'</div>', unsafe_allow_html=True)
    else:
    ...
    if df is not None and not df.empty:
        ...
        st.plotly_chart(make_chart(df.tail(300),
            title=f"Live — {cfg.get('ticker_name')}",
            fast=fast, slow=slow, pos=pos, h=500),
            use_container_width=True, key="live_chart")

    # ── Elliott Wave Full Panel ───────────────────────────────────
    st.markdown("---")
    ew = _g("ew", {})
    show_ew = (ew and ew.get("ok") and
               ew.get("status", "") not in ("", "Insufficient data", "Structure forming"))
    if show_ew:
        ...
        for col, lbl, val, color in [
            (wc[0], "Structure",    s,          dc),
            (wc[1], "Wave Type",    wtype,      dc),
            (wc[2], "Direction",    d,          dc),
            (wc[3], "Current Wave", cw,         "#e2e8f0"),
            (wc[4], "Confidence",   f"{conf}%", conf_c),
        ]:
            col.markdown(
                f'<div class="mc"><div style="color:#a0aec0;font-size:11px">{lbl}</div>'
                f'<div style="color:{color};font-size:13px;font-weight:700;margin-top:2px">{val}</div>'
                f'</div>', unsafe_allow_html=True)
        ...
        if ew.get("recommendation"):
            st.markdown("##### 📋 EW Trade Recommendation")
            rec_panel(ew, cfg)

        bias = ew.get("trade_bias", ""); am = ew.get("after_msg", "")
        if am:   st.info(f"💡 {am}")
        if bias: st.success(f"📊 EW Bias: **{bias}**  (combine with EMA confirmation for best results)")
        ...
        st.info("🌊 Elliott Wave will appear once enough candles load…")
    else:
    ...
    for ln in reversed(logs[-80:]):
        c = ("#fc8181"   if "ERR" in ln or "WARN" in ln
             ...
             else "#f6ad55" if "EXIT" in ln or "SIGNAL" in ln
             else "#a0aec0")
    ...
    if lt:
        ...
        trade_stats(lt)
        ltd = pd.DataFrame(lt)
        cs  = ["#","Type","Entry Time","Exit Time","Entry","Exit","SL","Target",
               "Entry Reason","Exit Reason","PnL","Qty"]
...
def tab_history(cfg):
    ...
    all_t = []
    for t in st.session_state.bt_trades:
        tc = t.copy(); tc["Source"] = "🔬 Backtest"; all_t.append(tc)
    for t in _g("trades", []):
        tc = t.copy(); tc.setdefault("Source", "🚀 Live"); all_t.append(tc)
    if not all_t:
    ...
    if not df.empty:
        ...
        co = ["#","Source","Type","Entry Time","Exit Time","Entry","Exit","SL","Target",
              "Entry Reason","Exit Reason","PnL","Qty","Violated"]
        sc = [c for c in co if c in df.columns]
        st.dataframe(style_df(df[sc]), use_container_width=True, height=450)
        st.download_button("⬇ CSV", df[sc].to_csv(index=False).encode(),
                           "history.csv", "text/csv", key="h_dl")
...
def tab_optimize(cfg):
    ...
    st.info("Grid-search → click ✅ Apply on any result to load settings into sidebar.")
    with st.expander("⚙️ Setup", expanded=True):
        ...
        if opt_strat == "EMA Crossover":
            ...
            sw_min = int(pc1.number_input("Swing win min", 2, 20,  3, key="opt_swm"))
            sw_max = int(pc2.number_input("Swing win max", 2, 30, 10, key="opt_swM"))
            sw_stp = max(1, int(pc3.number_input("Swing win step", 1, 10, 1, key="opt_swS")))
    ...
    if st.button("🚀 Run Optimization", type="primary", use_container_width=True, key="btn_opt"):
        with st.spinner("Fetching data…"):
            df_opt = fetch_ui(cfg.get("symbol", "^NSEI"), opt_tf, opt_period)
        ...
        grid = {"sl_points": frange(sl_min, sl_max, sl_stp),
                "target_points": frange(tg_min, tg_max, tg_stp)}
        if opt_strat == "EMA Crossover":
        ...
        def pcb(done, total):
            ...
            pt_txt.markdown(f"**{done:,}/{total:,}** ({done/total*100:.0f}%)")

        base = {**cfg, "strategy": opt_strat, "quantity": opt_qty}
        results = run_opt(df_opt, base, grid, metric=opt_metric,
                          min_tr=min_trades, cb=pcb)
    ...
    if not results: st.warning("No valid results. Try wider ranges."); return
    ...
    st.markdown(f"### 🏆 Top {min(len(display), 15)} Results — click ✅ Apply")
    ...
    for i, res in enumerate(display[:15]):
        ...
        if res.get("fast_ema") is not None:
            parts.append(f"F={int(res['fast_ema'])} S={int(res['slow_ema'])}")
        if res.get("ew_swing_window") is not None:
            parts.append(f"SwWin={res['ew_swing_window']}")
        ...
        rr  = float(res.get("RR", 0));        sc  = float(res.get("Score", 0))
        tr  = int(res.get("Trades", 0))
        ac  = "#48bb78" if acc >= desired_acc else "#f6ad55"
        ...
        if rc[6].button("✅ Apply", key=f"oa_{i}", use_container_width=True):
            if res.get("fast_ema") is not None:
                st.session_state["_aFE"] = int(res["fast_ema"])
                st.session_state["_aSE"] = int(res["slow_ema"])
                st.session_state["s_fe"] = int(res["fast_ema"])
                st.session_state["s_se"] = int(res["slow_ema"])
            if res.get("sl_points") is not None:
                st.session_state["_aSL"] = float(res["sl_points"])
                st.session_state["s_slp"] = float(res["sl_points"])
            if res.get("target_points") is not None:
                st.session_state["_aTG"] = float(res["target_points"])
                st.session_state["s_tgp2"] = float(res["target_points"])
            st.success(f"✅ Applied: {param_str}"); st.rerun()
    ...
    if sc2:
        for col in ["fast_ema","slow_ema","Trades","ew_swing_window"]:
            if col in rdf.columns:
                rdf[col] = rdf[col].apply(lambda x: int(x) if pd.notna(x) else x)
        st.dataframe(style_df(rdf[sc2]), use_container_width=True, height=360)
    if not rdf.empty:
        st.download_button("⬇ CSV", rdf.to_csv(index=False).encode(),
                           "opt.csv", "text/csv", key="opt_dl")

# ════════════════════════════════════════════════════════════════════
# MAIN
# Auto-refresh: rendered AFTER full page, so user sees content
# before it refreshes. 2s sleep in main thread is fine here —
# page is fully painted, and Streamlit buffers the next render.
# ════════════════════════════════════════════════════════════════════
def main():
    st.markdown(
        '<div class="hdr">'
        '<div><h1>📈 Smart Investing</h1>'
        '<p>Professional Algorithmic Trading · NSE · BSE · Crypto · Commodities</p></div>'
        '<div style="color:#90cdf4;font-size:12px;text-align:right">'
        'EMA · Elliott Wave · Full Automation · Dhan Broker</div></div>',
        unsafe_allow_html=True)

    cfg = sidebar()
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🔬 Backtesting", "🚀 Live Trading", "📚 Trade History", "🔧 Optimization"])
    with tab1: tab_backtest(cfg)
    with tab2: tab_live(cfg)
    with tab3: tab_history(cfg)
    with tab4: tab_optimize(cfg)

    # Auto-refresh ONLY when live trading is running
    if _g("running", False):
        time.sleep(2)
        st.rerun()
