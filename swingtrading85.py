# ╔══════════════════════════════════════════════════════════════╗
# ║        SMART INVESTING  –  Algo Trading Platform v2         ║
# ║  NSE · BSE · Crypto · Gold · Silver  |  Dhan Broker Ready  ║
# ╚══════════════════════════════════════════════════════════════╝
# Fixes in v2:
#  1. applymap → map  (pandas ≥2.1 compatible)
#  2. live_log & all live_* session_state used from background thread
#     now use module-level thread-safe dict (_TS) — no AttributeError
#  3. Simple Buy / Simple Sell fires IMMEDIATELY on every tick
#  4. IP auto-detection — no static IP required; shows steps to whitelist
...
# ══════════════════════════════════════════════════════════════════
# THREAD-SAFE MODULE-LEVEL STORE
# Background threads CANNOT safely access st.session_state.
# All live state lives in _TS; _sync() copies it to session_state
# on each Streamlit rerun so the UI sees fresh values.
# ══════════════════════════════════════════════════════════════════
_TS_LOCK = threading.Lock()
_TS: dict = {
    "live_running":False, "live_status":"STOPPED",
    "live_position":None, "live_trades":[],
    "live_data":None,     "live_ltp":None, "live_prev_close":None,
    "live_log":[],        "live_last_ts":None, "live_ew":{},
    "_rl_ts":0.0,
}

def _ts_get(key, default=None):
    with _TS_LOCK: return _TS.get(key, default)

def _ts_set(key, val):
    with _TS_LOCK: _TS[key] = val

def _ts_append(key, val, mx=300):
    with _TS_LOCK:
        _TS[key].append(val)
        if len(_TS[key]) > mx: _TS[key] = _TS[key][-mx:]
...
for _k,_v in {"live_thread":None,"backtest_trades":[],"backtest_violations":[],
              "backtest_df":None,"backtest_ran":False,"dhan_client":None}.items():
...
# ══════════════════════════════════════════════════════════════════
def ema_tv(series, n):
    """EMA seeded with first-n SMA — matches TradingView exactly."""
...
def detect_ew(df):
    ...
    if len(swings)<4:
        return {"status":"Forming","direction":"UNCLEAR","waves":[],"current_wave":"Accumulating",
                "wave_detail":{},"next_targets":{},"after_msg":"Wait for clearer pattern",
    ...
    if len(swings)>=4:
        for start,label,sign in [("H","CORRECTIVE_DOWN",-1),("L","CORRECTIVE_UP",1)]:
            ...
            return {"status":f"ABC Correction ({'Bearish' if sign==-1 else 'Bullish'})",
                    ...
                    "next_targets":{"C @ 61.8% A":round(p[2]-sign*amv*0.618,2),
                                    "C = A":round(p[2]-sign*amv,2)},
                    "after_msg":"Expect new impulse after ABC","swing_points":swings,"current_price":cp,
                    "trade_bias":"SELL" if sign==-1 else "BUY"}
    return {"status":"Structure Forming","direction":"UNCLEAR","waves":[],"current_wave":"Accumulating",
            "wave_detail":{},"next_targets":{},"after_msg":"Insufficient pivots",
            "swing_points":swings,"current_price":cp}
...
def register_ip_ui():
    ip=get_my_ip()
    st.info(
        f"📡 **Your current public IP:** `{ip}`\n\n"
        "**SEBI rule: IP must be whitelisted before placing orders on Dhan.**\n\n"
        "Steps:\n"
        "1. Login → [Dhan Console](https://console.dhan.co)\n"
        "2. **Profile → API → IP Whitelist → Add IP → Save**\n"
        "3. If your ISP gives a **dynamic IP** (changes every session), "
        "re-register each time or ask your ISP for a **static IP** / use a VPN with fixed IP."
    )
...
def live_thread(cfg,symbol):
    ...
    while _ts_get("live_running",False):
        try:
            ...
            # ── SL/Target check (vs LTP every tick) ────────────────
            if pos:
                sl=pos["sl"]; tgt=pos["tgt"]; pt=pos["type"]
                if sl_t=="Trailing SL":
                    trail=float(cfg.get("sl_points",10))
                ...
                if ep is not None:
                    ...
                    _ts_append("live_trades",t)
                    _ts_set("live_position",None)
            ...
            if _ts_get("live_position") is None:
                ...
                # Simple Buy/Sell: IMMEDIATE (fires on first tick, not candle boundary)
                ...
                # EMA/EW: execute pending (next candle open)
                    ...
                    # Candle-boundary signal generation
...
def style_df(df):
    """Row colouring by PnL. Uses .map() — pandas ≥2.1 compatible (no applymap)."""
    ...
    if "SL/Tgt Violated" in df.columns:
        styled=styled.map(
            lambda v:("background-color:#2d1515;color:#fc8181;font-weight:700" if v is True else ""),
...
def tab_backtest(cfg):
    ...
    st.info("📌 EMA: signal on candle N → entry at N+1 open  |  "
            "Buy: Low vs SL first (conservative), then High vs Target  |  "
            "Sell: High vs SL first, then Low vs Target")
    if st.button("▶ Run Backtest",type="primary",key="btn_bt"):
        ...
        if df is None or df.empty: st.error("❌ No data returned. Check ticker/interval/period."); return
    ...
    fast=int(cfg.get("fast_ema",9)); slow=int(cfg.get("slow_ema",15))
    if not trades: st.warning("No trades generated — try a different timeframe or strategy."); return

    trade_stats(trades); st.markdown("---")
    if violations:
        st.error(f"⚠️ **{len(violations)} SL/Target Violations** — both SL & Target hit same candle (uncertain execution order in live trading)")
    ...
    st.plotly_chart(make_chart(df.tail(500),trades=trades[:50],
...
# ══════════════════════════════════════════════════════════════════
def tab_live(cfg):
    _sync()   # pull _TS → session_state so display is always fresh
    ltp_banner(cfg)
    ...
    if df is not None and not df.empty:
        lr=df.iloc[-1]; ef=lr.get("ema_fast",float("nan")); es=lr.get("ema_slow",float("nan"))
        st.markdown(f"""<div style="background:#1a1f2e;border:1px solid #2d3748;border-radius:8px;
          ...
          O:{lr['open']:.2f} H:{lr['high']:.2f} L:{lr['low']:.2f} C:{lr['close']:.2f}
          &nbsp;|&nbsp; EMA({cfg.get('fast_ema',9)}): <b style="color:#f6ad55">{ef:.2f}</b>
          &nbsp; EMA({cfg.get('slow_ema',15)}): <b style="color:#76e4f7">{es:.2f}</b>
          &nbsp;|&nbsp; ATR: {lr.get('atr',0):.2f}
    ...
    if ew:
        ...
        if tgts:
            st.markdown("**Next Targets:**"); tcols=st.columns(len(tgts))
    ...
    else: st.info("🌊 Elliott Wave analysis will appear once live trading starts.")
    ...
    for ln in reversed(logs[-80:]):
        c=("#fc8181" if "ERR" in ln or "WARN" in ln
           else "#48bb78" if ("ENTRY" in ln or "STARTED" in ln)
    ...
    if lt:
        ...
        trade_stats(lt)
        ltd=pd.DataFrame(lt)
        cs=["Trade #","Type","Entry Time","Exit Time","Entry Price","Exit Price",
...
def main():
    st.markdown("""<div class="app-hdr">
      ...
      <div style="color:#90cdf4;font-size:12px;text-align:right">EMA · Elliott Wave · Dhan Broker · Automated</div>
    </div>""",unsafe_allow_html=True)
    cfg=sidebar()
