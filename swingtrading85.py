...
# ║  • Auto-refresh via HTML meta-refresh (no time.sleep block)      ║
    ...
    while _get("running", False):
        try:
            ...
            if pos:
                ...
                ep=None; er=None
                if pt == "buy":
                    if ltp <= sl:                                   ep=sl;  er=f"SL {ltp:.2f}≤{sl:.2f}"
                    elif tg_t!="Trailing Target" and ltp >= tgt:   ep=tgt; er=f"Tgt {ltp:.2f}≥{tgt:.2f}"
                else:
                    if ltp >= sl:                                   ep=sl;  er=f"SL {ltp:.2f}≥{sl:.2f}"
                    elif tg_t!="Trailing Target" and ltp <= tgt:   ep=tgt; er=f"Tgt {ltp:.2f}≤{tgt:.2f}"
...
# ════════════════════════════════════════════════════════════════════
# MAIN
# Auto-refresh via HTML meta-refresh — zero blocking, zero sleep.
# The browser auto-reloads the page every 3 seconds when live trading.
# This is the most reliable refresh method in Streamlit.
# ════════════════════════════════════════════════════════════════════
def main():
    st.markdown(
        '<div class="hdr">'
    ...
    # Uses HTML meta-refresh: browser re-requests the page every 3s.
    # NO time.sleep(), NO st.rerun() — completely non-blocking.
    # The thread keeps running in the background regardless of refreshes.
