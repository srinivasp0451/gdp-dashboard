...
def build_backtest_chart(df: pd.DataFrame, trades: list, cfg: dict) -> go.Figure:
    ...
    fig.add_trace(go.Candlestick(
        ...
        increasing=dict(line=dict(color="#00ff88"), fillcolor="rgba(0,255,136,0.18)"),
        decreasing=dict(line=dict(color="#ff4757"), fillcolor="rgba(255,71,87,0.18)"),
    ), row=1, col=1)
    ...
    # Volume
    vol_colors = ["rgba(0,255,136,0.35)" if c >= o else "rgba(255,71,87,0.35)"
...
def build_live_chart(df: pd.DataFrame, cfg: dict) -> go.Figure:
    ...
    fig.add_trace(go.Candlestick(
        x=df.index, open=df["Open"], high=df["High"],
        low=df["Low"], close=df["Close"], name="Price",
        increasing=dict(line=dict(color="#00ff88"), fillcolor="rgba(0,255,136,0.10)"),
        decreasing=dict(line=dict(color="#ff4757"), fillcolor="rgba(255,71,87,0.10)"),
    ), row=1, col=1)
    ...
    # Volume
    vol_colors = ["rgba(0,255,136,0.30)" if c >= o else "rgba(255,71,87,0.30)"
                  for c, o in zip(df["Close"], df["Open"])]
    fig.add_trace(go.Bar(
        x=df.index, y=df["Volume"], name="Volume",
        marker_color=vol_colors
    ...
    if running:
        with st.expander("📋 Active Configuration", expanded=False):
            rows = [
                ...
                ("Period",        cfg["period"]),
                ("Strategy",      cfg["strategy"]),
                ("Fast EMA",      str(cfg["fast_ema"])),
                ("Slow EMA",      str(cfg["slow_ema"])),
                ("SL Type",       cfg["sl_type"]),
                ("SL Points",     str(cfg["sl_points"])),
                ("Target Type",   cfg["target_type"]),
                ("Target Points", str(cfg["target_points"])),
                ("RR Ratio",      str(cfg["rr_ratio"])),
                ("Quantity",      str(cfg["quantity"])),
                ("Cooldown",      f"{cfg['cooldown']} s"),
                ("No Overlap",    str(cfg["no_overlap"])),
                ("Dhan Enabled",  str(cfg["dhan_enabled"])),
            ]
            st.dataframe(
