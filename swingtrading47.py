...
def format_time_ago(time_diff):
    ...
    if months > 0:
        ...
        if remaining_days > 0:
            return f"{months} month(s) and {remaining_days} day(s) ago"
        return f"{months} month(s) ago"
    elif days > 0:
        remaining_hours = hours % 24
        if remaining_hours > 0:
            return f"{days} day(s) and {remaining_hours} hour(s) ago"
        return f"{days} day(s) ago"
    elif hours > 0:
        remaining_minutes = minutes % 60
        if remaining_minutes > 0:
            return f"{hours} hour(s) and {remaining_minutes} minute(s) ago"
...
def safe_time_diff(dt1, dt2):
    """Safely calculate time difference handling timezone issues"""
    try:
        # Ensure both are timezone aware
        if dt1.tzinfo is None:
            dt1 = IST.localize(dt1)
        if dt2.tzinfo is None:
            dt2 = IST.localize(dt2)
        
        # Convert both to same timezone
        dt1 = dt1.astimezone(IST)
        dt2 = dt2.astimezone(IST)
        
        return dt1 - dt2
    except:
        # Fallback: remove timezone info and calculate
    ...
    if primary_tf:
        ...
        if nearby_levels:
            for level in nearby_levels[:3]:
                time_ago = safe_time_diff(datetime.now(IST), level['date'])
                ...
                summary_parts.append(
                    ...
                    f"Distance: {abs(distance_points):.2f} points ({abs(distance_pct):.2f}%) {'above' if distance_points > 0 else 'below'} | "
        ...
        if not extreme_events.empty:
            for _, event in extreme_events.iterrows():
                ...
                if event_idx < len(zscore_data) - 10:
                    ...
                    future_price = zscore_data.iloc[event_idx+10]['Price']
                    points_move = future_price - price
                    
                    if abs(z_val) > 2:
                        time_diff = safe_time_diff(pd.Timestamp.now(tz=IST), pd.to_datetime(event['DateTime']))
                        time_ago_str = format_time_ago(time_diff)
                        
                        if z_val < -2:
                            st.success(f"""
                            **Extreme Oversold Event (Z-Score = {z_val:.2f})**
                            
                            📅 **When**: {event['DateTime']} ({time_ago_str})
                            
                            💰 **Price Details**:
                            - Entry Price: ₹{price:.2f}
                            - Price 10 periods later: ₹{future_price:.2f}
                            - Movement: {abs(points_move):.2f} points ({abs(future_changes):.2f}%) {'UP ⬆️' if points_move > 0 else 'DOWN ⬇️'}
                            
                            📊 **Current Price**: ₹{zscore_data.iloc[-1]['Price']:.2f}
                            
                            🎯 **Signal**: {event['Mean_Reversion_Signal']}
                            
                            💡 **Pattern**: When Z-score drops below -2, price typically rebounds as it's too far below the mean. Historical data shows {abs(future_changes):.2f}% move in next 10 periods.
                            """)
                        elif z_val > 2:
                            st.warning(f"""
                            **Extreme Overbought Event (Z-Score = {z_val:.2f})**
                            ...
                            - Entry Price: ₹{price:.2f}
                            - Price 10 periods later: ₹{future_price:.2f}
                            - Movement: {abs(points_move):.2f} points ({abs(future_changes):.2f}%) {'DOWN ⬇️' if points_move < 0 else 'UP ⬆️'}
                            
                            📊 **Current Price**: ₹{zscore_data.iloc[-1]['Price']:.2f}
                            
                            🎯 **Signal**: {event['Mean_Reversion_Signal']}
                            
                            💡 **Pattern**: When Z-score exceeds +2, price typically pulls back to mean. Historical data shows {abs(future_changes):.2f}% move in next 10 periods.
                            """)
        ...
        if not high_vol_events.empty:
            for _, event in high_vol_events.iterrows():
                ...
                if event_idx < len(vol_data) - 10:
                    ...
                    future_price = vol_data.iloc[event_idx+10]['Price']
                    points_move = future_price - price
                    ...
                    **High Volatility Event ({event['Volatility_Bin']})**
                    ...
                    - Entry Price: ₹{price:.2f}
                    - Price 10 periods later: ₹{future_price:.2f}
                    - Net Movement: {abs(points_move):.2f} points ({abs(direction_move):.2f}%) {'UP ⬆️' if direction_move > 0 else 'DOWN ⬇️'}
                    - Average Move per Period: {avg_move:.2f}%
                    
                    📊 **Current Price**: ₹{vol_data.iloc[-1]['Price']:.2f}
                    
                    📈 **Volatility**: {vol:.1f}% (Annualized)
                    
                    💡 **Insight**: High volatility = High risk/reward. In the 10 periods after this event, average price swing was {avg_move:.2f}% per period, with net {direction_move:+.2f}% move ({abs(points_move):.2f} points).
                    """)
                ...
                with sub_tab1:
                    ...
                    if sr_levels:
                        ...
                        for level in sr_levels[:5]:
                            ...
                            summary = f"""
                            {level_type_emoji} **{level['type'].upper()}** @ ₹{level['level']:.2f}
                            - **Current Price**: ₹{current_price:.2f}
                            - **Distance**: {abs(distance_points):.2f} points ({abs(distance_pct):.2f}%) {'above current price' if distance_points > 0 else 'below current price'}
                            - **Strength**: {level['touches']} touches, sustained for {level['sustained']} periods
                            - **Formed**: {level['date'].strftime('%Y-%m-%d %H:%M IST')} ({time_ago_str})
                            """
                            
                            if level['type'] == 'support' and abs(distance_pct) < 2:
                                st.success(summary + "\n- **Action**: Price near support - Watch for bounce")
                            elif level['type'] == 'resistance' and abs(distance_pct) < 2:
                                st.warning(summary + "\n- **Action**: Price near resistance - Watch for rejection")
                            else:
                                st.info(summary)
                    else:
