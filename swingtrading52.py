...
class TradingSystem:
    ...
    def calculate_sl_target(self, entry_price: float, position_type: str) -> Tuple[float, float]:
        ...
        if position_type == 'LONG':
            if self.sl_type == 'Custom Points':
                sl = entry_price - self.sl_value
            else:
                sl = entry_price - self.sl_value
            
            if self.target_type == 'Custom Points':
                target = entry_price + self.target_value
            else:
                target = entry_price + self.target_value
        else:  # SHORT
            if self.sl_type == 'Custom Points':
    ...
    def get_market_status(self, position: Dict, current_price: float) -> str:
        ...
        if 'Trail SL' in self.sl_type and pnl_pct > 0:
...
def main():
    ...
    with st.sidebar:
        ...
        if strategy_type == "EMA/SMA Crossover":
            ...
            ticker2 = st.text_input("Second Ticker:", "^NSEBANK")
        ...
        sl_type = st.selectbox("Stop Loss Type:", ["Custom Points", "Trail SL", "Signal Based"])
        if sl_type != "Signal Based":
            sl_value = st.number_input("SL Points:", min_value=1.0, value=50.0, step=5.0)
        else:
            sl_value = 0
        
        target_type = st.selectbox("Target Type:", ["Custom Points", "Trail Target", "Signal Based"])
