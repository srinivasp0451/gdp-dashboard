"""
streamlit_swing_walkforward_final.py

Full self-contained Streamlit app:
- Uses your original strategy rules (SMA20/50, RSI, MACD, BB mid, Volume surge, EMA trend).
- No TA-Lib required (pure pandas / numpy).
- Walk-forward optimization (choose best params per train window by Profit Factor).
- Real P/L simulation for each signal (SL/TP/indicator invalidation).
- Aggregates test trades and shows Profit Factor, Win Rate, Net Profit.
- Continuation check (yesterday -> today) using most recent train window's best params.
- Safe column mapping, safe slider defaults, optional param combo limit for speed.
Paste into a file and run: streamlit run streamlit_swing_walkforward_final.py
"""
...
st.set_page_config(page_title="Swing Walk-Forward Final", layout="wide")
st.title("Swing Trading — Walk-Forward Backtest (Final)")
...
# -------------------------
# Produce signals using your original rules
# -------------------------
def produce_signals(df, params):
    """
    Returns DataFrame of entry signals:
    columns: EntryIdx, EntryDate, Type ('Long'/'Short'), Entry, SL, TP, Reasons
    """
...
# -------------------------
# Simulate outcomes (SL / TP / indicator invalidation)
# -------------------------
def simulate_outcomes(df, signals, max_holding_days=20):
    """
    For each signal, scan forward until:
    - TP hit (use High >= TP)
    - SL hit (use Low <= SL)
    - Indicator invalidation (SMA crossover) -> exit at close of that day
    - If none within max_holding_days -> exit at last close
    Returns trades dataframe with P/L and exit info, gross_profit and gross_loss (abs)
    """
    if signals.empty:
    ...
    for _, s in signals.iterrows():
        ...
        for j in range(idx + 1, min(idx + 1 + max_holding_days, len(df))):
            ...
            date = df.iloc[j]['Date']
            sma20 = df.iloc[j]['SMA20']
            sma50 = df.iloc[j]['SMA50']

            # TP / SL checks
            if direction == 'Long':
                if pd.notna(tp) and high >= tp:
                    exit_price = tp; exit_date = date; pnl = exit_price - entry_price; hit = 'TP'
                    gross_profit += max(0.0, pnl)
                    break
                if pd.notna(sl) and low <= sl:
                    exit_price = sl; exit_date = date; pnl = exit_price - entry_price; hit = 'SL'
                    gross_loss += min(0.0, pnl)
                    break
                # invalidation: SMA20 < SMA50
                if pd.notna(sma20) and pd.notna(sma50) and sma20 < sma50:
                    exit_price = close; exit_date = date; pnl = exit_price - entry_price; hit = 'Invalidation'
                ...
                if pd.notna(tp) and low <= tp:
                    exit_price = tp; exit_date = date; pnl = entry_price - exit_price; hit = 'TP'
                    gross_profit += max(0.0, pnl)
                    break
                if pd.notna(sl) and high >= sl:
                    exit_price = sl; exit_date = date; pnl = entry_price - exit_price; hit = 'SL'
                    gross_loss += min(0.0, pnl)
                    break
                # invalidation: SMA20 > SMA50
                if pd.notna(sma20) and pd.notna(sma50) and sma20 > sma50:
        ...
        # if no exit found, close at last available close within window
        if exit_price is None:
            idx_close = min(idx + max_holding_days, len(df) - 1)
            exit_price = float(df.iloc[idx_close]['Close'])
            exit_date = df.iloc[idx_close]['Date']
...
def run_optimization(train_df, param_grid, max_param_combos=0):
    ...
    for params in param_grid:
        ...
        if (pf > best_pf) or (pf == best_pf and stats['total_trades'] > (best_result[1]['total_trades'] if best_result[1] else 0)):
            best_pf = pf
            best_result = (param_obj, stats, trades)
    return best_result  # (params, stats, trades) or (None,None,None)
...
def continuation_check(df, best_params):
    ...
    # try find signal on yesterday's date
...
if not uploaded_file:
    st.info("Upload CSV with daily OHLCV. Column name variants (close, ltp, adj close) are supported.")
    st.stop()
...
# case-insensitive fallback renames
rename_map = {}
...
n = len(df)
if n < 30:
    st.warning(f"Small dataset: {n} rows. Walk-forward may be noisy.")

# compute indicators
df = compute_indicators(df)
st.success("Indicators computed")

# parameter grid (expanded per your request)
...
st.sidebar.write(f"Total param combos: {len(param_grid)} (limit using the above field to speed up)")
...
if st.button("Run Walk-Forward + Live Recommendation"):
    ...
    if not wf_results:
        st.error("Walk-forward produced no valid windows/params. Try smaller test window or limit param combos.")
        st.stop()

    st.success("Walk-forward completed")
    st.write(f"Windows evaluated: {len(wf_results)}")

    # aggregate combined test trades metrics
    if not combined_test_trades.empty:
        combined_test_trades = combined_test_trades.copy()
        # compute cumulative P/L if present
        if 'P/L' in combined_test_trades.columns:
            combined_test_trades['Cum_PnL'] = combined_test_trades['P/L'].cumsum()
        net_profit = combined_test_trades['P/L'].sum() if 'P/L' in combined_test_trades.columns else None
        win_rate = (combined_test_trades['P/L'] > 0).mean() * 100 if 'P/L' in combined_test_trades.columns else None
        gross_profit = combined_test_trades.loc[combined_test_trades['P/L'] > 0, 'P/L'].sum() if 'P/L' in combined_test_trades.columns else 0.0
        gross_loss = combined_test_trades.loc[combined_test_trades['P/L'] < 0, 'P/L'].sum() if 'P/L' in combined_test_trades.columns else 0.0
        profit_factor = (gross_profit / abs(gross_loss)) if (gross_loss != 0) else (np.inf if gross_profit > 0 else None)
    else:
        net_profit = 0.0; win_rate = 0.0; profit_factor = None

    st.header("Aggregate Walk-Forward Test Results")
    st.write(f"Net Profit (points): {net_profit:.2f}" if net_profit is not None else "Net Profit: N/A")
    st.write(f"Weighted Win Rate: {win_rate:.2f}%" if win_rate is not None else "Win Rate: N/A")
    ...
    # combined trades table & equity plot
    if not combined_test_trades.empty:
        st.subheader("Combined Test Trades (sample)")
        display_cols = ['EntryDate','ExitDate','Type','Entry','Exit','P/L','Hit']
        available = [c for c in display_cols if c in combined_test_trades.columns]
        st.dataframe(combined_test_trades[available].head(300))
    ...
    # most recent best params -> live recommendation
...
st.caption("If anything still errors: paste the full traceback and the CSV header (column names). I'll patch quickly.")
